...
public partial class Apply_OpenProve_Apply_OpenProve_Detail : BasePage
{
    ...
    public void LoadPage()
    {
        ...
        txtPhone.Text = dr["OfficeAutomation_Document_OpenProve_SignPhone"].ToString();
        ...
        else
        {
            ...
            for (int n = 0; n < detailCount; n++)
            {
                ...
                SbJs.Append("$('#rdoKind" + i + (dr["OfficeAutomation_Document_OpenProve_Detail_Kind"].ToString().Trim() == "1" ? "1" : dr["OfficeAutomation_Document_OpenProve_Detail_Kind"].ToString().Trim() == "2" ? "2" : dr["OfficeAutomation_Document_OpenProve_Detail_Kind"].ToString().Trim() == "3" ? "3" : "0") + "').attr('checked','checked');");
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        //{
            ...
            if (IsNewApply)
            {
                ...
                //RunJS("window.showModalDialog(\"/Apply/Apply_UploadFile.aspx?MainID=" + MainID + "\", '" + MainID + "', \"dialogHeight=165px\");var win=window.showModalDialog(\"Apply_OpenProve_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=800px;dialogHeight=320px\");if(win=='success'){window.location='/Apply/Apply.aspx'; }");
                if (!bs)
                {
                    ...
                    RunJS("alert('申请表现已保存成功。');window.location='../Apply.aspx'; ");
                }
                else
                    da_OfficeAutomation_Flow_Inherit.Insert(t_OfficeAutomation_Flow);//52100-2016/10/14
                    RunJS("alert('申请表现已保存成功！');var win=window.showModalDialog(\"Apply_OpenProve_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=800px;dialogHeight=320px\");if(win=='success'){window.location='../Apply.aspx'; }");
                #endregion
            }
            else
            {
                ...
                if (Purview.Contains("OA_ITPower_001"))//是否有编辑权限
                {
                    if (!Update())
                    {
                        ...
                        RunJS("alert('保存成功！');var win=window.showModalDialog(\"Apply_OpenProve_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=800px;dialogHeight=320px\");if(win=='success'){window.location='/Apply/Apply.aspx'; }");
                }
                else
                    Alert("未开通编辑修改权限。");
                #endregion
    ...
    private T_OfficeAutomation_Document_OpenProve GetModelFromPage(Guid UndertakeProjID)
    {
     ...
     //   t_OfficeAutomation_Document_OpenProve.OfficeAutomation_Document_OpenProve_Rank = txtRank.Text;
      //  t_OfficeAutomation_Document_OpenProve.open = txtPhone.Text;
        ...
        try
        {
            ...
            if (Purview.Contains("OA_ITPower_001"))//是否有编辑权限
            {
                ...
                if (!b)
                {
                    ...
                    RunJS("alert('所作的修改已保存！');var win=window.showModalDialog(\"Apply_OpenProve_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=800px;dialogHeight=320px\");if(win=='success'){window.location='/Apply/Apply.aspx'; }");
            }
            else
                Alert("未开通编辑修改权限。");
            #endregion
