...
public partial class Apply_BadDebts_Apply_BadDebts_Detail : BasePage
{
    ...
    public void LoadPage()
    {
        ...
        txtMoneyCount.Text = dr["OfficeAutomation_Document_BadDebts_MoneyCount"].ToString();
        ...
        else
        {
            ...
            for (int n = 0; n < detailCount; n++)
            {
                ...
                SbJs.Append("$('#txtAddress" + i + "').val('" + dr["OfficeAutomation_Document_BadDebts_Detail_Address"] + "');");
                SbJs.Append("$('#txtOwnerBadMoney" + i + "').val('" + dr["OfficeAutomation_Document_BadDebts_Detail_OwnerBadMoney"] + "');");
                SbJs.Append("$('#txtClientBadMoney" + i + "').val('" + dr["OfficeAutomation_Document_BadDebts_Detail_ClientBadMoney"] + "');");
                SbJs.Append("$('#txtBadReason" + i + "').val('" + dr["OfficeAutomation_Document_BadDebts_Detail_BadReason"] + "');");
                ...
                //SbJs.Append("$('#txtAutoBadDate" + i + "').val('" + DateTime.Parse(dr["OfficeAutomation_Document_BadDebts_Detail_AutoBadDate"].ToString()).ToString("yyyy-MM-dd") + "');");
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            if (IsNewApply)
            {
                ...
                t_OfficeAutomation_Document_BadDebts.OfficeAutomation_Document_BadDebts_ID = Guid.NewGuid();
                ...
                InsertBadDebtsDetail(t_OfficeAutomation_Document_BadDebts.OfficeAutomation_Document_BadDebts_ID);
                ...
                if (t_OfficeAutomation_Document_BadDebts.OfficeAutomation_Document_BadDebts_MoneyCount > 20000)
                {
        ...
        if (Purview.Contains("OA_ITPower_002"))
        {
            try
            {
                ...
                if (flowIDx == "6")
                {
                    ...
                    InsertBadDebtsDetail(new Guid(ID));
                }
    ...
    protected void btnSignSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (EmployeeID == CommonConst.EMP_FINANCE_PUBLIC_ID || EmployeeID == "43781" || EmployeeID == "20208")
                da_OfficeAutomation_Document_BadDebts_Inherit.AddRemarkByID_II(ID, CommonConst.SIGN_FINANCE);
            Alert("标记成功。");
        }
        catch
        {
    ...
    protected void btnSignBad_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                da_OfficeAutomation_Document_BadDebts_Inherit.AddRemarkByID_II(ID, "已坏账");
                Alert("标记成功。");
            }
        }
        catch
    ...
    protected void btnUploadBadDebts_Click(object sender, EventArgs e)
    {
        ...
        if (path != "")
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                try
                {
                    string sReportID, sAddress, sOwnerBadMoney, sClientBadMoney, sBadReason, AutoBadDate;
                    bool bIsSpecialAdjust, IsAutoBad;
                    int success = 0;
                    for (i = 0; i <= ds.Tables[0].Rows.Count - 1; i++)
                    {
                        sReportID = ds.Tables[0].Rows[i]["成交报告编号"].ToString();
                        sAddress = ds.Tables[0].Rows[i]["成交地址"].ToString();
                        sOwnerBadMoney = ds.Tables[0].Rows[i]["业主坏账金额佣金"].ToString();
                        sClientBadMoney = ds.Tables[0].Rows[i]["客户坏账金额佣金"].ToString();
                        sBadReason = ds.Tables[0].Rows[i]["坏账原因"].ToString();
                        bIsSpecialAdjust = ds.Tables[0].Rows[i]["是否特殊调整"].ToString() == "是" ? true : false;
                        IsAutoBad = ds.Tables[0].Rows[i]["是否已自动坏账"].ToString() == "是" ? true : false;
                        AutoBadDate = ds.Tables[0].Rows[i]["自动坏账时间"].ToString();
    ...
    private void InsertBadDebtsDetail(Guid gBadDebtsID)
    {
        if (hdDetail.Value == "")
            return;
        ...
        for (int i = 0; i < details.Length; i++)
        {
            ...
            t_OfficeAutomation_Document_BadDebts_Detail.OfficeAutomation_Document_BadDebts_Detail_OwnerBadMoney = Decimal.Parse(detail[2]);
            t_OfficeAutomation_Document_BadDebts_Detail.OfficeAutomation_Document_BadDebts_Detail_ClientBadMoney = Decimal.Parse(detail[3]);
            t_OfficeAutomation_Document_BadDebts_Detail.OfficeAutomation_Document_BadDebts_Detail_BadReason = detail[4];
            t_OfficeAutomation_Document_BadDebts_Detail.OfficeAutomation_Document_BadDebts_Detail_IsSpecialAdjust = detail[5] == "1";
            t_OfficeAutomation_Document_BadDebts_Detail.OfficeAutomation_Document_BadDebts_Detail_IsAutoBad = detail[6] == "1";
            t_OfficeAutomation_Document_BadDebts_Detail.OfficeAutomation_Document_BadDebts_Detail_AutoBadDate = detail[7];
            t_OfficeAutomation_Document_BadDebts_Detail.OfficeAutomation_Document_BadDebts_Detail_Owner = "";
            t_OfficeAutomation_Document_BadDebts_Detail.OfficeAutomation_Document_BadDebts_Detail_Client = "";

            da_OfficeAutomation_Document_BadDebts_Detail_Inherit.Insert(t_OfficeAutomation_Document_BadDebts_Detail);
        }

        DA_OfficeAutomation_Document_BadDebts_Inherit da_OfficeAutomation_Document_BadDebts_Inherit = new DA_OfficeAutomation_Document_BadDebts_Inherit();
        da_OfficeAutomation_Document_BadDebts_Inherit.ReplaceRemarkByID(gBadDebtsID.ToString(), (details.Length > 3 ? "财务部处理" : "法律部处理"));
    }
