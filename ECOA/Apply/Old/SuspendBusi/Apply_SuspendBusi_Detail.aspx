<%@ Page ValidateRequest="false" Title="暂停营业申请表 - 广州中原电子审批系统" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="Apply_SuspendBusi_Detail.aspx.cs" Inherits="Apply_SuspendBusi_Apply_SuspendBusi_Detail" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    <script type="text/javascript">
        ...
        $(function() {    
            ...
            if($("#<%=rdbDefault1.ClientID %>").prop("checked")){
                $("#ShowRent").show();
            }
           
            $("#<%=rdbDefault1.ClientID %>").click(function() {
                $("#ShowRent").show();
            });
            $("#<%=rdbDefault2.ClientID %>").click(function() {
                $("#ShowRent").hide();
            });
            $("#<%=txtMoneyD1.ClientID%>").blur(function(){
                if (isNaN($("#<%=txtMoneyD1.ClientID%>").val())) {
                    alert("回收按金金额必须输入纯数字");
                    $("#<%=txtMoneyD1.ClientID%>").focus();
                    return false;
                }
                CaculateSumDefault();
            });
            $("#<%=txtMoneyD2.ClientID%>").blur(function(){
                if (isNaN($("#<%=txtMoneyD2.ClientID%>").val())) {
                    alert("回收其他押金金额必须输入纯数字");
                    $("#<%=txtMoneyD2.ClientID%>").focus();
                    return false;
                }
                CaculateSumDefault();
            });
            $("#<%=txtMoneyD3.ClientID%>").blur(function(){
                if (isNaN($("#<%=txtMoneyD3.ClientID%>").val())) {
                    alert("违约金额必须输入纯数字");
                    $("#<%=txtMoneyD3.ClientID%>").focus();
                    return false;
                }
                CaculateSumDefault();
            });
            $("#<%=rdbDeposit1.ClientID %>").click(function() {
                CaculateSumDefault();
            });
            $("#<%=rdbDeposit2.ClientID %>").click(function() {
                CaculateSumDefault();
            });
            $("#<%=rdbAeposit1.ClientID %>").click(function() {
                CaculateSumDefault();
            });
            $("#<%=rdbAeposit2.ClientID %>").click(function() {
                CaculateSumDefault();
            });
            $("#<%=rdbIsD1.ClientID %>").click(function() {
                CaculateSumDefault();
            });
            $("#<%=rdbIsD2.ClientID %>").click(function() {
                CaculateSumDefault();
            });
            CaculateSumDefault();

            $("#<%=txtSuspensionMonth.ClientID%>").click(function () {
                CDate1();
            });
            $("#<%=txtSEMonth.ClientID%>").click(function () {
                CDate2();
            });
            ...
            $("#<%=txtSuspensionBeginDate.ClientID%>").datepicker();
            $("#<%=txtSuspensionEndDate.ClientID%>").datepicker();
            $("#<%=txtExpireEndDate.ClientID%>").datepicker();

	        //var text = document.getElementById("txtIDx14");
            //autoTextarea(document.getElementById("txtIDx14")); //使对话框自适应
            $.each($("textarea"), function (idx, item) { autoTextarea(this); }); //使对话框自适应
        });

        function CDate1(){
            var z;
            var yb = new Date($("#<%=txtSuspensionBeginDate.ClientID%>").val().replace(/\-/g, "\/")).getFullYear();
            var ya = new Date($("#<%=txtSuspensionEndDate.ClientID%>").val().replace(/\-/g, "\/")).getFullYear();
            var mb = new Date($("#<%=txtSuspensionBeginDate.ClientID%>").val().replace(/\-/g, "\/")).getMonth();
            var ma = new Date($("#<%=txtSuspensionEndDate.ClientID%>").val().replace(/\-/g, "\/")).getMonth();
            z = (ya - yb)*12 + ma - mb
            $("#<%=txtSuspensionMonth.ClientID %>").val(z);
        }
        function CDate2(){//*-
            var z;
            var yb = new Date($("#<%=txtStartDate.ClientID%>").val().replace(/\-/g, "\/")).getFullYear();
            var ya = new Date($("#<%=txtEndDate.ClientID%>").val().replace(/\-/g, "\/")).getFullYear();
            var mb = new Date($("#<%=txtStartDate.ClientID%>").val().replace(/\-/g, "\/")).getMonth();
            var ma = new Date($("#<%=txtEndDate.ClientID%>").val().replace(/\-/g, "\/")).getMonth();
            z = (ya - yb)*12 + ma - mb
            $("#<%=txtSEMonth.ClientID %>").val(z);
        }

        function CaculateSumDefault(){
            var a = 0;
            if($("#<%=rdbDeposit1.ClientID%>").prop("checked")){
                a += $("#<%=txtMoneyD1.ClientID%>").val()*1;
            }
            if($("#<%=rdbAeposit1.ClientID%>").prop("checked")){
                a += $("#<%=txtMoneyD2.ClientID%>").val()*1;
            }
            if($("#<%=rdbIsD1.ClientID%>").prop("checked")){
                a += $("#<%=txtMoneyD3.ClientID%>").val()*1;
            }
            $("#SumDefault").html(a.toFixed(2));
        }
	    ...
	    function check() {
	        ...
	        if ($.trim($("#<%=txtStartDate.ClientID %>").val()) == "") { alert("分行暂停营业开始日期不可为空！"); $("#<%=txtStartDate.ClientID %>").focus(); return false; }
	        if ($.trim($("#<%=txtEndDate.ClientID %>").val()) == "") { alert("分行暂停营业结束日期不可为空！"); $("#<%=txtEndDate.ClientID %>").focus(); return false; }
	        if ($.trim($("#<%=txtReason.ClientID %>").val()) == "") { alert("暂停营业原因不可为空！"); $("#<%=txtReason.ClientID %>").focus(); return false; }

	        if (!$("#<%=rdbLeaseState1.ClientID %>").prop("checked") && !$("#<%=rdbLeaseState2.ClientID %>").prop("checked")) {
	            alert("请选择分行租赁状况");
	            return false;
	        }
	        ...
	        if (!$("#<%=rdbInsureHandle1.ClientID %>").prop("checked") && !$("#<%=rdbInsureHandle2.ClientID %>").prop("checked")) {
	            alert("请选择保险处理方式");
	            return false;
	        }
            ...
            if (!$("#<%=rdbRayHandle1.ClientID %>").prop("checked") && !$("#<%=rdbRayHandle2.ClientID %>").prop("checked")) {
		        alert("请选择光纤处理方式");
		        return false;
            }

	        if (!$("#<%=rdbADHandle1.ClientID %>").prop("checked") && !$("#<%=rdbADHandle2.ClientID %>").prop("checked")) {
	            alert("请选择AD处理方式");
	            return false;
	        }

	        if (!$("#<%=rdbStationeryHandle1.ClientID %>").prop("checked") && !$("#<%=rdbStationeryHandle2.ClientID %>").prop("checked")) {
	            alert("请选择文具、印刷品、复印机、饮用水等配置处理方式");
	            return false;
	        }
	        ...
	        //if ($("#<%=txtRent.ClientID %>").val() == "") {
	        //    alert("目前月租金（含税）不能为空。");
	        //    $("#<%=txtRent.ClientID %>").focus();
	        //    return false;
	        //}
	        //if ($("#<%=txtSuspensionBeginDate.ClientID %>").val() == "") {
	        //    alert("累计暂停营业开始时间不能为空。");
	        //    $("#<%=txtSuspensionBeginDate.ClientID %>").focus();
	        //    return false;
	        //}
	        //if ($("#<%=txtSuspensionEndDate.ClientID %>").val() == "") {
	        //    alert("累计暂停营业结束时间不能为空。");
	        //    $("#<%=txtSuspensionEndDate.ClientID %>").focus();
	        //    return false;
	        //}

	        //if ($("#<%=txtSuspensionMonth.ClientID %>").val() == "") {
	        //    alert("暂停营业时长不能为空。");
	        //    $("#<%=txtSuspensionMonth.ClientID %>").focus();
	        //    return false;
	        //}
	        //if ($("#<%=txtSEMonth.ClientID %>").val() == "") {
	        //    alert("申请暂停时长不能为空。");
	        //    $("#<%=txtSEMonth.ClientID %>").focus();
	        //    return false;
            //}

	        //if (!$("#<%=rdbDefault1.ClientID %>").prop("checked") && !$("#<%=rdbDefault2.ClientID %>").prop("checked")) {
	        //    alert("请选择是否会产生违约金。");
	        //    return false;
	        //}
	        if ($("#<%=rdbDefault1.ClientID %>").prop("checked")) {
	            if (!$("#<%=rdbDeposit1.ClientID %>").prop("checked") && !$("#<%=rdbDeposit2.ClientID %>").prop("checked")) {
	                alert("请选择是否能回收按金。");
	                $("#<%=rdbDeposit1.ClientID %>").focus();
	                return false;
	            }
	            if ($("#<%=rdbDeposit1.ClientID %>").prop("checked")){
	                if ($("#<%=txtMoneyD1.ClientID %>").val() == "") {
	                    alert("回收按金金额不能为空。");
	                    $("#<%=txtMoneyD1.ClientID %>").focus();
	                    return false;
	                }
	            }
	            if (!$("#<%=rdbAeposit1.ClientID %>").prop("checked") && !$("#<%=rdbAeposit2.ClientID %>").prop("checked")) {
	                alert("请选择是否能回收其他押金。");
	                $("#<%=rdbAeposit1.ClientID %>").focus();
	                return false;
	            }
	            if ($("#<%=rdbAeposit1.ClientID %>").prop("checked")){
	                if ($("#<%=txtMoneyD2.ClientID %>").val() == "") {
	                    alert("回收其他押金金额不能为空。");
	                    $("#<%=txtMoneyD2.ClientID %>").focus();
	                    return false;
	                }
	            }
	            if (!$("#<%=rdbIsD1.ClientID %>").prop("checked") && !$("#<%=rdbIsD2.ClientID %>").prop("checked")) {
	                alert("请选择是否会产生违约金。");
	                $("#<%=rdbIsD1.ClientID %>").focus();
	                return false;
                }
                if ($("#<%=rdbIsD1.ClientID %>").prop("checked")){
	                if ($("#<%=txtMoneyD3.ClientID %>").val() == "") {
	                    alert("违约金额不能为空。");
	                    $("#<%=txtMoneyD3.ClientID %>").focus();
	                    return false;
                    }
        ...
        function upload() {
		    ...
		    if(sReturnValue=='success')
		        window.location='Apply_SuspendBusi_Detail.aspx?MainID=<%=MainID %>';
        }

        function editflow(){
            var win=window.showModalDialog("Apply_SuspendBusi_Flow.aspx?MainID=<%=MainID %>","","dialogWidth=830px;dialogHeight=280px");
            if(win=='success')
                window.location="Apply_SuspendBusi_Detail.aspx?MainID=<%=MainID %>";
        }
        ...
        function sign(idx) {
            //if(idx!='11'&&idx!='12'&&idx!=14&&idx!='15'&&idx!='130'&&idx!='131'){
		...
		<div style='text-align: center'>
			...
			<table id="tbAround"  width='640px'>
                ...
                <tr>
					<td>本期租约</td>
					<td class="tl PL10">
                        <asp:TextBox ID="txtExpireDate" runat="server" Width="80px"></asp:TextBox>~
                        <asp:TextBox ID="txtExpireEndDate" runat="server" Width="80px"></asp:TextBox>
					</td>
                    <td>此次申请暂停<br />营业起止日期</td>
					<td class="tl PL10">
                        <asp:TextBox ID="txtStartDate" runat="server" Width="80px"></asp:TextBox>~
                        <asp:TextBox ID="txtEndDate" runat="server" Width="80px"></asp:TextBox><br />
                        <span style="display: none">共<asp:TextBox ID="txtSEMonth" runat="server" Width="65px"></asp:TextBox>个月</span>
					</td>
                ...
                <tr style="display: none">
                    <td>累计暂停营业时间</td>
                    <td class="tl PL10">
                        <asp:TextBox ID="txtSuspensionBeginDate" runat="server" Width="80px"></asp:TextBox>~
                        <asp:TextBox ID="txtSuspensionEndDate" runat="server" Width="80px"></asp:TextBox><br />
                        共<asp:TextBox ID="txtSuspensionMonth" runat="server" Width="65px"></asp:TextBox>个月
					</td>
                    <td>目前月租金（含税）</td>
                    <td class="tl PL10"><asp:TextBox ID="txtRent" runat="server"></asp:TextBox></td>
                </tr>
                <tr style="display: none">
                    <td style="display: none">违约成本</td>
                    <td class="tl PL10" colspan="3">
                        是否会产生违约金：
                        <asp:RadioButton ID="rdbDefault1" runat="server" GroupName="Default" Text="是" />
                        <asp:RadioButton ID="rdbDefault2" runat="server" GroupName="Default" Text="否" />
                        <div id="ShowRent" style="display:none;">
                            （1）是否能回收按金
                            　　<asp:RadioButton ID="rdbDeposit1" runat="server" GroupName="Deposit" Text="是" />
                            <asp:RadioButton ID="rdbDeposit2" runat="server" GroupName="Deposit" Text="否" />　
                            具体金额<asp:TextBox ID="txtMoneyD1" runat="server"></asp:TextBox>元<br />
                            （2）是否能回收其他押金
                            <asp:RadioButton ID="rdbAeposit1" runat="server" GroupName="Aeposit" Text="是" />
                            <asp:RadioButton ID="rdbAeposit2" runat="server" GroupName="Aeposit" Text="否" />　
                            具体金额<asp:TextBox ID="txtMoneyD2" runat="server"></asp:TextBox>元<br />
                            （3）是否会产生违约金　
                            <asp:RadioButton ID="rdbIsD1" runat="server" GroupName="IsD" Text="是" />
                            <asp:RadioButton ID="rdbIsD2" runat="server" GroupName="IsD" Text="否" />　
                            具体金额<asp:TextBox ID="txtMoneyD3" runat="server"></asp:TextBox>元<br />
                            共<span id="SumDefault"></span>元。
                        </div>
                    </td>
                </tr>
                ...
                <tr>
                    <td>暂停营业原因</td>
					<td class="tl PL10" colspan="3">
						<asp:textbox id="txtReason" runat="server" TextMode="MultiLine" Width="98%" Rows="10" style="overflow: auto;" Height="162px"></asp:textbox>
					</td>
				</tr>
                <tr class="noborder">
                ...
                <tr class="noborder">
					<td class="tl PL10" colspan="4">
						<asp:RadioButton ID="rdbLeaseState1" runat="server" Text="转租" GroupName="LeaseState"/>&nbsp;&nbsp;<asp:RadioButton ID="rdbLeaseState2" runat="server" Text="保持原状" GroupName="LeaseState" />
					</td>
				</tr>
                <tr class="noborder">
					<td class="tl PL10" colspan="4">
                ...
                <tr class="noborder">
					<td class="tl PL10" colspan="4">
						<asp:RadioButton ID="rdbInsureHandle1" runat="server" Text="继续购买" GroupName="InsureHandle"/>&nbsp;&nbsp;<asp:RadioButton ID="rdbInsureHandle2" runat="server" Text="停止保险" GroupName="InsureHandle" />
					</td>
				</tr>
                <tr class="noborder">
					<td class="tl PL10" colspan="4">
                ...
                <tr class="noborder">
					<td class="tl PL10" colspan="4">
						电话&nbsp;&nbsp;<asp:RadioButton ID="rdbPhoneHandle1" runat="server" Text="全部撤销" GroupName="PhoneHandle"/>&nbsp;&nbsp;<asp:RadioButton ID="rdbPhoneHandle2" runat="server" Text="停机留号（直线5元 / 月 / 条，中继线15元 / 月 / 条）" GroupName="PhoneHandle" />
					</td>
				</tr>
                <tr class="noborder">
					<td class="tl PL10" colspan="4">
                ...
                <tr class="noborder">
					<td class="tl PL10" colspan="4">
						光纤&nbsp;&nbsp;<asp:RadioButton ID="rdbRayHandle1" runat="server" Text="暂停服务" GroupName="RayHandle"/>&nbsp;&nbsp;<asp:RadioButton ID="rdbRayHandle2" runat="server" Text="撤销" GroupName="RayHandle" />
					</td>
				</tr>
                <tr class="noborder">
					<td class="tl PL10" colspan="4">
						AD&nbsp;&nbsp;&nbsp;&nbsp;<asp:RadioButton ID="rdbADHandle1" runat="server" Text="暂停服务" GroupName="ADHandle"/>&nbsp;&nbsp;<asp:RadioButton ID="rdbADHandle2" runat="server" Text="撤销" GroupName="ADHandle" />
					</td>
				</tr>       
                <tr class="noborder">
					<td class="tl PL10" colspan="4">
						5) 文具、印刷品、复印机、饮用水等配置&nbsp;&nbsp;<asp:RadioButton ID="rdbStationeryHandle1" runat="server" Text="暂停服务" GroupName="StationeryHandle"/>&nbsp;&nbsp;<asp:RadioButton ID="rdbStationeryHandle2" runat="server" Text="撤销" GroupName="StationeryHandle" />
					</td>
				</tr>
                <tr class="noborder">
					<td class="tl PL10" colspan="4">
						6) 分行截至申请月的工程款摊销余额为<asp:TextBox ID="txtPaymentAmortizeRemaining" runat="server" Width="80"></asp:TextBox>元，工程款摊销到期日为<asp:TextBox ID="txtPaymentAmortizeEndDate" runat="server" Width="80"></asp:TextBox>
                ...
                <tr id="trManager1" class="noborder" style="height: 85px; border-top:1px solid #000000;">
					...
					<td colspan="3" class="tl PL10">
						...
						<textarea id="txtIDx1" style="width: 98%; overflow: auto; height: 86px;"></textarea><input type="button" id="btnSignIDx1" value="签名" onclick="sign('1')" style="display: none;" /><div style="text-align: right; padding-right: 30px; padding-bottom: 10px;">日期：<span id="spanDateIDx1">_________</span></div>
					</td>
				</tr>
				<tr id="trManager2" class="noborder" style="height: 85px;">
					...
					<td colspan="3" class="tl PL10">
						...
						<textarea id="txtIDx2" style="width: 98%; overflow: auto; height: 86px;"></textarea>
                        <input type="button" id="btnSignIDx2" value="签名" onclick="sign('2')" style="display: none;" />
                        <%--<div style="text-align: right; padding-right: 30px; padding-bottom: 10px;">日期：--%>
				...
				<tr id="trManager3" class="noborder" style="height: 85px;">
					...
					<td colspan="3" class="tl PL10">
						...
						<textarea id="txtIDx3" style="width: 98%; overflow: auto; height: 85px;"></textarea><input type="button" id="btnSignIDx3" value="签名" onclick="sign('3')" style="display: none;" /><div style="text-align: right; padding-right: 30px; padding-bottom: 10px;">日期：<span id="spanDateIDx3">_________</span></div>
					</td>
				</tr>
				...
				<tr id="trShowFlow5" class="noborder" style="height: 85px;">
					<td colspan="3" class="tl PL10" style=" ">
						...
						<textarea id="txtIDx5" style="width: 98%; overflow: auto; height: 86px;"></textarea><input type="button" id="btnSignIDx5" value="签署意见" onclick="sign('5')" style="display: none;" /><div class="signdate">日期：<span id="spanDateIDx5">_________</span></div>
					</td>
				</tr>
                ...
                <tr id="trShowFlow8" class="noborder" style="height: 85px;"> 
					...
					<td colspan="3" class="tl PL10" style=" ">
						...
						<textarea id="txtIDx8" style="width: 98%; overflow: auto; height: 86px;"></textarea><input type="button" id="btnSignIDx8" value="签署意见" onclick="sign('8')" style="display: none;" /><div class="signdate">日期：<span id="spanDateIDx8">_________</span></div>
					</td>
				</tr>
                ...
                <tr id="trShowFlow12" class="noborder" style="height: 85px;">
					<td colspan="3" class="tl PL10" style=" ">
						...
						<textarea id="txtIDx12" style="width: 98%; overflow: auto; height: 86px;"></textarea><input type="button" id="btnSignIDx12" value="签署意见" onclick="sign('12')" style="display: none;" /><div class="signdate">日期：<span id="spanDateIDx12">_________</span></div>
					</td>
				</tr>
                ...
                <tr id="trShowFlow13" class="noborder" style="height: 85px;"> 
					...
					<td colspan="3" class="tl PL10" style=" ">
						...
						<textarea id="txtIDx13" style="width: 98%; overflow: auto; height: 86px;"></textarea><input type="button" id="btnSignIDx13" value="签署意见" onclick="sign('13')" style="display: none;" /><div class="signdate">日期：<span id="spanDateIDx13">_________</span></div>
					</td>
				</tr>
				<tr id="trLogistics2" class="noborder" style="height: 85px;">
					<td colspan="3" class="tl PL10" style=" ">
						<input id="rdbYesIDx14" type="radio" name="agree14" /><label for="rdbYesIDx14">同意</label><input id="rdbNoIDx14" type="radio" name="agree14" /><label for="rdbNoIDx14">不同意</label><input id="rdbOtherIDx14" type="radio" name="agree14" value="1" /><label for="rdbOtherIDx14">其他意见</label>
                      ...
                      　<a id="dfd" href="javascript:void(0)" style="display: none;" onclick="deleteFlow()">删除添加的审批环节</a><br />
						<textarea id="txtIDx14" style="width: 98%; overflow: auto; height: 87px;"></textarea><input type="button" id="btnSignIDx14" value="签署意见" onclick="sign('14')" style="display: none;" /><div class="signdate">日期：<span id="spanDateIDx14">_________</span></div>
					</td>
				</tr>
				...
				<tr id="trGeneralManager" class="noborder" style="height: 85px;">
					...
					<td colspan="3" class="tl PL10" style=" ">
						...
						<textarea id="txtIDx15" style="width: 98%; overflow: auto; height: 86px;"></textarea><input type="button" id="btnSignIDx15" value="签署意见" onclick="sign('15')" style="display: none;" /><div class="signdate">日期：<span id="spanDateIDx15">_________</span></div>
					</td>
				</tr>
