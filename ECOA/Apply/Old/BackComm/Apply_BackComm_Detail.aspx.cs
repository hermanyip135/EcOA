...
public partial class Apply_BackComm_Apply_BackComm_Detail : BasePage
{
    ...
    public void LoadPage()
    {
        ...
        //if ((EmployeeID == CommonConst.EMP_FINANCE_PUBLIC_ID || EmployeeID == "16945" || EmployeeID == "20208") && flowState == "3")
        //2019-10-21 财务部将 胡雅丝（16945） 权限移交比 钟惠贤（43781）
        if ((EmployeeID == CommonConst.EMP_FINANCE_PUBLIC_ID || EmployeeID == "43781" || EmployeeID == "20208") && flowState == "3")
                    ...
                    if (hdIsAgree.Value != "0")//同意或其他意见
                    {
                        ...
                        if (da_OfficeAutomation_Flow_Inherit.IsFlowComplete(MainID))
                        {
                            ...
                            if (employeeList.Contains("黄轩明"))
                                employname = CommonConst.EMP_GMO_NAME + ",李小清,潘焕心,梁晶晶";
                            else
                                employname = "李小清,潘焕心,梁晶晶";
                            employnames = employname.Split(',');
                            for (int i = 0; i < employnames.Length; i++)
                            {
                                if (!employeeList.Contains(employnames[i]))
    ...
    protected void btnSignSave_Click(object sender, EventArgs e)
    {
        ...
        {
            DA_OfficeAutomation_Document_BackComm_Inherit da_OfficeAutomation_Document_BackComm_Inherit = new DA_OfficeAutomation_Document_BackComm_Inherit();
            //if (EmployeeID == CommonConst.EMP_FINANCE_PUBLIC_ID || EmployeeID == "16945" || EmployeeID == "20208")
            //2019-10-21 财务部将 胡雅丝（16945） 权限移交比 钟惠贤（43781）
            if (EmployeeID == CommonConst.EMP_FINANCE_PUBLIC_ID || EmployeeID == "43781" || EmployeeID == "20208")
                da_OfficeAutomation_Document_BackComm_Inherit.AddRemarkByID_II(ID, CommonConst.SIGN_FINANCE);
            Alert("标记成功。");
        }
        catch
        {
