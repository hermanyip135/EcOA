<%@ Page Title="申请内容修改申请表 - 广州中原电子审批系统" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="Apply_SysLogist_Detail.aspx.cs" Inherits="Apply_SysLogist_Apply_SysLogist_Detail" %>
...
//            $("#li3").attr("class", "current");
            ...
            $("#<%=txtHeaderIDx3.ClientID %>").autocomplete({ 
                ...
                source: function( request, response ) {
                    ...
                    response( $.ui.autocomplete.filter(jManagerJSON, extractLast( request.term ) ) );
                },
                focus: function() {
            ...
            if(confirm("是否确认审核？"))
            {
                ...
                if(idx=='4' && $.trim(follower)!=''){
                    $.ajax({
                        ...
                        type: "post",
                        dataType: "text",
                        data: "action=savesyslogistfollower&sysreqid=<%=ID %>&follower=" + follower+"&apply=" + $.trim($("#<%=txtApplicant.ClientID %>").val()),
                        success: function() {}
                    })
                }
        ...
        function editflow(){
            ...
            if(win=='success')
                window.location="Apply_SysLogist_Detail.aspx?MainID=<%=MainID %>";
        }
          ...
          var plantime=$('#<%=txtPlanTime.ClientID %>').val();
            ...
            { 
                $.ajax({
                    ...
                    type: "post",
                    dataType: "text",
                    data: "action=savesyslogistplantime&sysreqid=<%=ID %>&plantime=" + plantime + "&follower=<%=EmployeeName %>&apply=" + $.trim($("#<%=txtApplicant.ClientID %>").val()),
            ...
            <table id="tbAround"  width='640px'>
                ...
                <tr id="trGeneralManager" class="noborder" style="height: 85px; display: none;">
					...
					<td colspan="3" class="tl PL10" style=" ">
						<input id="rdbYesIDx16" type="radio" name="agree16" /><label for="rdbYesIDx16">同意</label><input id="rdbNoIDx16" type="radio" name="agree16" /><label for="rdbNoIDx16">不同意</label>
                        <input id="rdbOtherIDx16" type="radio" name="agree16" /><label for="rdbOtherIDx16">其他意见</label><br />
						<textarea id="txtIDx16" rows="6" style="width: 98%; overflow: auto;"></textarea><input type="button" id="btnSignIDx16" value="签署意见" onclick="sign('16')" style="display: none;" />
                        <br />
