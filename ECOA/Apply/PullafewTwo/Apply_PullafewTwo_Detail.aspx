<%@ Page ValidateRequest="false" Title="二级市场一手项目欠必要性文件拉数申请 - 广州中原电子审批系统" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="Apply_PullafewTwo_Detail.aspx.cs" Inherits="Apply_PullafewTwo_Apply_PullafewTwo_Detail" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function CaculateMThree(){
            ...
            var txt1jsum = 0;
            for(var x = 1;x <= i; x ++){
                //txt1gsum += $("#txtDetail_1d"+ x).val()*1;
                //txt1hsum += $("#txtDetail_1e"+ x).val()*1;
                //txt1isum += $("#txtDetail_1f"+ x).val()*1;
                //txt1iisum += $("#txtDetail_1ff"+ x).val()*1;
                //txt1jsum += $("#txtDetail_1g"+ x).val()*1;

            }
            $("#<%=txt1g.ClientID %>").val(txt1gsum);
            $("#<%=txt1h.ClientID %>").val(txt1hsum);
            $("#<%=txt1i.ClientID %>").val(txt1isum);
            $("#<%=txt1ii.ClientID %>").val(txt1iisum);
            $("#<%=txt1j.ClientID %>").val(txt1jsum);
	    ...
	    function check() {
	        ...
	        if($.trim($("#<%=txt1c.ClientID %>").val())=="") {
	            alert("成交总业绩（年）不可为空！");
	            $("#<%=txt1c.ClientID %>").focus();
	            return false;
	        }

	        if($.trim($("#<%=txt1d.ClientID %>").val())=="") {
	            alert("成交总业绩（月）不可为空！");
	            $("#<%=txt1d.ClientID %>").focus();
	            return false;
	        }

	        if($.trim($("#<%=txt1e.ClientID %>").val())=="") {
	            alert("年拉数不可为空！");
	            $("#<%=txt1e.ClientID %>").focus();
	            return false;
	        }

	        if($.trim($("#<%=txt1f.ClientID %>").val())=="") {
	            alert("月拉数不可为空！");
	            $("#<%=txt1f.ClientID %>").focus();
	            return false;
	        }
	        ...
	        if($.trim($("#<%=txt1ii.ClientID %>").val())=="") {
	            alert("三级市场欠收业绩不可为空！");
	            $("#<%=txt1ii.ClientID %>").focus();
	            return false;
            }
	        ...
	        $("#tbDetail tr").each(function(n) {
	            if (n != 0 && n < $("#tbDetail tr").size() - 2) {
	                if ($.trim($("#txtDetail_1a" + n).val()) == "") {
	                    ...
	                    alert("第" + n + "行的二级市场欠收业绩必须填写。");
	                    $("#txtDetail_1f" + n).focus();
	                    flag = false;
	                    return;
	                }
	                else if ($.trim($("#txtDetail_1ff" + n).val()) == "") {
	                    alert("第" + n + "行的三级市场欠收业绩必须填写。");
	                    $("#txtDetail_1ff" + n).focus();
	                    flag = false;
	                    return;
	                }
	                else if ($.trim($("#txtDetail_1g" + n).val()) == "") {
	                    alert("第" + n + "行的欠收业绩合计必须填写。");
	                    $("#txtDetail_1g" + n).focus();
	                    flag = false;
	                    return;
	                }
	                else if ($.trim($("#txtDetail_1h" + n).val()) == "") {
	                    alert("第" + n + "行的欠交客户确认函原因必须填写。");
	        ...
	        $("#tbDetail3 tr").each(function(n) {
	            if (n != 0 && n < $("#tbDetail3 tr").size() - 2) {
	                ...
	                else if ($.trim($("#txtLeaseTerm_1g" + n).val()) == "") {
	                    alert("第" + n + "行的合计必须填写。");
	                    $("#txtLeaseTerm_1g" + n).focus();
	                    flag = false;
	                    return;
	                }
		...
		function upload() {
			...
			if(sReturnValue=='success')
				window.location='Apply_PullafewTwo_Detail.aspx?MainID=<%=MainID %>';
		}

		function editflow(){
			var win=window.showModalDialog("Apply_PullafewTwo_Flow.aspx?MainID=<%=MainID %>","","dialogWidth=800px;dialogHeight=320px");
					if(win=='success')
						window.location="Apply_PullafewTwo_Detail.aspx?MainID=<%=MainID %>";
		}
		...
		function addRow() {
		    ...
		    var html = '<tr id="trDetail' + i + '">'
                ...
                + '         <td><textarea id="txtDetail_1ff' + i + '" style="width: 80px; overflow: hidden;" rows="1"></textarea></td>'
		...
		function addRow3() {
		    ...
		    var html = '<tr id="trDetail3' + i3 + '">'
				...
				+ '     </tr>';
		    //var html = '<tr id="trDetail3' + i + '"><table><tr><td>这是'+ i +'个</td></tr></table></tr>'
		    $("#trFlag3").before(html);
		    $.each($("textarea"), function (idx, item) { autoTextarea(this); });
		    //$("#trFlag").before("<tr><td>这是"+ i +"个</td></tr>");
		    //$("#txtDetail_Department"+ i).autocomplete({source: jJSON});
			...
			<table id="tbAround"  width='840px'>
				...
				<tr>
					<td colspan="4" class="tl PL10" style="line-height: 25px">
                        ...
                        <table id="tbDetail" width='98%' class='sample tc' align="center">
                            <thead>
                                <tr>
                            ...
                            <tr id="trFlag">
                                ...
                                <td><asp:TextBox ID="txt1ii" runat="server" Width="80px" TextMode="MultiLine" Rows="1"></asp:TextBox></td>
                 ...
                 <tr>
					<td colspan="4" class="tl PL10" style="line-height: 25px">
                        ...
                        <table id="tbDetail3" width='98%' class='sample tc' align="center">
                            <thead>
                                <tr>
                                    <td rowspan="2">项目负责人</td>
                                    <td rowspan="2">楼盘名称</td>
                                    ...
                                    <td rowspan="2">
                                        ...
                                        <asp:TextBox ID="txt2b" runat="server" Width="30px"></asp:TextBox>月<br />成交总业绩
                                    </td>
                                    <td colspan="3">
                                        <asp:TextBox ID="txt2c" runat="server" Width="30px"></asp:TextBox>年
                                        <asp:TextBox ID="txt2cc" runat="server" Width="30px"></asp:TextBox>月拉数
                                    </td>
                                    <td rowspan="2">欠交文件</td>
                                    <td rowspan="2">欠交原因</td>
                                </tr>
                                <tr>
                                    <td>二级市场欠收业绩</td>
                                    <td>三级市场欠收业绩</td>
                            ...
                            <tr id="trFlag3">
                                <td colspan="3">合计</td>
                                <td><asp:TextBox ID="txt2d" runat="server" Width="80px" TextMode="MultiLine" Rows="1"></asp:TextBox></td>
                                <td><asp:TextBox ID="txt2f" runat="server" Width="80px" TextMode="MultiLine" Rows="1"></asp:TextBox></td>
                                <td><asp:TextBox ID="txt2g" runat="server" Width="80px" TextMode="MultiLine" Rows="1"></asp:TextBox></td>
                                <td><asp:TextBox ID="txt2h" runat="server" Width="80px" TextMode="MultiLine" Rows="1"></asp:TextBox></td>
                                <td></td>
                            </tr>
                        </table>
				...
				<tr>
					<td class="tl PL10" colspan="4" style="line-height: 25px">
                        <span style="font-weight: bold">项目部联动项目三级市场联动成交各区域实际拉数业绩见下表：</span><br />
                ...
                <tr>
                    <td class="tl PL10" colspan="4" style="line-height: 25px">
                        <table id="tbDetail4" class='sample tc' width='95%' align="center">
                            <tr>
                                <td colspan="2">
                                    截至<asp:TextBox ID="txt4a" runat="server" Width="30px"></asp:TextBox>年<asp:TextBox ID="txt4b" runat="server" Width="30px"></asp:TextBox>月二级市场自接项目拉数及坏账冲回情况统计
                                </td>
                            </tr>
                            <tr>
                                <td>对应成交月份</td>
...
<%--                <tr id="trLogistics1" class="noborder" style="height: 85px;display: none;">
    ...
    <asp:Button ID="btnWillEnd" runat="server" Text="结束" OnClick="btnWillEnd_Click" Visible="False" /></td>
				...
				<tr id="trLogistics1" class="noborder" style="height: 85px;">
					<td  class="auto-style3" >后勤事务部<br />审批</td>
					<td colspan="3" class="tl PL10" style=" ">
...
<%--                <tr id="tlsc2" class="noborder" style="height: 85px; display: none;">
                ...
                <tr id="trShowFlow201" class="noborder" style="height: 85px;display:none">
					<td class="auto-style3" >后勤事务部</td>
					<td colspan="3" class="tl PL10" style=" ">
						<input id="rdbYesIDx201" type="radio" name="agree201" /><label for="rdbYesIDx201">同意</label><input id="rdbNoIDx201" type="radio" name="agree201" /><label for="rdbNoIDx201">不同意</label><input id="rdbOtherIDx201" type="radio" name="agree201" /><label for="rdbOtherIDx201">其他意见</label>
                        <asp:CheckBox ID="ckbAddIDx201" runat="server" Text="增加审批环节"  Visible="false"/><br />
						<textarea id="txtIDx201" rows="5" style="width: 98%; overflow: auto;"></textarea><input type="button" id="btnSignIDx201" value="签署意见" onclick="sign('201')" style="display: none;" />
                        <br />
                        <span style="padding-right: 30px; padding-bottom: 10px; text-align: right; float: right;">
                            日期：<span id="spanDateIDx201">_________</span>
                        </span>
                    </td>
                </tr>
        ...
        <div id="PageBottom">
		<hr />
        <span style="font-size: large; padding-top: 10px; padding-bottom: 10px"><a href="../../资料/二级市场一手项目欠必要性文件拉数申请_导入模板.xlsx">下载导入模板</a></span>
        <br />
