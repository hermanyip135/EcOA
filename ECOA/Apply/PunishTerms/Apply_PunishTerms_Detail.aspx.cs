...
public partial class Apply_PunishTerms_Apply_PunishTerms_Detail : BasePage
{
    #region 变量声明及定义
    bool tpdf = false;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            try
            {
                if (Session["FLG_ReWrite51"].ToString() == "1")
                {
                    ViewState["FLG_ReWrite"] = "1";
                    Session["FLG_ReWrite51"] = null;
                }
            }
            catch
            {
    ...
    public void LoadPage()
    {
        ...
        DA_OfficeAutomation_Document_PunishTerms_Detail_Inherit da_OfficeAutomation_Document_PunishTerms_Detail_Inherit = new DataAccess.Operate.DA_OfficeAutomation_Document_PunishTerms_Detail_Inherit();
        ...
        ds = da_OfficeAutomation_Document_PunishTerms_Inherit.SelectByMainID(MainID);
        DataRow dr = ds.Tables[0].Rows[0];
        ID = dr["OfficeAutomation_Document_PunishTerms_ID"].ToString();
        string applicant = dr["OfficeAutomation_Document_PunishTerms_Apply"].ToString();
        ApplyN = applicant;
        txtApplyID.Text = dr["OfficeAutomation_Document_PunishTerms_ApplyID"].ToString();
        txtDepartment.Text = dr["OfficeAutomation_Document_PunishTerms_Department"].ToString();
        lblApply.Text = applicant;
        lblApplyDate.Text = DateTime.Parse(dr["OfficeAutomation_Document_PunishTerms_ApplyDate"].ToString()).ToString("yyyy-MM-dd");
        txtContract.Text = dr["OfficeAutomation_Document_PunishTerms_Contract"].ToString();
        txtContractKind.Text = dr["OfficeAutomation_Document_PunishTerms_ContractKind"].ToString();
        txtAgentPeriod.Text = dr["OfficeAutomation_Document_PunishTerms_AgentPeriod"].ToString();

        SerialNumber = dr["OfficeAutomation_SerialNumber"].ToString();

        ds = da_OfficeAutomation_Document_PunishTerms_Detail_Inherit.SelectByID(ID);
        ...
        else
        {
            ...
            for (int n = 0; n < detailCount; n++)
            {
                ...
                SbJs.Append("$('#txtDetail_No" + i + "').val('" + dr["OfficeAutomation_Document_PunishTerms_Detail_No"] + "');");
                SbJs.Append("$('#txtDetail_Point" + i + "').val('" + dr["OfficeAutomation_Document_PunishTerms_Detail_Point"] + "');");
                SbJs.Append("$('#txtDetail_Terms" + i + "').val('" + dr["OfficeAutomation_Document_PunishTerms_Detail_Terms"] + "');");
            }
        }
    ...
    public void DrawDetailTable(int n)
    {
        ...
        for (int i = 1; i <= n; i++)
        {
            ...
            SbHtml.Append("<td class=\"tl PL10\" style=\"line-height: 30px;padding-left: 10px\">");

			SbHtml.Append("第<input type=\"text\" id=\"txtDetail_No" + i + "\" style=\"width:50px\"/>条的 ");
			SbHtml.Append("第<input type=\"text\" id=\"txtDetail_Point" + i + "\" style=\"width:50px\"/>点 ");
            SbHtml.Append("（条款名：<input type=\"text\" id=\"txtDetail_Terms" + i + "\" style=\"width:340px;\"/>）<br />");
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        T_OfficeAutomation_Document_PunishTerms t_OfficeAutomation_Document_PunishTerms = new T_OfficeAutomation_Document_PunishTerms();
        ...
        try
        {
            ...
            if (IsNewApply)
            {
                ...
                t_OfficeAutomation_Main.OfficeAutomation_SerialNumber = "PunishTerms" + DateTime.Now.ToString("yyyyMMddHHmmssfffffff");
                t_OfficeAutomation_Main.OfficeAutomation_DocumentID = 46; //在《申请表字典表》找到该表的主键，这步一错则会打开别的表
                ...
                t_OfficeAutomation_Document_PunishTerms = GetModelFromPage(Guid.NewGuid());
                ...
                t_OfficeAutomation_Main.OfficeAutomation_Main_Summary = txtContract.Text;
                ...
                da_OfficeAutomation_Document_PunishTerms_Inherit.Insert(t_OfficeAutomation_Document_PunishTerms);//插入申请表

                InsertPunishTermsDetail(t_OfficeAutomation_Document_PunishTerms.OfficeAutomation_Document_PunishTerms_ID); //新增详情表
                ...
                Common.AddLog(EmployeeID, EmployeeName, 50, new Guid(MainID), 1);//日志，创建申请表
                Common.AddLog(EmployeeID, EmployeeName, 3, new Guid(MainID), 1);//日志，创建流程

                RunJS("window.showModalDialog(\"/Apply/Apply_UploadFile.aspx?MainID=" + MainID + "\", '" + MainID + "', \"dialogHeight=165px\");var win=window.showModalDialog(\"Apply_PunishTerms_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=830px;dialogHeight=340px\");if(win=='success'){window.location='/Apply/Apply_Search.aspx'; }");
                #endregion
            }
            else
            {
    ...
    protected void btnDownload_Click(object sender, EventArgs e)
    {
        ...
        if (files.Count > 0)
        {
            ...
            Common.AddLog(EmployeeID, EmployeeName, 50, new Guid(MainID), 8);
        }
        else
            Alert("请勾选文件再下载！");
    }
    ...
    protected void btnSign_Click(object sender, EventArgs e)
    {
        ...
        DA_OfficeAutomation_Document_PunishTerms_Inherit da_OfficeAutomation_Document_PunishTerms_Inherit = new DA_OfficeAutomation_Document_PunishTerms_Inherit();
        DA_OfficeAutomation_Document_PunishTerms_Detail_Inherit da_OfficeAutomation_Document_PunishTerms_Detail_Inherit = new DA_OfficeAutomation_Document_PunishTerms_Detail_Inherit();

        DataSet ds = da_OfficeAutomation_Document_PunishTerms_Inherit.SelectByMainID(MainID);
        DataRow drRow = ds.Tables[0].Rows[0];
        ID = drRow["OfficeAutomation_Document_PunishTerms_ID"].ToString(); 
        ...
        if (Purview.Contains("OA_ITPower_002"))
        {
            try
            {
                ...
                if (isSignSuccess)
                {
                    ...
                    string apply = ds.Tables[0].Rows[0]["OfficeAutomation_Document_PunishTerms_Apply"].ToString();
                    string employname;
                    string serialNumber = drRow["OfficeAutomation_SerialNumber"].ToString();
                    string documentName = da_OfficeAutomation_Flow_Inherit.GetDocumentNameByMainID(MainID);
                    string department = drRow["OfficeAutomation_Document_PunishTerms_Department"].ToString();
                    ...
                    sbMailBody.Append("<br/><br/>申请人：" + drRow["OfficeAutomation_Document_PunishTerms_Apply"]);
                    sbMailBody.Append("<br/>发文部门：" + drRow["OfficeAutomation_Document_PunishTerms_Department"]);
                    sbMailBody.Append("<br/>申请日期：" + DateTime.Parse(drRow["OfficeAutomation_Document_PunishTerms_ApplyDate"].ToString()).ToString("yyyy-MM-dd"));
                    sbMailBody.Append("<br/>项目名：" + drRow["OfficeAutomation_Document_PunishTerms_ApplyID"]);
                    sbMailBody.Append("<br/>合同名：" + drRow["OfficeAutomation_Document_PunishTerms_Contract"]);
                    sbMailBody.Append("<br/>合同类别：" + drRow["OfficeAutomation_Document_PunishTerms_ContractKind"]);
                    sbMailBody.Append("<br/>签署代理期：" + drRow["OfficeAutomation_Document_PunishTerms_AgentPeriod"]);
                    sbMailBody.Append("<br/><br/>");

                    sbMailBody.Append("<br/>详细情况：<br/>");

                    ds = da_OfficeAutomation_Document_PunishTerms_Detail_Inherit.SelectByMainID(MainID);
                    foreach (DataRow dr in ds.Tables[0].Rows)
                    {
                        sbMailBody.Append("<br/>第" + dr["OfficeAutomation_Document_PunishTerms_Detail_No"].ToString() + "条的");
                        sbMailBody.Append("第" + dr["OfficeAutomation_Document_PunishTerms_Detail_Point"].ToString() + "点");
                        sbMailBody.Append("(条款名：" + dr["OfficeAutomation_Document_PunishTerms_Detail_Terms"].ToString() + ")");
    ...
    protected void btnSignSave_Click(object sender, EventArgs e)
    {
        ...
        //        da_OfficeAutomation_Document_PunishTerms_Inherit.AddRemarkByID(ID, CommonConst.SIGN_FINANCE);
        //    Alert("标记成功。");
        //}
        //catch
        //{
    ...
    private T_OfficeAutomation_Document_PunishTerms GetModelFromPage(Guid UndertakeProjID)
    {
        T_OfficeAutomation_Document_PunishTerms t_OfficeAutomation_Document_PunishTerms = new T_OfficeAutomation_Document_PunishTerms();
        t_OfficeAutomation_Document_PunishTerms.OfficeAutomation_Document_PunishTerms_ID = UndertakeProjID;
        t_OfficeAutomation_Document_PunishTerms.OfficeAutomation_Document_PunishTerms_MainID = new Guid(MainID);
        t_OfficeAutomation_Document_PunishTerms.OfficeAutomation_Document_PunishTerms_Apply = EmployeeName;
        t_OfficeAutomation_Document_PunishTerms.OfficeAutomation_Document_PunishTerms_ApplyDate = DateTime.Now;
        t_OfficeAutomation_Document_PunishTerms.OfficeAutomation_Document_PunishTerms_ApplyID = txtApplyID.Text;
        t_OfficeAutomation_Document_PunishTerms.OfficeAutomation_Document_PunishTerms_Department = txtDepartment.Text;
        t_OfficeAutomation_Document_PunishTerms.OfficeAutomation_Document_PunishTerms_Contract = txtContract.Text;
        t_OfficeAutomation_Document_PunishTerms.OfficeAutomation_Document_PunishTerms_ContractKind = txtContractKind.Text;
        t_OfficeAutomation_Document_PunishTerms.OfficeAutomation_Document_PunishTerms_AgentPeriod = txtAgentPeriod.Text;

        return t_OfficeAutomation_Document_PunishTerms;
    }
    ...
    private void Update()
    {
        ...
        DataSet ds = new DataSet();
        ds = da_OfficeAutomation_Document_PunishTerms_Inherit.SelectByMainID(MainID);
        ID = ds.Tables[0].Rows[0]["OfficeAutomation_Document_PunishTerms_ID"].ToString();

        t_OfficeAutomation_Document_PunishTerms = GetModelFromPage(new Guid(ID));

        string apply = EmployeeName;
        string depname = txtDepartment.Text;
        string summary = txtContract.Text;
        string applydate = "";
        string mainid = MainID;

        da_OfficeAutomation_Main_Inherit.UpdateMain(mainid, depname, apply, applydate, summary);
        da_OfficeAutomation_Document_PunishTerms_Inherit.Update(t_OfficeAutomation_Document_PunishTerms); //修改申请表

        //先删除后增加详情表
        DA_OfficeAutomation_Document_PunishTerms_Detail_Inherit da_OfficeAutomation_Document_PunishTerms_Detail_Inherit = new DA_OfficeAutomation_Document_PunishTerms_Detail_Inherit();
        da_OfficeAutomation_Document_PunishTerms_Detail_Inherit.Delete(ID);
        InsertPunishTermsDetail(new Guid(ID));

        Common.AddLog(EmployeeID, EmployeeName, 50, new Guid(MainID), 2); //日志，修改申请表
    }
    ...
    /// 新增详情表
    /// </summary>
    /// <param name="gPunishTermsID"></param>
    private void InsertPunishTermsDetail(Guid gPunishTermsID)
    {
        if (hdDetail.Value == "")
            return;

        T_OfficeAutomation_Document_PunishTerms_Detail t_OfficeAutomation_Document_PunishTerms_Detail;
        DA_OfficeAutomation_Document_PunishTerms_Detail_Inherit da_OfficeAutomation_Document_PunishTerms_Detail_Inherit = new DA_OfficeAutomation_Document_PunishTerms_Detail_Inherit();
        ...
        try
        {
            for (int i = 0; i < details.Length; i++)
            {
                ...
                if (detail[0] == "")
                    continue;
                t_OfficeAutomation_Document_PunishTerms_Detail = new T_OfficeAutomation_Document_PunishTerms_Detail();
                t_OfficeAutomation_Document_PunishTerms_Detail.OfficeAutomation_Document_PunishTerms_Detail_ID = Guid.NewGuid();
                t_OfficeAutomation_Document_PunishTerms_Detail.OfficeAutomation_Document_PunishTerms_Detail_MainID = gPunishTermsID;
                t_OfficeAutomation_Document_PunishTerms_Detail.OfficeAutomation_Document_PunishTerms_Detail_No = detail[0];
                t_OfficeAutomation_Document_PunishTerms_Detail.OfficeAutomation_Document_PunishTerms_Detail_Point = detail[1];
                t_OfficeAutomation_Document_PunishTerms_Detail.OfficeAutomation_Document_PunishTerms_Detail_Terms = detail[2];

                da_OfficeAutomation_Document_PunishTerms_Detail_Inherit.Insert(t_OfficeAutomation_Document_PunishTerms_Detail);
            }
        }
        catch(Exception ee)
        {
    ...
    protected void btnReWrite_Click(object sender, EventArgs e)
    {
        ...
        Session["FLG_ReWrite51"] = "1";
        Response.Write("<script>window.open('Apply_PunishTerms_Detail.aspx?MainID=" + MainID + "');</script>");
    }
    #region 自动导入详情表
    /// <summary>
    /// 自动导入详情表
    ...
    protected void btnSPDF_Click(object sender, EventArgs e) //M_PDF
    {
        try
        {
            ...
            Response.AddHeader("content-disposition", "attachment; filename=" + HttpUtility.UrlEncode("项目部代理合同扣罚条款签名确认表.pdf"));//强制下载 
    ...
    protected void btnCancelSign_Click(object sender, EventArgs e) //20141202：CancelSign
    {
        try
        {
            ...
            string signEmployeeName = EmployeeName;
            DA_OfficeAutomation_Document_PunishTerms_Inherit da_OfficeAutomation_Document_PunishTerms_Inherit = new DA_OfficeAutomation_Document_PunishTerms_Inherit();
            DataSet ds = da_OfficeAutomation_Document_PunishTerms_Inherit.SelectByMainID(MainID); //在不同的表中要修改
            DataRow drRow = ds.Tables[0].Rows[0];
            string apply = ds.Tables[0].Rows[0]["OfficeAutomation_Document_PunishTerms_Apply"].ToString();
            string employname;
            string serialNumber = drRow["OfficeAutomation_SerialNumber"].ToString();
    ...
    protected void btnEditFlow_DoClick(object sender, EventArgs e)
    {
        try
        {
            ...
            //在不同的表中要修改
            RunJS("var win=window.showModalDialog(\"Apply_PunishTerms_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=830px;dialogHeight=340px\");if(win=='success'){window.location='/Ajax/Editor_Flow.ashx?MainID=" + MainID + "&applyUrl=" + applyUrl + "&apply=" + apply + "&serialNumber=" + serialNumber + "&department=" + department + "&Jhref=" + Page.Request.Url + "&mid=" + EmployeeID + "'; }");
