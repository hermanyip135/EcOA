...
public partial class Apply_BusinessTravel_Apply_BusinessTravel_Detail : BasePage
{
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            if (IsNewApply)
            {
                ...
                RunJS("var win=window.showModalDialog(\"/Apply/Apply_UploadFile.aspx?MainID=" + MainID + "\", '" + MainID + "', \"dialogHeight=165px\");if(win=='success'){alert('保存成功！');window.location='/Apply/Apply_Search.aspx'; }else{alert('保存成功！');window.location='/Apply/Apply_Search.aspx';}");
                #endregion
            }
            else
            {
                ...
                if (Purview.Contains("OA_ITPower_001"))//是否有编辑权限
                {
                    ...
                    if (tempsave)
                    {
                        ...
                        RunJS("var win=window.showModalDialog(\"/Apply/Apply_UploadFile.aspx?MainID=" + MainID + "\", '" + MainID + "', \"dialogHeight=165px\");;if(win=='success'){window.location='/Apply/Apply_Search.aspx'; }");
                    }
                    //20000元＜总费用≤100000元审批至董事总经理
                    var val = decimal.Parse(this.hiTotal.Value);
                    if (20000 < val && val <= 100000)
    ...
    private T_OfficeAutomation_Document_BusinessTravel GetModelFromPage(Guid BusinessTravelID)
    {
        ...
        model.OfficeAutomation_Document_BusinessTravel_Total = hiTotal.Value;
        return model;
    }
