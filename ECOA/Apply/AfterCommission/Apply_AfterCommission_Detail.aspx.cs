...
public partial class Apply_AfterCommission_Apply_AfterCommission_Detail : BasePage
{
    #region 变量声明及定义
    bool tpdf = false;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            try
            {
                if (Session["FLG_ReWrite42"].ToString() == "1")
                {
                    ViewState["FLG_ReWrite"] = "1";
                    Session["FLG_ReWrite42"] = null;
                }
            }
            catch
            {
    ...
    public void LoadPage()
    {
        ...
        ds = da_OfficeAutomation_Document_AfterCommission_Inherit.SelectByMainID(MainID);
        DataRow dr = ds.Tables[0].Rows[0];
        ID = dr["OfficeAutomation_Document_AfterCommission_ID"].ToString();
        string applicant = dr["OfficeAutomation_Document_AfterCommission_Apply"].ToString();
        ApplyN = applicant;
        txtApplyID.Text = dr["OfficeAutomation_Document_AfterCommission_ApplyID"].ToString();
        if(GetQueryString("Dsp") != "")
            txtDepartment.Text = GetQueryString("Dsp");
        else
            txtDepartment.Text = dr["OfficeAutomation_Document_AfterCommission_Department"].ToString();
        lblApply.Text = applicant;
        lblApplyDate.Text = DateTime.Parse(dr["OfficeAutomation_Document_AfterCommission_ApplyDate"].ToString()).ToString("yyyy-MM-dd");
        txtReplyPhone.Text = dr["OfficeAutomation_Document_AfterCommission_ReplyPhone"].ToString();
        txtSubject.Text = dr["OfficeAutomation_Document_AfterCommission_Subject"].ToString();
        txtName.Text = dr["OfficeAutomation_Document_AfterCommission_Name"].ToString();
        txtHasCommission.Text = dr["OfficeAutomation_Document_AfterCommission_HasCommission"].ToString();
        txtCommissionMoney.Text = dr["OfficeAutomation_Document_AfterCommission_CommissionMoney"].ToString();
        txtContact.Text = dr["OfficeAutomation_Document_AfterCommission_Contact"].ToString();
        txtAddress.Text = dr["OfficeAutomation_Document_AfterCommission_Address"].ToString();
        txtReason.Text = dr["OfficeAutomation_Document_AfterCommission_Reason"].ToString();
        txtData.Text = dr["OfficeAutomation_Document_AfterCommission_Data"].ToString();
        ...
        if(txtDepartment.Text=="法律部")
            SbJs.Append("$('#trComcate').show();");
        else
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        T_OfficeAutomation_Document_AfterCommission t_OfficeAutomation_Document_AfterCommission = new T_OfficeAutomation_Document_AfterCommission();
        ...
        try
        {
            ...
            if (IsNewApply)
            {
                ...
                t_OfficeAutomation_Main.OfficeAutomation_SerialNumber = "AfterCommission" + DateTime.Now.ToString("yyyyMMddHHmmssfffffff");
                t_OfficeAutomation_Main.OfficeAutomation_DocumentID = 37; //在《申请表字典表》t_Dic_OfficeAutomation_Document，找到该表的主键，这步一错则会打开别的表
                ...
                t_OfficeAutomation_Document_AfterCommission = GetModelFromPage(Guid.NewGuid());
                
                t_OfficeAutomation_Main.OfficeAutomation_Main_Apply = t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_Apply;
                t_OfficeAutomation_Main.OfficeAutomation_Main_Department = t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_Department;
                t_OfficeAutomation_Main.OfficeAutomation_Main_ApplyDate = t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_ApplyDate;
                t_OfficeAutomation_Main.OfficeAutomation_Main_Summary = t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_Subject;

                da_OfficeAutomation_Main_Inherit.Insert(t_OfficeAutomation_Main);
                da_OfficeAutomation_Document_AfterCommission_Inherit.Insert(t_OfficeAutomation_Document_AfterCommission);//插入申请表

                //InsertAfterCommissionDetail(t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_ID);
                ...
                Common.AddLog(EmployeeID, EmployeeName, 41, new Guid(MainID), 1);//日志，创建申请表
                Common.AddLog(EmployeeID, EmployeeName, 3, new Guid(MainID), 1);//日志，创建流程

                if(txtDepartment.Text != "法律部")
                    RunJS("window.showModalDialog(\"/Apply/Apply_UploadFile.aspx?MainID=" + MainID + "\", '" + MainID + "', \"dialogHeight=165px\");var win=window.showModalDialog(\"Apply_AfterCommission_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=800px;dialogHeight=320px\");if(win=='success'){window.location='/Apply/Apply_Search.aspx'; }");
                else
                    RunJS("window.showModalDialog(\"/Apply/Apply_UploadFile.aspx?MainID=" + MainID + "\", '" + MainID + "', \"dialogHeight=165px\");var win=window.showModalDialog(\"Apply_AfterCommission_Flow2.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=800px;dialogHeight=320px\");if(win=='success'){window.location='/Apply/Apply_Search.aspx'; }");
                #endregion
            }
            else
            {
                ...
                if (Purview.Contains("OA_ITPower_001"))//是否有编辑权限
                {
                    ...
                    DA_OfficeAutomation_Document_Flow_Inherit da_OfficeAutomation_Document_Flow_Inherit = new DA_OfficeAutomation_Document_Flow_Inherit();
                    if(txtDepartment.Text=="法律部")
                        da_OfficeAutomation_Flow_Inherit.UpdateIdxByMainIDAndIdx(MainID, 2, 4);
                    else
                        da_OfficeAutomation_Flow_Inherit.UpdateIdxByMainIDAndIdx(MainID, 4, 2);
    ...
    protected void btnSign_Click(object sender, EventArgs e)
    {
        ...
        DataSet ds = da_OfficeAutomation_Document_AfterCommission_Inherit.SelectByMainID(MainID);
        DataRow drRow = ds.Tables[0].Rows[0];
        ID = drRow["OfficeAutomation_Document_AfterCommission_ID"].ToString(); 
        ...
        if (Purview.Contains("OA_ITPower_002"))
        {
            try
            {
                ...
                if (isSignSuccess)
                {
                    ...
                    string apply = ds.Tables[0].Rows[0]["OfficeAutomation_Document_AfterCommission_Apply"].ToString();
                    string employname;
                    string serialNumber = drRow["OfficeAutomation_SerialNumber"].ToString();
                    string documentName = da_OfficeAutomation_Flow_Inherit.GetDocumentNameByMainID(MainID);
                    string department = drRow["OfficeAutomation_Document_AfterCommission_Department"].ToString();
                    ...
                    sbMailBody.Append("<br/><br/>申请人：" + drRow["OfficeAutomation_Document_AfterCommission_Apply"]);
                    sbMailBody.Append("<br/>发文部门：" + drRow["OfficeAutomation_Document_AfterCommission_Department"]);
                    sbMailBody.Append("<br/>发文日期：" + DateTime.Parse(drRow["OfficeAutomation_Document_AfterCommission_ApplyDate"].ToString()).ToString("yyyy-MM-dd"));
                    sbMailBody.Append("<br/>发文编号：" + drRow["OfficeAutomation_Document_AfterCommission_ApplyID"]);
                    sbMailBody.Append("<br/>申请追佣项目名称：" + drRow["OfficeAutomation_Document_AfterCommission_Subject"]);
                    sbMailBody.Append("<br/>回复电话：" + drRow["OfficeAutomation_Document_AfterCommission_ReplyPhone"]);
                    sbMailBody.Append("<br/>所属发展商集团：" + drRow["OfficeAutomation_Document_AfterCommission_Name"]);
                    sbMailBody.Append("<br/>之前已收佣情况：" + drRow["OfficeAutomation_Document_AfterCommission_HasCommission"]);
                    sbMailBody.Append("<br/>申请追佣金额：" + drRow["OfficeAutomation_Document_AfterCommission_CommissionMoney"]);
                    sbMailBody.Append("<br/>发展商联系人及电话：" + drRow["OfficeAutomation_Document_AfterCommission_Contact"]);
                    sbMailBody.Append("<br/>发展商发函地址：" + drRow["OfficeAutomation_Document_AfterCommission_Address"]);
                    sbMailBody.Append("<br/>申请追佣原因：" + drRow["OfficeAutomation_Document_AfterCommission_Reason"]);
                    sbMailBody.Append("<br/>可提交的追佣资料：" + drRow["OfficeAutomation_Document_AfterCommission_Data"]);

                    sbMailBody.Append("<br/><br/>");

                    //sbMailBody.Append("<br/>其它坏账详细情况：");

                    //ds = da_OfficeAutomation_Document_AfterCommission_Detail_Inherit.SelectByMainID(MainID);
                    //foreach (DataRow dr in ds.Tables[0].Rows)
                    //{
                    //    sbMailBody.Append("<br/>经办人：" + dr["OfficeAutomation_Document_AfterCommission_Detail_Controler"]);
                    //    sbMailBody.Append("<br/>物业成交编号：" + dr["OfficeAutomation_Document_AfterCommission_Detail_PropertyID"]);
                    //    sbMailBody.Append("<br/>物业成交日期：" + dr["OfficeAutomation_Document_AfterCommission_Detail_PropertyDate"]);
                    //    sbMailBody.Append("<br/>原成交价：" + dr["OfficeAutomation_Document_AfterCommission_Detail_OldDeal"]);
                    //    sbMailBody.Append("<br/>现成交价：" + dr["OfficeAutomation_Document_AfterCommission_Detail_NewDeal"]);
                    //    sbMailBody.Append("<br/>成交价调整：" + dr["OfficeAutomation_Document_AfterCommission_Detail_AjustDeal"]);
                    //    sbMailBody.Append("<br/>应收佣金：" + dr["OfficeAutomation_Document_AfterCommission_Detail_ShouldComm"]);
                    //    sbMailBody.Append("<br/>实收佣金：" + dr["OfficeAutomation_Document_AfterCommission_Detail_ActualComm"]);
                    //    sbMailBody.Append("<br/>调整佣金：" + dr["OfficeAutomation_Document_AfterCommission_Detail_AjustComm"]);
                    //    sbMailBody.Append("<br/>客户联系电话：" + dr["OfficeAutomation_Document_AfterCommission_Detail_Commitment"]);
                    //    sbMailBody.Append("<br/>坏帐原因：" + dr["OfficeAutomation_Document_AfterCommission_Detail_Reason"]);
    ...
    protected void btnSignSave_Click(object sender, EventArgs e)
    {
        ...
        //        da_OfficeAutomation_Document_AfterCommission_Inherit.AddRemarkByID(ID, CommonConst.SIGN_FINANCE);
        //    Alert("标记成功。");
        //}
        //catch
        //{
    ...
    private T_OfficeAutomation_Document_AfterCommission GetModelFromPage(Guid UndertakeProjID)
    {
        T_OfficeAutomation_Document_AfterCommission t_OfficeAutomation_Document_AfterCommission = new T_OfficeAutomation_Document_AfterCommission();
        t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_ID = UndertakeProjID;
        t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_MainID = new Guid(MainID);
        t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_Apply = EmployeeName;
        t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_ApplyDate = DateTime.Now;
        t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_ApplyID = txtApplyID.Text;
        t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_Department = txtDepartment.Text;
        t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_Subject = txtSubject.Text;
        t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_ReplyPhone = txtReplyPhone.Text;

        t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_Name = txtName.Text;
        t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_HasCommission = txtHasCommission.Text;
        t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_CommissionMoney = txtCommissionMoney.Text;
        t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_Contact = txtContact.Text;
        t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_Address = txtAddress.Text;
        t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_Reason = txtReason.Text;
        t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_Data = txtData.Text;

        return t_OfficeAutomation_Document_AfterCommission;
    }
    ...
    private void Update()
    {
        ...
        DataSet ds = new DataSet();
        ds = da_OfficeAutomation_Document_AfterCommission_Inherit.SelectByMainID(MainID);
        ID = ds.Tables[0].Rows[0]["OfficeAutomation_Document_AfterCommission_ID"].ToString();

        t_OfficeAutomation_Document_AfterCommission = GetModelFromPage(new Guid(ID));

        string apply = t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_Apply;
        string depname = t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_Department;
        string summary = t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_Subject;
        string applydate = t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_ApplyDate.ToString("yyyy-MM-dd");
        string mainid = t_OfficeAutomation_Document_AfterCommission.OfficeAutomation_Document_AfterCommission_MainID.ToString();

        da_OfficeAutomation_Main_Inherit.UpdateMain(mainid, depname, apply, applydate, summary);
        da_OfficeAutomation_Document_AfterCommission_Inherit.Update(t_OfficeAutomation_Document_AfterCommission);//修改申请表

        Common.AddLog(EmployeeID, EmployeeName, 41, new Guid(MainID), 2);//日志，修改申请表
    }
    ...
    protected void btnReWrite_Click(object sender, EventArgs e)
    {
        ...
        Session["FLG_ReWrite42"] = "1";
        Response.Write("<script>window.open('Apply_AfterCommission_Detail.aspx?MainID=" + MainID + "');</script>");
    }
    protected void btnSPDF_Click(object sender, EventArgs e) //M_PDF
    {
        try
        {
            ...
            Response.AddHeader("content-disposition", "attachment; filename=" + HttpUtility.UrlEncode("物业部一手项目追佣申请表.pdf"));//强制下载 
    ...
    protected void btnCancelSign_Click(object sender, EventArgs e) //20141202：CancelSign
    {
        try
        {
            ...
            string signEmployeeName = EmployeeName;
            DA_OfficeAutomation_Document_AfterCommission_Inherit da_OfficeAutomation_Document_AfterCommission_Inherit = new DA_OfficeAutomation_Document_AfterCommission_Inherit();
            DataSet ds = da_OfficeAutomation_Document_AfterCommission_Inherit.SelectByMainID(MainID); //在不同的表中要修改
            DataRow drRow = ds.Tables[0].Rows[0];
            string apply = ds.Tables[0].Rows[0]["OfficeAutomation_Document_AfterCommission_Apply"].ToString();
            string employname;
            string serialNumber = drRow["OfficeAutomation_SerialNumber"].ToString();
    ...
    protected void btnSAlterC_Click(object sender, EventArgs e) //20141216：M_AlterC
    {
        try
        {
            ...
            da_OfficeAutomation_Main_Inherit.Update(t_OfficeAutomation_Main);//AlterC_1a
    ...
    protected void btnEditFlow_DoClick(object sender, EventArgs e)
    {
        try
        {
            ...
            //在不同的表中要修改
            if (department != "法律部")
                RunJS("var win=window.showModalDialog(\"Apply_AfterCommission_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=830px;dialogHeight=280px\");if(win=='success'){window.location='/Ajax/Editor_Flow.ashx?MainID=" + MainID + "&applyUrl=" + applyUrl + "&apply=" + apply + "&serialNumber=" + serialNumber + "&department=" + department + "&Jhref=" + Page.Request.Url + "&mid=" + EmployeeID + "'; }");
            else
                RunJS("var win=window.showModalDialog(\"Apply_AfterCommission_Flow2.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=830px;dialogHeight=280px\");if(win=='success'){window.location='/Ajax/Editor_Flow.ashx?MainID=" + MainID + "&applyUrl=" + applyUrl + "&apply=" + apply + "&serialNumber=" + serialNumber + "&department=" + department + "&Jhref=" + Page.Request.Url + "&mid=" + EmployeeID + "'; }");
        }
        catch (Exception ex)
        {
    ...
    {
        DA_OfficeAutomation_Flow_Inherit da_OfficeAutomation_Flow_Inherit = new DA_OfficeAutomation_Flow_Inherit();
        DA_OfficeAutomation_Document_AfterCommission_Inherit da_OfficeAutomation_Document_BadDebts_Inherit = new DA_OfficeAutomation_Document_AfterCommission_Inherit();
