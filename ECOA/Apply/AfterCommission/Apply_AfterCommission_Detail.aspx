<%@ Page ValidateRequest="false" Title="物业部一手项目追佣申请表 - 广州中原电子审批系统" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="Apply_AfterCommission_Detail.aspx.cs" Inherits="Apply_AfterCommission_Apply_AfterCommission_Detail" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
		...
		$(function() {      
		    ...
		    //$("#tbAround").css("border", "1px solid #98b8b5");
		    //$("#tbAround").find("td").css("color","#022241");
		    //$("span:contains('文档编码')").css("color","#022241");
		    //$("#tbAround").find("td").each(function (i) {
		    //    $(this).css("border", "1px solid #98b8b5");
		    //});
		    //$("input").css("border", "1px solid #98b8b5");
		    //$("textarea").css("border", "1px solid #98b8b5");
		    //$("#tbAround").find("tr[id^=trManager]:not(:first)").find("td").each(function (i) {
		    //    $(this).css("border-top-style","hidden");
		    //});
		    //$("#tbAround").find("tr[id^=trManager]").find("td:not(:first)").each(function (i) {
		    //    $(this).css("border-left-style","hidden");
		    //});
		    //$("#tbAround").find("tr.noborder").each(function (i) {
		    //    $("label:contains('同意'):not(:contains('不'))").css("color", "#008162");
		    //    $("label:contains('不同意')").css("color", "#fe0100");
		    //    $("label:contains('其他意见')").css("color", "#008162");
		    //});
		    ...
		    autoTextarea(document.getElementById("txtIDx4"));
		    autoTextarea(document.getElementById("txtIDx5"));
	    ...
	    function check() {
	        ...
	        if($.trim($("#<%=txtHasCommission.ClientID %>").val())=="") {
	            alert("之前已收佣情况不可为空！");
	            $("#<%=txtHasCommission.ClientID %>").focus();
	            return false;
	        }
	        if($.trim($("#<%=txtCommissionMoney.ClientID %>").val())=="") {
	            alert("申请追佣金额不可为空！");
	            $("#<%=txtCommissionMoney.ClientID %>").focus();
	            return false;
            }
            if($.trim($("#<%=txtContact.ClientID %>").val())=="") {
                alert("发展商联系人及电话不可为空！");
	        ...
	        if($.trim($("#<%=txtData.ClientID %>").val())=="") {
	            alert("可提交的追佣资料不可为空！");
	            $("#<%=txtData.ClientID %>").focus();
	            return false;
            }
	    }
		...
		function upload() {
		    ...
		    if(sReturnValue=='success')
			    window.location='Apply_AfterCommission_Detail.aspx?MainID=<%=MainID %>';
		}

        function editflow(){
            var vad = $.trim($("#<%=txtDepartment.ClientID %>").val());
            if(vad != '法律部')
                var win=window.showModalDialog("Apply_AfterCommission_Flow.aspx?MainID=<%=MainID %>","","dialogWidth=800px;dialogHeight=320px");
            else
                var win=window.showModalDialog("Apply_AfterCommission_Flow2.aspx?MainID=<%=MainID %>","","dialogWidth=800px;dialogHeight=320px");
            if(win=='success')
            {
                $("#<%=txtDepartment.ClientID %>").val(vad);
                window.location="Apply_AfterCommission_Detail.aspx?MainID=<%=MainID %>&Dsp="+vad;
            }
        }
        function editflow2(){
            var win=window.showModalDialog("Apply_AfterCommission_Flow2.aspx?MainID=<%=MainID %>","","dialogWidth=800px;dialogHeight=320px");
            if(win=='success')
                window.location="Apply_AfterCommission_Detail.aspx?MainID=<%=MainID %>&Dsp=法律部";
        }
		...
		<div style='text-align: center'>
			...
			<table id="tbAround"  width='640px'>
				...
				<tr>
                    <td>发文部门</td>
                    <td class="tl PL10">
                        <asp:TextBox ID="txtDepartment" runat="server"></asp:TextBox>&nbsp;-&nbsp;<input type="hidden" id="hdDepartmentID" runat="server" /><asp:Label ID="lblApply" runat="server"></asp:Label>
                    </td>
                    <td>发文日期</td>
				...
				<tr>
                    <td  colspan="4" style="text-align: left; line-height: 25px; padding-left: 10px;">
                        申请追佣项目名称：<asp:textbox id="txtSubject" runat="server" Width="190px"></asp:textbox>　所属发展商集团：<asp:textbox id="txtName" runat="server" Width="182px"></asp:textbox><br />
                        之前已收佣情况：<asp:textbox id="txtHasCommission" runat="server" Width="200px"></asp:textbox>　申请追佣金额：<asp:textbox id="txtCommissionMoney" runat="server" Width="196px"></asp:textbox><br />
                        发展商联系人及联系电话：<asp:textbox id="txtContact" runat="server" Width="452px"></asp:textbox><br />
                        发展商发函地址：<asp:textbox id="txtAddress" runat="server" Width="500px"></asp:textbox><br />
                        <div style="padding-top: 7px">
                            <span style="vertical-align: top;">申请追佣原因：</span>
                            <asp:textbox id="txtReason" runat="server" TextMode="MultiLine" Width="510px" Height="150px"></asp:textbox><br />
                        </div>          
                        <div style="padding-top: 7px">
                            <span style="vertical-align: top;">可提交的追佣资料：</span>                                                              
                            <asp:textbox id="txtData" runat="server" Width="485px" Height="150px" TextMode="MultiLine"></asp:textbox><br />  
                        </div>                                      
                        接盘部门知悉并同意，通过法律部追收的一手项目佣金，公司将扣除追佣总额的15%作行政费用。
                    </td>
				</tr>
				...
				<tr  id="trComcate" class="noborder" style="height: 85px;display: none;">
				...
				<tr id="trGeneralManager" class="noborder" style="height: 100px;">
					<td >董事总经理<br />审批意见</td>
					<td colspan="3" class="tl PL10" style=" ">
                        ...
                        <input id="rdbOtherIDx5" type="radio" name="agree5" /><label for="rdbOtherIDx5">其他意见</label><br /><br />
...
<ItemStyle HorizontalAlign="Center" VerticalAlign="Middle" Width="300px"></ItemStyle>
				</asp:TemplateField>
			</Columns>
