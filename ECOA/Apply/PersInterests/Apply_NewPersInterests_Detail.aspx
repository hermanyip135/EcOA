...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function check() {
            ...
            if(!$("#<%=rbInApply.ClientID %>").prop('checked') && !$("#<%=rbNoInApply.ClientID %>").prop('checked') ) {
                alert("请选择是否在职");
                $("#<%=rbInApply.ClientID %>").focus();
                return false;
            }
            if($("#<%=rbInApply.ClientID %>").prop('checked') )
            {
            ...
            if($.trim($("#<%=txtTfid1.ClientID %>").val())=="") {
                alert("身份证号不可为空。");
                $("#<%=txtTfid1.ClientID %>").focus();
                return false;
            }
            ...
            if($.trim($("#<%=txtTfid3.ClientID %>").val())=="") {
                alert("职位不可为空。");
                $("#<%=txtTfid3.ClientID %>").focus();
                return false;
            }
            if($.trim($("#<%=txtTfid4.ClientID %>").val())=="") {
                alert("入职不可为空。");
                $("#<%=txtTfid4.ClientID %>").focus();
                return false;
            }
               ...
               ) 
            {
             ...
             if($("#<%=cbApplyContent1.ClientID%>").prop("checked") || $("#<%=ddlInterestsType.ClientID%>").val() == "1") {
                ...
                if(!$("#<%=rdbDealKind2.ClientID%>").prop("checked") && !$("#<%=rdbDealKind3.ClientID%>").prop("checked") && !$("#<%=rdbDealKind4.ClientID%>").prop("checked")){
                
                    alert("请选择交易类型。");
                    $("#<%=rdbDealKind2.ClientID %>").focus();
                    return false;
                }
            ...
            $(".trDetail").each(function(n) {
                ...
                if (n != 0 && n != $("#tbDetail tr").size()-1 && $("#<%=cbApplyContent2.ClientID%>").prop("checked")) {
                    ...
                    else if (!$("#txtDetail_rdInApply" + n).prop('checked')&& !$("#txtDetail_rbNoInApply" + n).prop('checked')) {
                        alert("第" + n + "行的是否在职须选择。");
                        $("#txtDetail_rdInApply" + n).focus();
                        flag = false;
                    }
                    else if($("#txtDetail_rdInApply" + n).prop('checked'))
                    {
                        if ($.trim($("#txtDetail_txtApplyForID" + n).val()) == "")
                        {
                            alert("第" + n + "行的在职/劳务,工号必须填写。");
                            $("#txtDetail_txtApplyForID" + n).focus();
                            flag = false;
                        }
                            ...
                            if($.trim($("#<%=txtTfid2.ClientID %>").val()) == $.trim($("#txtDetail_InDepartment" + n).val()) && $("#relationshipOne" + n).val()=="直系亲属")
                            {
                                alert("第" + n + "行据公司规定直系亲属不能入职同一组别。");
                                $("#txtDetail_InDepartment" + n).focus();
                                flag = false;
                            }
            ...
            else if ($.trim($("#txtDetail_txtTfid" + n).val()) == "") {
                alert("第" + n + "行的身份证必须填写。");
                $("#txtDetail_txtTfid" + n).focus();
                flag = false;
            }
    ...
    if(fl){
        data += $.trim($("#txtDetail_RelativesName" + n).val()) 
            ...
            + "&&" +($("#txtDetail_rdInApply" + n).prop('checked')?"1":"0")
            + "&&" + $.trim($("#txtDetail_txtApplyForID" + n).val())
            + "&&" +$.trim($("#txtDetail_txtTfid" + n).val())
            + "||";
    }else{
        data += " && && && ||";
    }
    ...
    function getSuggestion(idx)
    {
        ...
        function getEmployee(n) {
            $.ajax({
                ...
                type: "post",
                dataType: "text",
                data: "action=GetEmployeeAndLabour&code=" + n.value,
                success: function(info) {
                    if (info != "fail") {
                        var infos = info.split("|");
                        $("#<%=txtApplyFor.ClientID%>").val(infos[1]);
                        $("#<%=hdApply.ClientID%>").val(infos[1]);
                        $("#<%=txtTfid1.ClientID%>").val(infos[2]);
                        $("#<%=txtTfid2.ClientID%>").val(infos[3]);
                        $("#<%=txtTfid3.ClientID%>").val(infos[5]);
        ...
        function addRow() {
            ...
            var html = '<tr class="trDetail">'
            +' <td class=\"tl PL10\" style="line-height: 30px">'
              
                +'<input type=\"radio\" id=\"txtDetail_rdInApply' + i + '\" name=\"rdApply' + i + '\"/>在职/劳务,工号<input type=\"text\" id=\"txtDetail_txtApplyForID' + i + '\" style=\"width:50px\" onblur=\"getCommEmployee(this,' + i + ');\"/>'
                +'<input type=\"radio\" id=\"txtDetail_rbNoInApply' + i + '\" name=\"rdApply' + i + '\"/>尚未入职<br/>'
				+ '             亲属姓名：<input type="text" id="txtDetail_RelativesName' + i + '" style="width:150px;"/>'
				+               '　所在部门：<input type="text" id="txtDetail_InDepartment' + i + '" style="width:200px;"/><br />'
                +               '担任职位：<input type="text" id="txtDetail_Position' + i + '" style="width:200px;"/>'
               +               '身份证：<input type=\"text\" id=\"txtDetail_txtTfid' + i + '\" style=\"width:200px;\"/> <br/>'
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
			...
			<table id="tbAround"  width='640px'>
                ...
                <tr>
                   <td class="auto-style2">﹡申请人</td>
                    <td colspan="3" style="text-align:left">
                        <asp:RadioButton ID="rbInApply" GroupName="gnInApply" runat="server" Text="在职/劳务,"/> 工号：<asp:TextBox ID="txtApplyForID" runat="server" Width="60px" onblur="getEmployee(this);"></asp:TextBox>
                    &nbsp; &nbsp;&nbsp;&nbsp;
                          <asp:RadioButton ID="rbNoInApply" GroupName="gnInApply" runat="server" Text="尚未入职" />
                    </td>
                </tr>
                <tr>
                    <td class="auto-style2">﹡申请人姓名</td>
                    <td class="tl PL10">
                        <%--<input id="txtApplyFor" type="text" runat="server" />--%>
                        <asp:TextBox ID="txtApplyFor" runat="server"></asp:TextBox>
                      <%--  工号：<asp:TextBox ID="txtApplyForID" runat="server" Width="50px" onblur="getEmployee(this);"></asp:TextBox>--%>
                        <%--<span id="spanApplyFor" style="display:none;">姓名：<input id="txtApplyFor" type="text" runat="server" readonly="readonly" style="background-color:Silver; width:50px;"/></span>--%>
                ...
                <tr class="trgray1">
                    ...
                    <td colspan="3" style="padding: 5px; text-align: left; line-height: 30px;">
                         <table id="tbAround3" style="border-collapse: collapse" width="510px">
                               <tr>
                                   <td class="tl PL10" rowspan="4">业权人：</td>
                                   <td>
                                       <asp:RadioButton ID="rdbPropertyHander1" GroupName="PropertyHander" runat="server" Text="本人" />
                                   </td>
                               </tr>
                               <tr><td>
                                    <asp:RadioButton ID="rdbPropertyHander2" GroupName="PropertyHander" runat="server" Text="本人及联名人" />
                                    姓名<asp:TextBox ID="txtMeAndHim" runat="server" Width="100px"></asp:TextBox> 
                                    关系 <asp:TextBox ID="txtRelationship1" runat="server" Width="100px"></asp:TextBox>
                               </td></tr>
                               <tr><td>
                                   业权联名人是否同为公司员工﹡ <asp:RadioButton ID="rbCompanyStaff" GroupName="CompanyStaff" runat="server" Text="是" />
                                   <asp:RadioButton ID="rbNoCompanyStaff" GroupName="CompanyStaff" runat="server" Text="否" />
                               </td></tr>
                               <tr><td>
                      ...
                      <%--  <asp:RadioButton ID="rdbPropertyHander2" GroupName="PropertyHander" runat="server" Text="本人及联名人" />
                        ...
                        物业地址：<asp:TextBox ID="txtBuilding" runat="server" Width="245px" ></asp:TextBox>
                        面积：<asp:TextBox ID="txtSquare" runat="server" Width="70px" ></asp:TextBox>平方米
                        <br />
                          该物业地址已申报：<label id="DeclaredNum" runat="server" style="color:red"></label>次
                        <br />
                        ...
                        1、如申报交易物业为二手物业，请上传《知悉函》 <a href="知悉函（购房）.docx"> 作为附件《知悉函》样本下载</a><br/>
                        2、公司不允许申请人以“代理人”身份促成物业交易，请知悉！
                ...
                <tr>
                   ...
                   <tr  id="tr1" class="noborder" style="height:30px;">
                    <td class="auto-style2">申请人</td>
                ...
                <tr  id="trManager1" class="noborder" style="height:30px;">
                    ...
                    <td colspan="3" class="tl PL10" style=" ">
                        <input id="rdbYesIDx2" type="radio" name="agree2" /><label for="rdbYesIDx2">同意</label>
                        <input id="rdbNoIDx2" type="radio" name="agree2" /><label for="rdbNoIDx2">不同意</label><br />
                        <textarea id="txtIDx2" rows="3" style="width:98%; overflow:auto; "></textarea>
                        <input type="button" id="btnSignIDx2" value="签名" onclick="sign('2')" style="display:none;"/>
                        <div class="signdate">日期：<span id="spanDateIDx2">_________</span></div></td>
                </tr>
                <tr  id="trManager2" class="noborder" style="height:30px;">
                    ...
                    <td colspan="3" class="tl PL10" >
                        ...
                        <input id="rdbNoIDx3" type="radio" name="agree3" /><label for="rdbNoIDx3">不同意</label><br />
                        ...
                        <div class="signdate">日期：<span id="spanDateIDx3">_________</span></div></td>
                </tr>
                <tr  id="trManager3" class="noborder" style="height:30px;">
                    ...
                    <td colspan="3" class="tl PL10">
                        ...
                        <input id="rdbNoIDx4" type="radio" name="agree4" /><label for="rdbNoIDx4">不同意</label><br />
                ...
                <tr class="noborder" style="height:30px;">
                    ...
                    <td colspan="3" class="tl PL10" style=" " >
                        ...
                        <asp:Button  Text="确认" runat="server"   Visible="false" ID="addhq" OnClick="addhq_Click"/>
                         <asp:Button  Text="取消审核人" runat="server"   Visible="false" ID="cleanhq" OnClick="cleanhq_Click"/>
                    </td>                            
                </tr>
                <%--<tr class="noborder" style="height:30px">
                    <td colspan="3" class="tl PL10" style=" " >
                        <input id="rdbYesIDx6" type="radio" name="agree6" /><label for="rdbYesIDx6">同意</label>
                    <input id="rdbNoIDx6" type="radio" name="agree6" /><label for="rdbNoIDx6">不同意</label>
                ...
                <tr class="noborder" style="height:30px; ">
                    <td colspan="3" class="tl PL10" style=" " >
                        <input id="rdbYesIDx7" type="radio" name="agree7" /><label for="rdbYesIDx7">同意</label>
                        <input id="rdbNoIDx7" type="radio" name="agree7" /><label for="rdbNoIDx7">不同意</label>
                   ...
                   <tr  id="trOrder" class="noborder" style="height:30px;;display:none">
                    <td class="auto-style2">其他审核人</td>
                    <td colspan="3" class="tl PL10">
                        ...
                        <input id="rdbNoIDx8" type="radio" name="agree8" /><label for="rdbNoIDx8">不同意</label>
                         <input id="rdbOtherIDx8" type="radio" name="agree8" /><label for="rdbOtherIDx8">其他意见</label>
                        <br />
                        <textarea id="txtIDx8" rows="3" style="width:98%; overflow:auto;"></textarea>
                        <input type="button" id="btnSignIDx8" value="签名" onclick="sign('8')" style="display:none;"/>
                ...
                <tr id="add1F" style="display: none">
                    ...
                    <tr id="trYY" class="noborder" style="height:30px; display:none; ">
                    <td class="auto-style2" >营运支持</td>
                    <td colspan="3" class="tl PL10" style=" " >
                        ...
                        <input type="button" id="btnSignIDx10" value="签署意见" onclick="sign('10')" style="display:none;"/><div class="signdate">日期：<span id="spanDateIDx10">_________</span></div>
                    </td>       
                </tr>
                ...
                <tr id="trLogistics" class="noborder" style="height:30px; display:none; ">
                    ...
                    <td colspan="3" class="tl PL10" style=" " >
                        ...
                        <label for="rdbYesIDx17">同意</label><input id="rdbNoIDx17" type="radio" name="agree17" />
                        <label for="rdbNoIDx17">不同意</label><input id="rdbOtherIDx17" type="radio" name="agree17" />
                        <label for="rdbOtherIDx17">其他意见</label><br />
                      ...
                      　<a id="dfd" href="javascript:void(0)" style="display: none;" onclick="deleteFlow()">删除添加的审批环节</a><br />
                        <textarea id="txtIDx17" rows="3" style="width:98%; overflow:auto;">
                        </textarea><input type="button" id="btnSignIDx17" value="签署意见" onclick="sign('17')" style="display:none;"/>
                ...
                <tr id="trGeneralManager" class="noborder" style="height:30px; display:none; ">
                    ...
                    <td colspan="3" class="tl PL10" style=" " >
                        ...
                        <input id="rdbOtherIDx18" type="radio" name="agree18" /><label for="rdbOtherIDx18">其他意见</label><br />
    ...
    <script type="text/javascript">
        $.each($("textarea:not([id*=txtDescribe])"), function (idx, item) { autoTextarea(this); }); //使对话框自适应
        $("#<%=rbNoInApply.ClientID%>").click(function(){
            $("#<%=txtApplyForID.ClientID%>").val("");
            $("#<%=txtApplyForID.ClientID%>").attr("disabled","disabled");
            $("#<%=txtApplyFor.ClientID%>").val("");
            $("#<%=hdApply.ClientID%>").val("");
            $("#<%=txtTfid1.ClientID%>").val("");
            $("#<%=txtTfid2.ClientID%>").val("");
        ...
        $("#<%=rbInApply.ClientID%>").click(function(){
            $("#<%=txtApplyForID.ClientID%>").val("");
            $("#<%=txtApplyForID.ClientID%>").removeAttr("disabled");
            $("#<%=txtApplyFor.ClientID%>").val("");
            $("#<%=hdApply.ClientID%>").val("");
            $("#<%=txtTfid1.ClientID%>").val("");
            $("#<%=txtTfid2.ClientID%>").val("");
        ...
        function getCommEmployee(n,num)
        {
            // var num =1;
            $.ajax({
                url: "/Ajax/HR_Handler.ashx",
                ...
                success: function(info) {
                    if (info != "fail") {
                        var infos = info.split("|");
                        $("#txtDetail_RelativesName"+num).val(infos[1]);
                        $("#txtDetail_InDepartment"+num).val(infos[3]);
                        $("#txtDetail_Position"+num).val(infos[5]);
                        $("#txtDetail_txtTfid"+num).val(infos[2]);
                        //  $("#spanApplyFor").show();
                    }
                    else{
                        $("#txtDetail_RelativesName"+num).val('');
                        $("#txtDetail_InDepartment"+num).val('');
                        $("#txtDetail_Position"+num).val('');
                        $("#txtDetail_txtTfid"+num).val('');
                        //  $("#spanApplyFor").hide();
                    }
                }
            })
        }
        function getLoadEmployee(n) {
            if(n !== "")
            {
                $.ajax({
                    url: "/Ajax/HR_Handler.ashx",
                    type: "post",
                    dataType: "text",
                    data: "action=GetEmployeeAndLabour&code=" + n,
                    success: function(info) {
                        if (info != "fail") {
                            var infos = info.split("|");
                            $("#<%=txtApplyFor.ClientID%>").val(infos[1]);
        ...
        $(window).load(function(){ 
            var vApplyForDate =$("#<%=txtApplyForDate.ClientID %>").val();
           
            if(vApplyForDate<'2018-12-18')
            {
                getLoadEmployee($("#<%=txtApplyForID.ClientID %>").val());
            }
        }); 
    </script>
</asp:Content>
