...
public partial class Apply_OndutyVacation_Apply_OndutyVacation_Detail : BasePage
{
    #region 变量声明及定义
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (IsNewApply)
            {
                ...
                try
                {
                    T_OfficeAutomation_Flow flows = da_OfficeAutomation_Flow_Inherit.SelectByMainIDAndIdx(MainID, 10);
                    if (flows == null || flows.OfficeAutomation_Flow_EmployeeID != EmployeeID)
                    {
                        ...
                        t_OfficeAutomation_Flow.OfficeAutomation_Flow_EmployeeID = txtApplyForCode.Text;
                        t_OfficeAutomation_Flow.OfficeAutomation_Flow_Employee = txtApplyForName.Value;
                ...
                //string applyUrl = Page.Request.Url.ToString() + "?MainID=" + MainID;
                ...
                //string msnBody = "您好，杨斯敏：您有编号为" + t_OfficeAutomation_Main.OfficeAutomation_SerialNumber + "的" + documentName + "需要您的审批。<br />申请表地址为：" + applyUrl + "<br /><br /><span style=\"font-weight: bolder\">注意：该链接需要登陆门户系统并打开电子审批系统后才能进入，否则只能跳到登陆页面。</span>";
                //Common.SendMessageEX(true, documentName, "杨斯敏", "请审批", msnBody, msnBody, MainID);
                //msnBody = "";
                //msnBody = "您好，林亦玲：您有编号为" + t_OfficeAutomation_Main.OfficeAutomation_SerialNumber + "的" + documentName + "需要您的审批。<br />申请表地址为：" + applyUrl + "<br /><br /><span style=\"font-weight: bolder\">注意：该链接需要登陆门户系统并打开电子审批系统后才能进入，否则只能跳到登陆页面。</span>";
                //Common.SendMessageEX(true, documentName, "林亦玲", "请审批", msnBody, msnBody, MainID);
    ...
    protected void btnSign_Click(object sender, EventArgs e)
    {
        ...
        if (Purview.Contains("OA_ITPower_002"))
        {
            try
            {
                ...
                t.OfficeAutomation_Document_OndutyVacation_Salary = rbtSalary1.Checked == true ? "10" : rbtSalary2.Checked == true ? "20" : "0";
                t.OfficeAutomation_Document_OndutyVacation_FullDate = rbtFullDate1.Checked == true ? "10" : rbtFullDate2.Checked == true ? "20" : "0";
                bll.Edit(t);
                DA_OfficeAutomation_Flow_Inherit da_OfficeAutomation_Flow_Inherit = new DA_OfficeAutomation_Flow_Inherit();
                ...
                bool isSignSuccess = Convert.ToInt32(flowIDx) >= 50 ? da_OfficeAutomation_Flow_Inherit.UpdateContainIsAgreeForSign(MainID, signEmployeeId, signEmployeeName, hdSuggestion.Value, flowIDx, hdIsAgree.Value) : da_OfficeAutomation_Flow_Inherit.UpdateContainIsAgreeForSignAndMulti(MainID, signEmployeeId, signEmployeeName, hdSuggestion.Value, flowIDx, hdIsAgree.Value);
                ...
                if (isSignSuccess)
                {
                    ...
                    sbMailBody.Append("<br/>申请日期：" + DateTime.Parse(drRow["OfficeAutomation_Document_OndutyVacation_ApplyDate"].ToString()).ToString("yyyy-MM-dd"));
                    ...
                    if (hdIsAgree.Value != "0")//同意或其他意见
                    {
                        ...
                        if (da_OfficeAutomation_Flow_Inherit.IsFlowComplete(MainID))
                        {
                            ...
                            msnBody = "";
                            //审批流程完成，通知梁健菁
                            msnBody = "您好，梁健菁：编号为" + serialNumber + "的" + documentName + "已通过所有人的审理。<br />申请表" + applyUrl + "<br /><br /><span style=\"font-weight: bolder\">注意：该链接需要登陆门户系统并打开电子审批系统后才能进入，否则只能跳到登陆页面。</span>";
                           
                            if (hdIsAgree.Value == "2")
                                Common.SendMessageEX(false, "梁健菁", "其他意见", msnBody, msnBody);
                            else
                                Common.SendMessageEX(false, "梁健菁", "申请已同意", msnBody, msnBody);
                        }
                        else
                        {
                            //通知申请人
    ...
    protected void btnSPDF_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            Response.AddHeader("content-disposition", "attachment; filename=" + HttpUtility.UrlEncode("假期申请.pdf"));//强制下载 
    ...
    private T_OfficeAutomation_Document_OndutyVacation GetModelFromPage(Guid EOndutyVacationID)
    {
        ...
        t.OfficeAutomation_Document_OndutyVacation_Position = txtPosition.Value;
        ...
        else if (rbtType10.Checked) sType = "10";
        else if (rbtType9.Checked)
        {
            sType = "9";
            t.OfficeAutomation_Document_OndutyVacation_OtherType = txtOtherType.Value;
        }
        ...
        t.OfficeAutomation_Document_OndutyVacation_BeSSDate = ddlBeSSDate.SelectedValue;
        t.OfficeAutomation_Document_OndutyVacation_BeMMDate = ddlBeMMDate.SelectedValue;
        t.OfficeAutomation_Document_OndutyVacation_EndDDDate = txtEndDDDate.Text;
        t.OfficeAutomation_Document_OndutyVacation_EndSSDate = ddlEndSSDate.SelectedValue;
        t.OfficeAutomation_Document_OndutyVacation_EndMMDate = ddlEndMMDate.SelectedValue;
    ...
    private void LoadPage() 
    {
        ...
        txtPosition.Value = t.OfficeAutomation_Document_OndutyVacation_Position;
        //txtApplyForDate.Text = Convert.ToDateTime(t.OfficeAutomation_Document_OndutyVacation_ApplyForDate).ToString("yyyy-MM-dd") ;
        ...
        { 
            ...
            case "9":
                ...
                txtOtherType.Value = t.OfficeAutomation_Document_OndutyVacation_OtherType;
                break;
            case "10":
                rbtType10.Checked = true;
                break;
        }
        txtBeDDDate.Text = t.OfficeAutomation_Document_OndutyVacation_BeDDDate;
         ddlBeSSDate.SelectedValue =t.OfficeAutomation_Document_OndutyVacation_BeSSDate ;
         ddlBeMMDate.SelectedValue = t.OfficeAutomation_Document_OndutyVacation_BeMMDate;
        txtEndDDDate.Text = t.OfficeAutomation_Document_OndutyVacation_EndDDDate;
        ddlEndSSDate.SelectedValue = t.OfficeAutomation_Document_OndutyVacation_EndSSDate;
        ddlEndMMDate.SelectedValue = t.OfficeAutomation_Document_OndutyVacation_EndMMDate;
