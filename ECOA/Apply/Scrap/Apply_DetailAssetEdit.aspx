<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Apply_DetailAssetEdit.aspx.cs" Inherits="Apply_Scrap_Apply_DetailAssetEdit" %>
...
<body>
    <form id="form1" runat="server">
        ...
        <div style="width: 90%; margin: 0 auto; text-align: center; padding-top: 15px;">
            <asp:Button ID="btnSure" runat="server" Text="确认" OnClick="btnSure_Click" CssClass="commonbtn submit" OnClientClick="return cansubmit();" Style="display: none" />
    ...
    <script type="text/javascript">
        ...
        function confirmzc(canedit) {
            ...
            var headhtml = '<tr><th style="width:4%">序  号</th><th style="width:4%">操  作</th><th style="width:15%">归　属</th><th style="width:15%">资产名称</th><th style="width:15%">财务编号</th><th style="width:15%">规格型号</th><th style="width:15%">入账时间</th><th style="width:8%">数量</th><th style="width:15%">折旧摊分剩余费用</th></tr>';

            var html = "";
            for (var i = 0; i < zc.length; i++) {
                html += "<tr>";
                html += "<td>" + (i + 1) + "<input type='hidden' name='cAssetID' value='" + zc[i].AssetID + "' /></td>";                                            //序号
                html += "<td><a href='javascript:;' onclick='del(\"" + zc[i].AssetID + "\",this)'><img src='/Images/disable.gif' /></a></td>";
                html += "<td class='cType'>" + zc[i].AssetType + "</td>";                          //归属
                html += "<td class='cClasses'>" + zc[i].AssetClasses + "</td>";                       //资产名称
                html += "<td class='cAssetAssetNo'>" + zc[i].AssetAssetNo + "</td>";                       //财务编号
                html += "<td class='cTS'>" + zc[i].AssetTS + "</td>";                            //规格型号
                html += "<td class='cAssetRecordedTime'>" + zc[i].AssetRecordedTime + "</td>";                  //入账时间
                html += "<td><input type='text' name='cAssetNumber' value='" + zc[i].AssetNumber + "' style='width:50px' /></td>";                     //数量
                html += "<td><input type='text' name='cAssetSurplusValue' value='" + zc[i].AssetSurplusValue + "' /></td>";                     //折旧摊分剩余费用
                //zc[i];
                html += "</tr>"
            }
            $("#tbody").html(html);
        ...
        function cansubmit()
        {
            ...
            $("#tb tr").each(function () {
                //alert("进来了");
                var AssetID = $(this).find("input[name='cAssetID']").val();
                if (AssetID == "" || AssetID == undefined)
                { return true;}
                var AssetType = $(this).find("td.cType").html();                             //归属
                var AssetClasses = $(this).find("td.cClasses").html();                       //资产名称
                var AssetAssetNo = $(this).find("td.cAssetAssetNo").html();                  //财务编码
                var AssetTS = $(this).find("td.cTS").html();                                 //规格型号
                var AssetRecordedTime = $(this).find("td.cAssetRecordedTime").html();        //入账时间
                var AssetNumber = $(this).find("input[name='cAssetNumber']").val();           //数量
                var AssetSurplusValue = $(this).find("input[name='cAssetSurplusValue']").val();//折旧摊分剩余费用
                //生成json
                data = { "AssetID": AssetID, "AssetType": AssetType, "AssetClasses": AssetClasses, "AssetAssetNo": AssetAssetNo, "AssetTS": AssetTS, "AssetRecordedTime": AssetRecordedTime, "AssetNumber": AssetNumber, "AssetSurplusValue": AssetSurplusValue };
                array.push(data);//插入
            });
