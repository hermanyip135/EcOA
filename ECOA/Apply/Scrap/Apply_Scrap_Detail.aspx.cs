...
public partial class Apply_Scrap_Apply_Scrap_Detail : BasePage
{
    ...
    public bool CanEdit = false;                //是否能够编辑
    ...
    public void LoadPage()
    {
        ...
        da_OfficeAutomation_Document_Scrap_Detail_Inherit.DeleteTemp(MainID); //M_NewTable：20151202 删除临时记录
        string flowState = "";
        try
        {
        ...
        if (EmployeeName == CommonConst.EMP_ADMINISTRATION_OPERATOR_NAME)
        {
            GetAllDepartment();
            this.btnSave.Visible = true;
            SbJs.Append("$(\"#btnSelect\").show();$(\"#DetailAsset\").show();"); //M_NewTable：20151202 //M_AssetAlter：20150827
        }
        ...
        if (dsa.Tables[0].Rows.Count > 0)
        {
            ...
            if (EmployeeName == CommonConst.EMP_ADMINISTRATION_OPERATOR_NAME || EmployeeName == CommonConst.EMP_FINANCE_OPERATOR_NAME || EmployeeName == "源浩灵")
                SbJs.Append("$(\"#SuSpUl\").show();$(\"#DetailAsset\").show();$(\"#DetailAsset\").css(\"margin-top\",\"30px\");"); //M_NewTable：20151202
        }

        #endregion


        ds = da_OfficeAutomation_Document_Scrap_Detail_Inherit.SelectTAsByMainID(MainID, txtDispatchDepartment.Text);
        rptRemainingCosts.DataSource = ds;
        rptRemainingCosts.DataBind();
        rptRemainingCosts.Visible = true;

        string json = AssetMethod.GetScrapAsset(MainID);       //获取detail json
        this.hidDetail.Value = json;

        //陈秀球、源浩灵、申请人，订单还在审理中时
        if ((EmployeeName == CommonConst.EMP_ADMINISTRATION_OPERATOR_NAME || EmployeeName == CommonConst.EMP_FINANCE_OPERATOR_NAME || IsApplicant) && flowState != "3") //M_NewTable：20151202
        {
        ...
        try
        {
            if (ViewState["FLG_ReWrite"].ToString() == "1")
            {
                ...
                SbJs.Append("$(\"#DetailAsset\").hide();$(\"#btnSelect\").show();$(\"#btnAddRow\").hide();$(\"#btnDeleteRow\").hide();$(\"#tbDetail tr[id*=trDetail]\").remove();i = 0;$(\"#SuSpUl\").show();$(\"#lkSpUl\").hide();"); //M_AssetAlter：20150827
                //else
                //    SbJs.Append("$(\"#btnAddRow\").show();$(\"#btnDeleteRow\").show();");
    ...
    public void DrawDetailTable(int n)
    {
        for (int i = 1; i <= n; i++)
        {
            ...
            SbHtml.Append("            使用部门：<input type=\"text\" id=\"txtDept_Name" + i + "\" style=\"width:160px;\" /><br />");
            SbHtml.Append("            财务编号：<input type=\"text\" id=\"txtAssetTag" + i + "\" style=\"width:160px;\" /><br />");
            SbHtml.Append("            归　　属：<input type=\"text\" id=\"txtPlaceRec" + i + "\" style=\"width:160px;\" /><br />");
            SbHtml.Append("            购买时间：<input type=\"text\" id=\"txtBuyDate" + i + "\" style=\"width:160px;\" /><br />");
            SbHtml.Append("    </td>");
            SbHtml.Append("     <td class=\"tl PL10\">");
            SbHtml.Append("            存　放　地　点　：<input type=\"text\" id=\"txtPlace" + i + "\" style=\"width:160px;\" /><br />");
            SbHtml.Append("            报　废　物　品　：<input type=\"text\" id=\"txtAssetName" + i + "\" style=\"width:160px;\" /><br />");
            SbHtml.Append("            规　格　型　号　：<input type=\"text\" id=\"txtModel" + i + "\" style=\"width:160px;\" /><br />");
            SbHtml.Append("            折旧摊分剩余费用：<input type=\"text\" id=\"txtSurplusValue" + i + "\" style=\"width:160px;\" /><br />");
            SbHtml.Append("    </td>");
            SbHtml.Append("</tr>");
        }
        SbJs.Append("i=" + n + ";");
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (IsNewApply)
        {
            ...
            var InsertDetailResult = InsertScrapDetail(t_OfficeAutomation_Document_Scrap.OfficeAutomation_Document_Scrap_ID);
            bool haveITAsset = InsertDetailResult.haveITAsset;
            ...
            //报废特殊流程
            ScrapDefaultFlow(InsertDetailResult);
    ...
    private void ScrapDefaultFlow(InsertScrapDetailBack isdb)
    {
        T_OfficeAutomation_Flow t_OfficeAutomation_Flow = new T_OfficeAutomation_Flow();
        DA_OfficeAutomation_Flow_Inherit da_OfficeAutomation_Flow_Inherit = new DA_OfficeAutomation_Flow_Inherit();
        ...
        if (isdb.haveITAsset)//是否有IT类资产，如果有则添加2步IT流程：第10步IT部维护组主管意见，第12步IT部负责人意见
        {
            var flows = da_OfficeAutomation_Flow_Inherit.SelectByMainIDAndIdx(MainID, 10);
            if (flows == null)
            {
        ...
        if (isdb.totalSurplusValue > 20000)
        {
            //sky 增加审核流程
        ...
        if (Purview.Contains("OA_ITPower_002"))
        {
            try
            {
                ...
                if (isSignSuccess)
                {
                    ...
                    {
                        sdtt = DateTime.Parse(dr["OfficeAutomation_Document_Scrap_Detail_BuyDate"].ToString()).ToString("yyyy-MM-dd");
                        ...
                        mailBody += "<br/>&nbsp;&nbsp;购买时间：" + (sdtt == "1900-01-01" ? "" : sdtt) + "&nbsp;&nbsp;折旧摊分剩余费用：" + dr["OfficeAutomation_Document_Scrap_Detail_SurplusValue"];
                        mailBody += "<br/>";
                    }
                    if (hdIsAgree.Value != "0")//同意或其他意见
                    {
                        ...
                        if (da_OfficeAutomation_Flow_Inherit.IsFlowComplete(MainID))
                        {
                            ...
                            foreach (DataRow dr in dsDetail.Tables[0].Rows)
                            {
                                if (hdIsAgree.Value == "1")
                                {
                                    ...
                                    try
                                    {
                                        AID = dr["OfficeAutomation_Document_Scrap_Detail_AssetAID"].ToString();
                                        LogCommonFunction.AddLog("调用:wsAsset.GetAssetDic(" + AID + ")", "调用");
                                        ws.AssetAdjustmentPass(AID);
                                        LogCommonFunction.AddLog("调用:wsAsset.GetAssetDic(" + AID + ")", "成功");
                                    }
                                    catch(Exception ex)
                                    {
                                        LogCommonFunction.AddLog("调用:wsAsset.GetAssetDic(" + AID + ")", "失败:" + ex.Message);
                                    }
                                }
                            }
                            //审批流程完成，通知申请人
                        ...
                        else
                        {
                            ...
                            if (flowIDx == "13" && hdIsAgree.Value == "2")
                            {
                                da_OfficeAutomation_Flow_Inherit.DeleteByMainIDAndIdxs(MainID, "16");
                                t_OfficeAutomation_Flow.OfficeAutomation_Flow_EmployeeID = CommonConst.EMP_LOGISTICS_MANAGER_ID;
                                t_OfficeAutomation_Flow.OfficeAutomation_Flow_Employee = CommonConst.EMP_LOGISTICS_MANAGER_NAME;
    ...
    private InsertScrapDetailBack Update()
    {
        ...
        var result = InsertScrapDetail(new Guid(ID));

        //特殊流程
        ScrapDefaultFlow(result);
        DeleteDetailXML();     //删除detailXML
    ...
    private InsertScrapDetailBack InsertScrapDetail(Guid gScrapID) //M_NewTable：20151202
    {
        bool haveITAsset = false;
        var result = new InsertScrapDetailBack();
        if (string.IsNullOrEmpty(this.hidDetail.Value))
        {
            throw new Exception("请选择资产");
        }
        ...
        var objlist = new List<T_OfficeAutomation_Document_Scrap_Detail>();
        DA_OfficeAutomation_Document_Scrap_Detail_Inherit da_OfficeAutomation_Document_Scrap_Detail_Inherit = new DA_OfficeAutomation_Document_Scrap_Detail_Inherit();
        DA_OfficeAutomation_Main_Inherit da_OfficeAutomation_Main_Inherit = new DA_OfficeAutomation_Main_Inherit();
        //DataSet ds = new DataSet();
        DataSet dsa = new DataSet();
        ...
        #region 解锁已在数据库的资源
        //解锁已在数据库的资源
        var dsDetail = da_OfficeAutomation_Document_Scrap_Detail_Inherit.SelectByMainID(MainID);
        foreach (DataRow dr in dsDetail.Tables[0].Rows)
        {
            ws.AssetAdjustmentReject(dr["OfficeAutomation_Document_Scrap_Detail_AssetAID"].ToString());
        }
        da_OfficeAutomation_Document_Scrap_Detail_Inherit.Delete(gScrapID.ToString());  //删除已在数据库的资源
        #endregion

        #region 增加新的资源
        var content = this.hidDetail.Value;
        var list = JsonConvert.DeserializeObject<List<CommonEntity.ScrapAsset>>(content);
        foreach (var item in list) //释放新选到的资产
        {
            ws.AssetAdjustmentReject(item.AssetID);
        }
        ...
        foreach (var item in list)
        {
            ...
            t_OfficeAutomation_Document_Scrap_Detail.OfficeAutomation_Document_Scrap_Detail_AssetTag = item.AssetAssetNo;
            t_OfficeAutomation_Document_Scrap_Detail.OfficeAutomation_Document_Scrap_Detail_Model = item.AssetTS;
            t_OfficeAutomation_Document_Scrap_Detail.OfficeAutomation_Document_Scrap_Detail_Number = item.AssetNumber;
            t_OfficeAutomation_Document_Scrap_Detail.OfficeAutomation_Document_Scrap_Detail_AssetName = item.AssetClasses;
            t_OfficeAutomation_Document_Scrap_Detail.OfficeAutomation_Document_Scrap_Detail_BuyDate = string.IsNullOrEmpty(item.AssetRecordedTime) ? DateTime.Parse("1900/1/1") : DateTime.Parse(item.AssetRecordedTime);//string.IsNullOrEmpty(dr["OfficeAutomation_Document_Scrap_Detail_BuyDate"].ToString()) ? DateTime.Parse("1900/1/1") : DateTime.Parse(dr["OfficeAutomation_Document_Scrap_Detail_BuyDate"].ToString());
            t_OfficeAutomation_Document_Scrap_Detail.OfficeAutomation_Document_Scrap_Detail_SurplusValue = item.AssetSurplusValue;
            t_OfficeAutomation_Document_Scrap_Detail.OfficeAutomation_Document_Scrap_Detail_PlaceRec = item.AssetType;

            s = GetAssetPlaceByName(txtDispatchDepartment.Text);
            try
            {
            ...
            t = ws.NewAdjustment(item.AssetAssetNo, int.Parse(s), 1, txtDispatchDepartment.Text, txtApplyDate.Text, 2, SerialNumber); //锁定资产
            if (t.Contains("*"))
            {
                ViewState["AssetWrong"] = t.Replace("*", string.Empty) + "，财务编号为：" + item.AssetAssetNo + "\\r\\n"; //是否有错误
            }
            t_OfficeAutomation_Document_Scrap_Detail.OfficeAutomation_Document_Scrap_Detail_AssetAID = t;

            tempSurplusValue = string.IsNullOrEmpty(item.AssetSurplusValue) ? 0 : decimal.Parse(item.AssetSurplusValue);

            result.maxSurplusValue = result.maxSurplusValue < tempSurplusValue ? tempSurplusValue : result.maxSurplusValue;     //保存最高折旧价
            result.totalSurplusValue += tempSurplusValue;   //所有资产折旧价总和

            objlist.Add(t_OfficeAutomation_Document_Scrap_Detail);
            if (ViewState["AssetWrong"] != "")
            {
                //跳出循环
                break;
            }
        ...
        if (ViewState["AssetWrong"] == "")
        {
            ...
            {
                dsa = da_OfficeAutomation_Main_Inherit.FindAssetsByAssetNo(item.OfficeAutomation_Document_Scrap_Detail_AssetTag.Trim());
                try
                {
        ...
        else
        {
            ...
            foreach (var item in objlist)
            {
                ws.AssetAdjustmentReject(item.OfficeAutomation_Document_Scrap_Detail_AssetAID);
            }
            RunJS("alert(\"" + ViewState["AssetWrong"].ToString() + "请改正后再上传，保存后需编辑前线的审批人员，以免审批流有误。\");$(\"#tbDetail\").html('');history.go(-1);");
        }
    ...
    protected void AtInsertTemporary() //M_NewTable：20151202
    {
        ...
        foreach (DataRow dr in ds.Tables[0].Rows)
        {
            ...
            t_OfficeAutomation_Document_Scrap_Detail.OfficeAutomation_Document_Scrap_Detail_BuyDate = string.IsNullOrEmpty(dr["OfficeAutomation_Document_Scrap_Detail_BuyDate"].ToString()) ? DateTime.Parse("1900/1/1") : DateTime.Parse(dr["OfficeAutomation_Document_Scrap_Detail_BuyDate"].ToString());
    ...
    private void DeleteDetailXML()
    {
        var xmlpath = CommonConst.GetTempXmlPath(MainID);   //临时保存资产的xml文件路径
        if (File.Exists(xmlpath))
        {
            File.Delete(xmlpath);   //删除临时xml
        }
    }
}
