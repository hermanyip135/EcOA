<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Apply_UIReq_Detail.aspx.cs" MasterPageFile="~/MasterPage.master" Inherits="Apply_UIReq_Apply_UIReq_Detail" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" Runat="Server">
    ...
    <script type="text/javascript">
        ...
        var follower=$('#<%=txtFollower.ClientID %>').val();
                if(idx=='4' && $.trim(follower)!=''){
                    $.ajax({
                        ...
                        type: "post",
                        dataType: "text",
                        data: "action=saveuireqfollower&sysreqid=<%=ID %>&follower=" + follower+"&apply=" + $.trim($("#<%=txtApplicant.ClientID %>").val()),
                        success: function() {}
                    })
                }
        ...
        function editflow(){
            ...
            if(win=='success')
                window.location="Apply_UIReq_Detail.aspx?MainID=<%=MainID %>";
        }
        ...
        function savePlanTime() {
            ...
            { 
                $.ajax({
                    ...
                    type: "post",
                    dataType: "text",
                    data: "action=saveuireqplantime&sysreqid=<%=ID %>&plantime=" + plantime + "&follower=<%=EmployeeName %>&apply=" + $.trim($("#<%=txtApplicant.ClientID %>").val()),
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    <div style='text-align:center; width:640px; margin:0px auto;'>
        <div class="noprint">
        <%=sbFlow.ToString() %>
        <asp:button runat="server" id="btnEditFlow2" text="编辑流程1" onclientclick="if(confirm('修改后，所修改环节的后续流程都必须重审，确定要修改吗？'))return true;else return false;" OnClick="btnEditFlow_DoClick" Visible="False" />
        </div>
            <!--打印正文开始-->
            ...
            <table id="tbAround"  width='640px'>
                ...
                <tr>
                    ...
                    <td colspan="3" class="tl PL10">
                        ...
                        <textarea id="txtIDx8" rows="4" style="width:98%; overflow:auto;"></textarea>
                        <input type="button" id="btnSignIDx8" value="签署意见" onclick="sign('8')" style="display:none;"/>
                        <div class="signdate">日期：<span id="spanDateIDx8">_________</span></div></td>
                </tr>	
