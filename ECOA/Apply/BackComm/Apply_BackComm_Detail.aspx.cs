...
public partial class Apply_BackComm_Apply_BackComm_Detail : BasePage
{
    #region 变量声明及定义
    bool tpdf = false;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            try
            {
                if (Session["FLG_ReWrite2"].ToString() == "1")
                {
                    ViewState["FLG_ReWrite"] = "1";
                    Session["FLG_ReWrite2"] = null;
                }
            }
            catch
            {
    ...
    public void LoadPage()
    {
        ...
        DA_OfficeAutomation_Document_BackComm_Detail_Inherit da_OfficeAutomation_Document_BackComm_Detail_Inherit = new DataAccess.Operate.DA_OfficeAutomation_Document_BackComm_Detail_Inherit();
        ...
        DataSet backComm = new DataSet();
        DataSet backComm_Detail = new DataSet();
        ...
        {
            //从暂存xml中读取
            var obj = new Common().GetTempSaveEntity<DataEntity.PageModel.Apply_BackComm_Detail>("BackCommData", "", Mainobj.OfficeAutomation_SerialNumber);
            backComm = Common.GetPageDetailDS<T_OfficeAutomation_Document_BackComm>(obj.BackComm, Mainobj);
            backComm_Detail = Common.GetDataSet<T_OfficeAutomation_Document_BackComm_Detail>(obj.BackComm_Detail);
            IsTempSave = true;
        }
        else
        {
            ...
            //从数据库中读取
            backComm = da_OfficeAutomation_Document_BackComm_Inherit.SelectByMainID(MainID);
            ID = backComm.Tables[0].Rows[0]["OfficeAutomation_Document_BackComm_ID"].ToString();
            backComm_Detail = da_OfficeAutomation_Document_BackComm_Detail_Inherit.SelectByID(ID);
        ...
        ID = dr["OfficeAutomation_Document_BackComm_ID"].ToString();
        string applicant = dr["OfficeAutomation_Document_BackComm_Apply"].ToString();
        ApplyN = applicant;
        txtApplyID.Text = dr["OfficeAutomation_Document_BackComm_ApplyID"].ToString();
        txtDepartment.Text = dr["OfficeAutomation_Document_BackComm_Department"].ToString();
        hdDepartmentID.Value = txtDepartment.Text;
        lblApply.Text = applicant;
        lblApplyDate.Text = DateTime.Parse(dr["OfficeAutomation_Document_BackComm_ApplyDate"].ToString()).ToString("yyyy-MM-dd");
        txtBuilding.Text = dr["OfficeAutomation_Document_BackComm_Building"].ToString();
        txtReplyPhone.Text = dr["OfficeAutomation_Document_BackComm_ReplyPhone"].ToString();
        txtReason.Text = dr["OfficeAutomation_Document_BackComm_Reason"].ToString();
        txtMoneyCount.Text = dr["OfficeAutomation_Document_BackComm_MoneyCount"].ToString();
        txtMeasure.Text = dr["OfficeAutomation_Document_BackComm_Measure"].ToString();
        SerialNumber = dr["OfficeAutomation_SerialNumber"].ToString();

        //ds = da_OfficeAutomation_Document_BackComm_Detail_Inherit.SelectByID(ID);
        ds = backComm_Detail;
        if (ds != null)
        {
            ...
            else
            {
                ...
                for (int n = 0; n < detailCount; n++)
                {
                    ...
                    SbJs.Append("$('#txtDepartment" + i + "').val('" + dr["OfficeAutomation_Document_BackComm_Detail_Department"] + "');");
                    SbJs.Append("$('#txtSales" + i + "').val('" + dr["OfficeAutomation_Document_BackComm_Detail_Sales"] + "');");
                    SbJs.Append("$('#txtTeam" + i + "').val('" + dr["OfficeAutomation_Document_BackComm_Detail_Team"] + "');");
                    SbJs.Append("$('#txtBackMoney" + i + "').val('" + dr["OfficeAutomation_Document_BackComm_Detail_BackMoney"] + "');");
                    SbJs.Append("$('#rdoIsOnJob" + i + (dr["OfficeAutomation_Document_BackComm_Detail_IsOnJob"].ToString() == "True" ? "1" : "0") + "').attr('checked','checked');");
                    SbJs.Append("$('#rdoIsPayComm" + i + (dr["OfficeAutomation_Document_BackComm_Detail_IsPayComm"].ToString() == "True" ? "1" : "0") + "').attr('checked','checked');");
                }
            }
        }
        else
        ...
        if (DateTime.Parse(lblApplyDate.Text) > DateTime.Parse("2015-08-13 11:35:00.000"))
        {
            ...
            if (flowsc != null)
                if (flowsc.OfficeAutomation_Flow_EmployeeID.Contains("0006") || flowsc.OfficeAutomation_Flow_EmployeeID.Contains("32129"))
                {
            ...
            if (decimal.Parse(txtMoneyCount.Text) > 20000 && flowState == "1")
            {
                flowsd = da_OfficeAutomation_Flow_Inherit.SelectByMainIDAndIdx(MainID, 11);
                if (flowsd == null)
                {
        ...
        //if ((EmployeeID == CommonConst.EMP_FINANCE_PUBLIC_ID || EmployeeID == "16945" || EmployeeID == "20208" || EmployeeID == "43781") && flowState == "3")
        //2019-10-21 财务部将 胡雅丝（16945） 权限移交比 钟惠贤（43781）
        if ((EmployeeID == CommonConst.EMP_FINANCE_PUBLIC_ID || EmployeeID == "43781" || EmployeeID == "20208" || EmployeeID == "43781") && flowState == "3")
        ...
        for (int i = 0; i < drc.Count; i++)
        {
            ...
            if (drc[i]["OfficeAutomation_Flow_Audit"].ToString() == "True")
            {
                SbJs.Append("$('#btnSignIDx" + drc[i]["OfficeAutomation_Flow_IDx"] + "').before('<div style=\"width: 150px; line-height: 65px; height: 2px;  margin-left:0px; float:left;\">"
                    + drc[i]["OfficeAutomation_Flow_Auditor"] + "</div>");//789
        ...
        try
        {
            if ((flowso.OfficeAutomation_Flow_EmployeeID.Contains("0001")
                    && flowso.OfficeAutomation_Flow_Employee.Contains("黄轩明")))
            {
    ...
    public void DrawDetailTable(int n)
    {
        ...
        for (int i = 1; i <= n; i++)
        {
            ...
            SbHtml.Append("     <td><input type=\"text\" id=\"txtDepartment" + i + "\" style=\"width:90%\"/><input type=\"hidden\" id=\"hdDepartmentID" + i + "\" /></td>");
            SbHtml.Append("     <td><input type=\"text\" id=\"txtSales" + i + "\" style=\"width:90%\"/></td>");
            SbHtml.Append("     <td><input type=\"text\" id=\"txtTeam" + i + "\" style=\"width:90%\"/></td>");
            SbHtml.Append("     <td><input type=\"text\" id=\"txtBackMoney" + i + "\" style=\"width:90%\" onkeyup=\"this.value=this.value.replace(/[^\\d|//.]/g,'');\"/></td>");
            SbHtml.Append("    <td>");
            SbHtml.Append("        <input type=\"radio\" id='rdoIsOnJob" + i + "1' name='IsOnJob" + i + "' /><label for='rdoIsOnJob" + i + "1'>是</label><input type=\"radio\" id='rdoIsOnJob" + i + "0' name='IsOnJob" + i + "' /><label for='rdoIsOnJob" + i + "0'>否</label>");
            SbHtml.Append("    </td>");
            SbHtml.Append("    <td>");
            SbHtml.Append("        <input type=\"radio\" id='rdoIsPayComm" + i + "1' name='IsPayComm" + i + "' /><label for='rdoIsPayComm" + i + "1'>是</label><input type=\"radio\" id='rdoIsPayComm" + i + "0' name='IsPayComm" + i + "' /><label for='rdoIsPayComm" + i + "0'>否</label>");
            SbHtml.Append("    </td>");
            SbHtml.Append("</tr>");
        }
        SbJs.Append("i=" + n + ";");
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        T_OfficeAutomation_Document_BackComm t_OfficeAutomation_Document_BackComm = new T_OfficeAutomation_Document_BackComm();
        T_OfficeAutomation_Flow t_OfficeAutomation_Flow = new T_OfficeAutomation_Flow();
        ...
        try
        {
            if (MainID == "")
            {
                ...
                t_OfficeAutomation_Main.OfficeAutomation_SerialNumber = "BackComm" + DateTime.Now.ToString("yyyyMMddHHmmssfffffff");
                t_OfficeAutomation_Main.OfficeAutomation_DocumentID = 11;
                ...
                t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_ID = Guid.NewGuid();
                t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_MainID = t_OfficeAutomation_Main.OfficeAutomation_Main_ID;
                t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_Apply = EmployeeName;
                t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_ApplyDate = DateTime.Now;
                t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_ApplyID = txtApplyID.Text;
                t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_Department = txtDepartment.Text;
                t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_Building = txtBuilding.Text;
                t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_ReplyPhone = txtReplyPhone.Text;
                t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_Reason = txtReason.Text;
                t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_MoneyCount = Decimal.Parse(txtMoneyCount.Text);
                t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_Measure = txtMeasure.Text;
                ...
                da_OfficeAutomation_Document_BackComm_Inherit.Insert(t_OfficeAutomation_Document_BackComm);//插入申请表

                InsertBackCommDetail(t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_ID);
                ...
                if (!"fail".Equals(sHRTree[0].Split(',')[0]))
                {
                ...
                if (t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_MoneyCount > 20000)
                {
                ...
                Common.AddLog(EmployeeID, EmployeeName, 15, new Guid(MainID), 1);//日志，创建申请表
                Common.AddLog(EmployeeID, EmployeeName, 3, new Guid(MainID), 1);//日志，创建流程

                RunJS("window.showModalDialog(\"/Apply/Apply_UploadFile.aspx?MainID=" + MainID + "\", '" + MainID + "', \"dialogHeight=165px\");var win=window.showModalDialog(\"Apply_BackComm_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=800px;dialogHeight=250px\");if(win=='success'){window.location='/Apply/Apply_Search.aspx'; }");
                #endregion
            }
            else
            {
                ...
                if (Purview.Contains("OA_ITPower_001"))//是否有编辑权限
                {
                    ...
                    if (tempsave)
                    {
                        ...
                        #endregion
                        RunJS("window.showModalDialog(\"/Apply/Apply_UploadFile.aspx?MainID=" + MainID + "\", '" + MainID + "', \"dialogHeight=165px\");var win=window.showModalDialog(\"Apply_BackComm_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=800px;dialogHeight=320px\");if(win=='success'){window.location='/Apply/Apply_Search.aspx'; }");
                    }
                    else
                    {
    ...
    {
        var SerialNumber = "BackComm" + DateTime.Now.ToString("yyyyMMddHHmmssfffff");
        var DocumentID = 11;
        ...
        {
            var BackComm_ID = ds.Tables[0].Rows[0]["OfficeAutomation_Document_BackComm_ID"].ToString();
            t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_ID = new Guid(BackComm_ID);
        }
        //t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_ID = Guid.NewGuid();
        t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_MainID = t_OfficeAutomation_Main.OfficeAutomation_Main_ID;
        t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_Apply = EmployeeName;
        t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_ApplyDate = DateTime.Now;
        t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_ApplyID = txtApplyID.Text == null ? "" : txtApplyID.Text;
        t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_Department = txtDepartment.Text == null ? "" : txtDepartment.Text;
        t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_Building = txtBuilding.Text == null ? "" : txtBuilding.Text;
        t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_ReplyPhone = txtReplyPhone.Text == null ? "" : txtReplyPhone.Text;
        t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_Reason = txtReason.Text == null ? "" : txtReason.Text;
        t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_MoneyCount = Decimal.Parse((txtMoneyCount.Text == null || txtMoneyCount.Text == "") ? "0" : txtMoneyCount.Text);
        t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_Measure = txtMeasure.Text == null ? "" : txtMeasure.Text;
        #endregion
        da_OfficeAutomation_Document_BackComm_Inherit.HandleBase(t_OfficeAutomation_Document_BackComm);//插入当前申请表关键内容
        var obj = new DataEntity.PageModel.Apply_BackComm_Detail();
        obj.MainEntity = t_OfficeAutomation_Main;
        obj.BackComm = t_OfficeAutomation_Document_BackComm;
        obj.BackComm_Detail = GetBackCommDetail(t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_ID);
        //写入xml
        var result = new Common().SaveTempSaveFile<DataEntity.PageModel.Apply_BackComm_Detail>(obj, "BackCommData", "", t_OfficeAutomation_Main.OfficeAutomation_SerialNumber);
        if (result)
        {
    ...
    protected void btnSign_Click(object sender, EventArgs e)
    {
        ...
        DA_OfficeAutomation_Document_BackComm_Inherit da_OfficeAutomation_Document_BackComm_Inherit = new DA_OfficeAutomation_Document_BackComm_Inherit();
        DA_OfficeAutomation_Document_BackComm_Detail_Inherit da_OfficeAutomation_Document_BackComm_Detail_Inherit = new DA_OfficeAutomation_Document_BackComm_Detail_Inherit();

        DataSet ds = da_OfficeAutomation_Document_BackComm_Inherit.SelectByMainID(MainID);
        DataRow drRow = ds.Tables[0].Rows[0];
        ID = drRow["OfficeAutomation_Document_BackComm_ID"].ToString();
        ...
        if (Purview.Contains("OA_ITPower_002"))
        {
            try
            {
                ...
                if (isSignSuccess)
                {
                    ...
                    string apply = ds.Tables[0].Rows[0]["OfficeAutomation_Document_BackComm_Apply"].ToString();
                    string employname;
                    string serialNumber = drRow["OfficeAutomation_SerialNumber"].ToString();
                    string documentName = da_OfficeAutomation_Flow_Inherit.GetDocumentNameByMainID(MainID);
                    string department = drRow["OfficeAutomation_Document_BackComm_Department"].ToString();
                    ...
                    sbMailBody.Append("<br/><br/>申请人：" + drRow["OfficeAutomation_Document_BackComm_Apply"]);
                    sbMailBody.Append("<br/>申请部门：" + drRow["OfficeAutomation_Document_BackComm_Department"]);
                    sbMailBody.Append("<br/>申请日期：" + DateTime.Parse(drRow["OfficeAutomation_Document_BackComm_ApplyDate"].ToString()).ToString("yyyy-MM-dd"));
                    sbMailBody.Append("<br/>文件主题：关于" + drRow["OfficeAutomation_Document_BackComm_Building"].ToString() + "的退佣申请");
                    sbMailBody.Append("<br/>回复电话：020-" + drRow["OfficeAutomation_Document_BackComm_ReplyPhone"]);
                    sbMailBody.Append("<br/>退佣原因：" + drRow["OfficeAutomation_Document_BackComm_Reason"]);
                    sbMailBody.Append("<br/>退佣交易涉及各级营业员的在职及佣金情况：");
                    ds = da_OfficeAutomation_Document_BackComm_Detail_Inherit.SelectByMainID(MainID);
                    foreach (DataRow dr in ds.Tables[0].Rows)
                    {
                        sbMailBody.Append("<br/>分行/组别：" + dr["OfficeAutomation_Document_BackComm_Detail_Department"]);
                        sbMailBody.Append("<br/>营业人员：" + dr["OfficeAutomation_Document_BackComm_Detail_Sales"]);
                        sbMailBody.Append("<br/>各级主管姓名：" + dr["OfficeAutomation_Document_BackComm_Detail_Team"]);
                        sbMailBody.Append("<br/>退佣金额：" + dr["OfficeAutomation_Document_BackComm_Detail_BackMoney"]);
                        sbMailBody.Append("<br/>是否在职：" + (dr["OfficeAutomation_Document_BackComm_Detail_Department"].ToString() == "True" ? "是" : "否"));
                        sbMailBody.Append("<br/>是否已出佣：" + (dr["OfficeAutomation_Document_BackComm_Detail_Department"].ToString() == "True" ? "是" : "否"));
                        sbMailBody.Append("<br/>");
                    }

                    sbMailBody.Append("<br/>合计退佣金额：" + drRow["OfficeAutomation_Document_BackComm_MoneyCount"]);
                    sbMailBody.Append("<br/>内部处理方案：" + drRow["OfficeAutomation_Document_BackComm_Measure"]);
    ...
    protected void btnSignSave_Click(object sender, EventArgs e)
    {
        ...
        {
            DA_OfficeAutomation_Document_BackComm_Inherit da_OfficeAutomation_Document_BackComm_Inherit = new DA_OfficeAutomation_Document_BackComm_Inherit();
            //if (EmployeeID == CommonConst.EMP_FINANCE_PUBLIC_ID || EmployeeID == "16945" || EmployeeID == "20208" || EmployeeID == "43781")
            //2019-10-21 财务部将 胡雅丝（16945） 权限移交比 钟惠贤（43781）
            if (EmployeeID == CommonConst.EMP_FINANCE_PUBLIC_ID || EmployeeID == "43781" || EmployeeID == "20208" || EmployeeID == "43781")
                da_OfficeAutomation_Document_BackComm_Inherit.AddRemarkByID_II(MainID, CommonConst.SIGN_FINANCE);
            Alert("标记成功。");
        }
        catch
        {
    ...
    private void Update()
    {
        ...
        DataSet ds = new DataSet();
        ds = da_OfficeAutomation_Document_BackComm_Inherit.SelectByMainID(MainID);
        ID = ds.Tables[0].Rows[0]["OfficeAutomation_Document_BackComm_ID"].ToString();

        t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_ID = new Guid(ID);
        t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_MainID = new Guid(MainID);
        t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_Apply = EmployeeName;
        t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_ApplyDate = DateTime.Now;
        t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_ApplyID = txtApplyID.Text;
        t_OfficeAutomation_Document_BackComm.OfficeAutomation_Document_BackComm_Department = txtDepartment.Text;
        ...
        da_OfficeAutomation_Document_BackComm_Inherit.Update(t_OfficeAutomation_Document_BackComm);//修改申请表

        DA_OfficeAutomation_Document_BackComm_Detail_Inherit da_OfficeAutomation_Document_BackComm_Detail_Inherit = new DA_OfficeAutomation_Document_BackComm_Detail_Inherit();
        da_OfficeAutomation_Document_BackComm_Detail_Inherit.Delete(ID);
        InsertBackCommDetail(new Guid(ID));
        ...
        {
            da_OfficeAutomation_Flow_Inherit.DeleteByMainIDAndIdxs(MainID, "9,10,11");
        }


        Common.AddLog(EmployeeID, EmployeeName, 15, new Guid(MainID), 2);//日志，修改申请表
    }
    ...
    /// 新增退佣申请明细
    /// </summary>
    /// <param name="gBackCommID"></param>
    private void InsertBackCommDetail(Guid gBackCommID)
    {
        if (hdDetail.Value == "")
            return;

        T_OfficeAutomation_Document_BackComm_Detail t_OfficeAutomation_Document_BackComm_Detail;
        DA_OfficeAutomation_Document_BackComm_Detail_Inherit da_OfficeAutomation_Document_BackComm_Detail_Inherit = new DA_OfficeAutomation_Document_BackComm_Detail_Inherit();
        ...
        for (int i = 0; i < details.Length; i++)
        {
            ...
            t_OfficeAutomation_Document_BackComm_Detail = new T_OfficeAutomation_Document_BackComm_Detail();
            t_OfficeAutomation_Document_BackComm_Detail.OfficeAutomation_Document_BackComm_Detail_ID = Guid.NewGuid();
            t_OfficeAutomation_Document_BackComm_Detail.OfficeAutomation_Document_BackComm_Detail_MainID = gBackCommID;
            t_OfficeAutomation_Document_BackComm_Detail.OfficeAutomation_Document_BackComm_Detail_Department = detail[0];
            t_OfficeAutomation_Document_BackComm_Detail.OfficeAutomation_Document_BackComm_Detail_Sales = detail[1];
            t_OfficeAutomation_Document_BackComm_Detail.OfficeAutomation_Document_BackComm_Detail_Team = detail[2];
            t_OfficeAutomation_Document_BackComm_Detail.OfficeAutomation_Document_BackComm_Detail_BackMoney = Decimal.Parse(detail[3]);
            t_OfficeAutomation_Document_BackComm_Detail.OfficeAutomation_Document_BackComm_Detail_IsOnJob = detail[4] == "1";
            t_OfficeAutomation_Document_BackComm_Detail.OfficeAutomation_Document_BackComm_Detail_IsPayComm = detail[5] == "1";

            da_OfficeAutomation_Document_BackComm_Detail_Inherit.Insert(t_OfficeAutomation_Document_BackComm_Detail);
        }
    }

    //获取子表临时数据
    private List<T_OfficeAutomation_Document_BackComm_Detail> GetBackCommDetail(Guid gBackCommID)
    {
        if (hdDetail.Value == "")
            return null;
        T_OfficeAutomation_Document_BackComm_Detail t_OfficeAutomation_Document_BackComm_Detail;
        var list = new List<T_OfficeAutomation_Document_BackComm_Detail>();
        string[] details = Regex.Split(hdDetail.Value, "\\|\\|");
        for (int i = 0; i < details.Length; i++)
        {
            string[] detail = Regex.Split(details[i], "\\&\\&");
            ...
            t_OfficeAutomation_Document_BackComm_Detail.OfficeAutomation_Document_BackComm_Detail_Sales = detail[1];
            t_OfficeAutomation_Document_BackComm_Detail.OfficeAutomation_Document_BackComm_Detail_Team = detail[2];
            t_OfficeAutomation_Document_BackComm_Detail.OfficeAutomation_Document_BackComm_Detail_BackMoney = Decimal.Parse(detail[3] == "" ? "0" : detail[3]);
            t_OfficeAutomation_Document_BackComm_Detail.OfficeAutomation_Document_BackComm_Detail_IsOnJob = detail[4] == "1";
            t_OfficeAutomation_Document_BackComm_Detail.OfficeAutomation_Document_BackComm_Detail_IsPayComm = detail[5] == "1";

            list.Add(t_OfficeAutomation_Document_BackComm_Detail);
        }
        return list;
    }
    #endregion
    ...
    protected void btnReWrite_Click(object sender, EventArgs e)
    {
        ...
        Session["FLG_ReWrite2"] = "1";
        Response.Write("<script>window.open('Apply_BackComm_Detail.aspx?MainID=" + MainID + "');</script>");
    }
    protected void btnSPDF_Click(object sender, EventArgs e) //M_PDF
    {
        try
        {
            ...
            Response.AddHeader("content-disposition", "attachment; filename=" + HttpUtility.UrlEncode("退佣申请表.pdf"));//强制下载 
    ...
    protected void btnSaveLogisticsFlow_Click(object sender, EventArgs e)
    {
        ...
        DataSet ds = new DataSet();
        ds = da_OfficeAutomation_Document_BackComm_Inherit.SelectByMainID(MainID);
        ID = ds.Tables[0].Rows[0]["OfficeAutomation_Document_BackComm_ID"].ToString(); //在不同的表要注意修改
    ...
    protected void btnCancelSign_Click(object sender, EventArgs e) //20141202：CancelSign
    {
        try
        {
            ...
            string signEmployeeName = EmployeeName;
            DA_OfficeAutomation_Document_BackComm_Inherit da_OfficeAutomation_Document_BackComm_Inherit = new DA_OfficeAutomation_Document_BackComm_Inherit();
            DataSet ds = da_OfficeAutomation_Document_BackComm_Inherit.SelectByMainID(MainID); //在不同的表中要修改
            DataRow drRow = ds.Tables[0].Rows[0];
            string apply = ds.Tables[0].Rows[0]["OfficeAutomation_Document_BackComm_Apply"].ToString();
            string employname;
            string serialNumber = drRow["OfficeAutomation_SerialNumber"].ToString();
    ...
    protected void btnEditFlow_DoClick(object sender, EventArgs e)
    {
        try
        {
            ...
            //在不同的表中要修改
            RunJS("var win=window.showModalDialog(\"Apply_BackComm_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=830px;dialogHeight=280px\");if(win=='success'){window.location='/Ajax/Editor_Flow.ashx?MainID=" + MainID + "&applyUrl=" + applyUrl + "&apply=" + apply + "&serialNumber=" + serialNumber + "&department=" + department + "&Jhref=" + Page.Request.Url + "&mid=" + EmployeeID + "'; }");
    ...
    {
        DA_OfficeAutomation_Flow_Inherit da_OfficeAutomation_Flow_Inherit = new DA_OfficeAutomation_Flow_Inherit();
        DA_OfficeAutomation_Document_BackComm_Inherit da_OfficeAutomation_Document_BadDebts_Inherit = new DA_OfficeAutomation_Document_BackComm_Inherit();
