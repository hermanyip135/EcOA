...
public partial class Apply_OpenProve_Apply_OpenProve_Detail : BasePage
{
    #region 变量声明及定义
    bool tpdf = false;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            try
            {
                if (Session["FLG_ReWrite76"].ToString() == "1")
                {
                    ViewState["FLG_ReWrite"] = "1";
                    Session["FLG_ReWrite76"] = null;
                }
            }
            catch
            {
    ...
    public void NewPage()
    {
        ...
        SbJs.Append("<script type=\"text/javascript\">$(\"#summaryLeft\").show();$(\"#summaryRight\").show();" + ApplyDisplayPart);
        //DrawDetailTable(0);
    ...
    {
        this.txtName.Attributes.Add("readonly", "true");
        this.txtEnterDate.Attributes.Add("readonly", "true");
        this.txtDepartment.Attributes.Add("readonly", "true");
        this.txtPosition.Attributes.Add("readonly", "true");
      //  this.txtRank.Attributes.Add("readonly", "true");
    }
    ...
    public void LoadPage()
    {
        ...
        DA_OfficeAutomation_Document_OpenProve_Detail_Inherit da_OfficeAutomation_Document_OpenProve_Detail_Inherit = new DataAccess.Operate.DA_OfficeAutomation_Document_OpenProve_Detail_Inherit();
        ...
        ds = da_OfficeAutomation_Document_OpenProve_Inherit.SelectByMainID(MainID);
        DataRow dr = ds.Tables[0].Rows[0];
        ID = dr["OfficeAutomation_Document_OpenProve_ID"].ToString();
        string applicant = dr["OfficeAutomation_Document_OpenProve_Apply"].ToString();
        ApplyN = applicant;
        txtDepartment.Text = dr["OfficeAutomation_Document_OpenProve_Department"].ToString();
        lblApply.Text = applicant;
        lblApplyDate.Text = DateTime.Parse(dr["OfficeAutomation_Document_OpenProve_ApplyDate"].ToString()).ToString("yyyy-MM-dd");
        txtCode.Text = dr["OfficeAutomation_Document_OpenProve_Code"].ToString();
        txtEnterDate.Text = dr["OfficeAutomation_Document_OpenProve_EnterDate"].ToString();
        txtName.Text = dr["OfficeAutomation_Document_OpenProve_Name"].ToString();
        txtPhone.Text = dr["OfficeAutomation_Document_OpenProve_Phone"].ToString();//联系电话
        txtSignAddress.Text = dr["OfficeAutomation_Document_OpenProve_SignAddress"].ToString();//签收地址
        txtSignPhone.Text = dr["OfficeAutomation_Document_OpenProve_SignPhone"].ToString();//签收电话
        txtRecipient.Text = dr["OfficeAutomation_Document_OpenProve_Recipient"].ToString();//签收人
        hiSignMode.Value = dr["OfficeAutomation_Document_OpenProve_SignMode"].ToString();//签收方式
        ExceptionalCase.Value = dr["OfficeAutomation_Document_OpenProve_ExceptionalCase"].ToString();
       // txtRank.Text = dr["OfficeAutomation_Document_OpenProve_Rank"].ToString();
        txtPosition.Text = dr["OfficeAutomation_Document_OpenProve_Position"].ToString();

        SerialNumber = dr["OfficeAutomation_SerialNumber"].ToString();

        ds = da_OfficeAutomation_Document_OpenProve_Detail_Inherit.SelectByID(ID);
        ...
        else
        {
            ...
            for (int n = 0; n < detailCount; n++)
            {
                ...
                SbJs.Append("$('#ddlProve" + i + "').val('" + dr["OfficeAutomation_Document_OpenProve_Detail_Prove"].ToString().Trim() + "');");
                SbJs.Append("$('#txtDetail_No" + i + "').val('" + dr["OfficeAutomation_Document_OpenProve_Detail_No"].ToString().Trim() + "');");
                //SbJs.Append("$('#rdoReasondb" + i + (dr["OfficeAutomation_Document_OpenProve_Detail_Reasondb"].ToString().Trim() == "1" ? "1" : "0") + "').attr('checked','checked');");
                SbJs.Append("$('#rdoReasondb" + i + (dr["OfficeAutomation_Document_OpenProve_Detail_Reasondb"].ToString().Trim()) + "').attr('checked','checked');");
                SbJs.Append("$('#txtDetail_Address" + i + "').val('" + dr["OfficeAutomation_Document_OpenProve_Detail_Address"].ToString().Trim() + "');");
                SbJs.Append("$('#txtDetail_Sdate" + i + "').val('" + dr["OfficeAutomation_Document_OpenProve_Detail_Sdate"].ToString().Trim() + "');");
                SbJs.Append("$('#txtDetail_Edate" + i + "').val('" + dr["OfficeAutomation_Document_OpenProve_Detail_Edate"].ToString().Trim() + "');");
                SbJs.Append("$('#txtDetail_AnotherR" + i + "').val('" + dr["OfficeAutomation_Document_OpenProve_Detail_AnotherR"].ToString().Trim().Replace("\n", "").Replace("\r", "") + "');");
                SbJs.Append("$('#rdoKind" + i + (dr["OfficeAutomation_Document_OpenProve_Detail_Kind"].ToString().Trim() == "1" ? "1" : dr["OfficeAutomation_Document_OpenProve_Detail_Kind"].ToString().Trim() == "2" ? "2" : dr["OfficeAutomation_Document_OpenProve_Detail_Kind"].ToString().Trim() == "3" ? "3" : dr["OfficeAutomation_Document_OpenProve_Detail_Kind"].ToString().Trim() == "4" ? "4" : "0") + "').attr('checked','checked');");
                SbJs.Append("$('#txtDetail_AnotherKind" + i + "').val('" + dr["OfficeAutomation_Document_OpenProve_Detail_AnotherKind"].ToString().Trim().Replace("\n", "").Replace("\r", "") + "');");
            }
        }
        #endregion
        ...
        for (int i = 0; i < drc.Count; i++)
        {
            ...
            if (drc[i]["OfficeAutomation_Flow_Audit"].ToString() == "True")
            {
                ...
                else if (drc[i]["OfficeAutomation_Flow_IsAgree"].ToString() == "1")
                    SbJs.Append("$('#rdbYesIDx" + drc[i]["OfficeAutomation_Flow_IDx"] + "').attr('checked','checked');");
                else if (drc[i]["OfficeAutomation_Flow_IsAgree"].ToString() == "100")
                    SbJs.Append("$('#rdbYes1IDx" + drc[i]["OfficeAutomation_Flow_IDx"] + "').attr('checked','checked');");
        ...
        if (flows == null)
        {
            SbJs.Append("$('#trManager1').hide();$('#trManager2').hide();$('#trManager3').hide();$('#btnEditFlow').hide();");
            btnSAlterC.Visible = false;
        }
    ...
    public void DrawDetailTable(int n)
    {
        ...
        for (int i = 1; i <= n; i++)
        {
            //SbHtml.Append("<tr class=\"noborder\" id=\"trDetail" + i + "\">");
            //SbHtml.Append("<td style=\"line-height: 15px;\">");
            //SbHtml.Append("<table style=\"line-height: 25px; border-collapse: collapse;\" width=\"100%\">");
            //SbHtml.Append("<tr class=\"noborder\"><td style=\"width:120px\">* 申请证明类别</td><td style=\"text-align: left; padding-left: 7px\"><select id=\"ddlProve" + i + "\" style=\"width:150px\"><option>请选择</option><option>在职证明</option><option>收入证明</option><option>在职收入证明</option><option>社保证明</option><option>公积金证明</option><option>社保公积金证明</option></select></td><td style=\"width:70px\">* 份数</td><td style=\"text-align: left; padding-left: 7px\"><input type=\"text\" id=\"txtDetail_No" + i + "\"/></td></tr>");
            //SbHtml.Append("<tr class=\"noborder\"><td rowspan=\"2\">﹡开具原因</td><td colspan=\"3\" class=\"tl PL10\"><input type=\"radio\" id=\"rdoReasondb" + i + "1\" name=\"Reasondb" + i + "\" /><label for=\"rdoReasondb" + i + "1\">办理签证：</label>出游地点<input type=\"text\" id=\"txtDetail_Address" + i + "\" style=\"width: 43%\"/><br/>&nbsp;&nbsp;出游时间<input type=\"text\" id=\"txtDetail_Sdate" + i + "\" style=\"width: 80px\"/>至<input type=\"text\" id=\"txtDetail_Edate" + i + "\" style=\"width: 80px\"/></td></tr>");
            //SbHtml.Append("<tr class=\"noborder\"><td colspan=\"3\" class=\"tl PL10\"><input type=\"radio\" id=\"rdoReasondb" + i + "0\" name=\"Reasondb" + i + "\" /><label for=\"rdoReasondb" + i + "0\">其他原因</label><input type=\"text\" id=\"txtDetail_AnotherR" + i + "\" style=\"width: 83.5%\"/></td></tr>");
            //SbHtml.Append("<tr class=\"noborder\"><td>* 收入证明金额类型</td><td colspan=\"3\" class=\"tl PL10\"><input type=\"radio\" id=\"rdoKind" + i + "1\" name=\"Kind" + i + "\" /><label for=\"rdoKind" + i + "1\">月均税前收入</label><input type=\"radio\" id=\"rdoKind" + i + "2\" name=\"Kind" + i + "\" /><label for=\"rdoKind" + i + "2\">月均税后收入</label><input type=\"radio\" id=\"rdoKind" + i + "3\" name=\"Kind" + i + "\" /><label for=\"rdoKind" + i + "3\">其他要求</label><input type=\"text\" id=\"txtDetail_AnotherKind" + i + "\" style=\"width: 47%\"/></td></tr>");
            //SbHtml.Append("</table></td></tr>");
            SbHtml.Append("<tr class=\"noborder\" id=\"trDetail" + i + "\">");
            SbHtml.Append("<td style=\"line-height: 15px;\">");
            SbHtml.Append("<table style=\"line-height: 25px; border-collapse: collapse;\" width=\"100%\">");
            SbHtml.Append("<tr class=\"noborder\"><td style=\"width:120px\">* 申请证明类别</td><td style=\"text-align: left; padding-left: 7px\"><select id=\"ddlProve" + i + "\" style=\"width:150px\"><option>请选择</option><option>在职证明</option><option>在职收入证明</option><option>社保证明</option><option>公积金证明</option></select></td><td style=\"width:70px\">* 份数</td><td style=\"text-align: left; padding-left: 7px\"><input type=\"text\" id=\"txtDetail_No" + i + "\"/></td></tr>");
            SbHtml.Append("<tr class=\"noborder\"><td rowspan=\"4\">﹡开具原因</td><td colspan=\"3\" class=\"tl PL10\"><input type=\"radio\" id=\"rdoReasondb" + i + "1\" name=\"Reasondb" + i + "\" /><label for=\"rdoReasondb" + i + "1\">办理签证：</label>出游地点<input type=\"text\" id=\"txtDetail_Address" + i + "\" style=\"width: 43%\"/><br/>&nbsp;&nbsp;出游时间<input type=\"text\" id=\"txtDetail_Sdate" + i + "\" style=\"width: 80px\"/>至<input type=\"text\" id=\"txtDetail_Edate" + i + "\" style=\"width: 80px\"/></td></tr>");
            SbHtml.Append("<tr class=\"noborder\"><td colspan=\"3\" class=\"tl PL10\"><input type=\"radio\" id=\"rdoReasondb" + i + "2\" name=\"Reasondb" + i + "\" /><label for=\"rdoReasondb" + i + "2\">购车</label></td></tr>");
            SbHtml.Append("<tr class=\"noborder\"><td colspan=\"3\" class=\"tl PL10\"><input type=\"radio\" id=\"rdoReasondb" + i + "3\" name=\"Reasondb" + i + "\" /><label for=\"rdoReasondb" + i + "3\">购房</label></td></tr>");
            SbHtml.Append("<tr class=\"noborder\"><td colspan=\"3\" class=\"tl PL10\"><input type=\"radio\" id=\"rdoReasondb" + i + "0\" name=\"Reasondb" + i + "\" /><label for=\"rdoReasondb" + i + "0\">其他原因</label><input type=\"text\" id=\"txtDetail_AnotherR" + i + "\" style=\"width: 83.5%\"/></td></tr>");
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        T_OfficeAutomation_Document_OpenProve t_OfficeAutomation_Document_OpenProve = new T_OfficeAutomation_Document_OpenProve();
        ...
        //{
            ...
            if (IsNewApply)
            {
                ...
                t_OfficeAutomation_Main.OfficeAutomation_SerialNumber = "OpenProve" + DateTime.Now.ToString("yyyyMMddHHmmssfffffff");
                t_OfficeAutomation_Main.OfficeAutomation_DocumentID = 70; //在《申请表字典表》t_Dic_OfficeAutomation_Document，找到该表的主键，这步一错则会打开别的表
                ...
                t_OfficeAutomation_Main.OfficeAutomation_Main_Summary = this.txtName.Text;

                

                //MainID = t_OfficeAutomation_Main.OfficeAutomation_Main_ID.ToString();
                t_OfficeAutomation_Document_OpenProve = GetModelFromPage(Guid.NewGuid());

                da_OfficeAutomation_Main_Inherit.Insert(t_OfficeAutomation_Main);
                da_OfficeAutomation_Document_OpenProve_Inherit.Add(t_OfficeAutomation_Document_OpenProve);//插入申请表

                bool bs = InsertOpenProveDetail(t_OfficeAutomation_Document_OpenProve.OfficeAutomation_Document_OpenProve_ID);
                ...
                Common.AddLog(EmployeeID, EmployeeName, 74, new Guid(MainID), 1);//日志，创建申请表

                //RunJS("window.showModalDialog(\"/Apply/Apply_UploadFile.aspx?MainID=" + MainID + "\", '" + MainID + "', \"dialogHeight=165px\");var win=window.showModalDialog(\"Apply_OpenProve_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=800px;dialogHeight=320px\");if(win=='success'){window.location='/Apply/Apply_Search.aspx'; }");
                if (!bs)
                {
                    ...
                    RunJS("alert('申请表现已保存成功。');window.location='/Apply/Apply_Search.aspx'; ");
                }
                else
                    da_OfficeAutomation_Flow_Inherit.Insert(t_OfficeAutomation_Flow);//52100-2016/10/14
                    RunJS("alert('申请表现已保存成功！');var win=window.showModalDialog(\"Apply_OpenProve_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=800px;dialogHeight=320px\");if(win=='success'){window.location='/Apply/Apply_Search.aspx'; }");
                #endregion
            }
            else
            {
                ...
                if (Purview.Contains("OA_ITPower_001"))//是否有编辑权限
                {
                    if (!Update())
                    {
                        da_OfficeAutomation_Flow_Inherit.DeleteByMainIDAndIdxs(MainID, "1,2,3");
                        RunJS("alert('保存成功。');window.location='/Apply/Apply_Search.aspx';");
                    }
                    else
                        RunJS("alert('保存成功！');var win=window.showModalDialog(\"Apply_OpenProve_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=800px;dialogHeight=320px\");if(win=='success'){window.location='/Apply/Apply_Search.aspx'; }");
                }
                else
                    Alert("未开通编辑修改权限。");
                #endregion
    ...
    protected void btnSign_Click(object sender, EventArgs e)
    {
        ...
        DataSet ds = da_OfficeAutomation_Document_OpenProve_Inherit.SelectByMainID(MainID);
        DataRow drRow = ds.Tables[0].Rows[0];
        ID = drRow["OfficeAutomation_Document_OpenProve_ID"].ToString(); 
        ...
        if (Purview.Contains("OA_ITPower_002"))
        {
            try
            {
                ...
                if (isSignSuccess)
                {
                    ...
                    string apply = ds.Tables[0].Rows[0]["OfficeAutomation_Document_OpenProve_Apply"].ToString();
                    string employname;
                    string serialNumber = drRow["OfficeAutomation_SerialNumber"].ToString();
                    string documentName = da_OfficeAutomation_Flow_Inherit.GetDocumentNameByMainID(MainID);
                    string department = drRow["OfficeAutomation_Document_OpenProve_Department"].ToString();
                    ...
                    sbMailBody.Append("<br/><br/>填写人：" + drRow["OfficeAutomation_Document_OpenProve_Apply"]);
                    sbMailBody.Append("<br/>填写日期：" + drRow["OfficeAutomation_Document_OpenProve_ApplyDate"]);
                    sbMailBody.Append("<br/>现任职部门：" + DateTime.Parse(drRow["OfficeAutomation_Document_OpenProve_ApplyDate"].ToString()).ToString("yyyy-MM-dd"));
                    sbMailBody.Append("<br/>发文编号：" + drRow["OfficeAutomation_Document_OpenProve_ApplyID"]);
                    sbMailBody.Append("<br/>姓名：" + drRow["OfficeAutomation_Document_OpenProve_Name"]);
                    sbMailBody.Append("<br/>工号：" + drRow["OfficeAutomation_Document_OpenProve_Code"]);

                    sbMailBody.Append("<br/>入职日期：" + drRow["OfficeAutomation_Document_OpenProve_EnterDate"]);
                    sbMailBody.Append("<br/>现职位：" + drRow["OfficeAutomation_Document_OpenProve_Position"]);
                    sbMailBody.Append("<br/>现职级：" + drRow["OfficeAutomation_Document_OpenProve_Rank"]);
    ...
    protected void btnSignSave_Click(object sender, EventArgs e)
    {
        ...
        //        da_OfficeAutomation_Document_OpenProve_Inherit.AddRemarkByID(ID, CommonConst.SIGN_FINANCE);
        //    Alert("标记成功。");
        //}
        //catch
        //{
    ...
    private T_OfficeAutomation_Document_OpenProve GetModelFromPage(Guid UndertakeProjID)
    {
        T_OfficeAutomation_Document_OpenProve t_OfficeAutomation_Document_OpenProve = new T_OfficeAutomation_Document_OpenProve();
        t_OfficeAutomation_Document_OpenProve.OfficeAutomation_Document_OpenProve_ID = UndertakeProjID;
        t_OfficeAutomation_Document_OpenProve.OfficeAutomation_Document_OpenProve_MainID = new Guid(MainID);
        t_OfficeAutomation_Document_OpenProve.OfficeAutomation_Document_OpenProve_Apply = EmployeeName;
        t_OfficeAutomation_Document_OpenProve.OfficeAutomation_Document_OpenProve_ApplyDate = DateTime.Now;
        t_OfficeAutomation_Document_OpenProve.OfficeAutomation_Document_OpenProve_ApplyID = "";
        t_OfficeAutomation_Document_OpenProve.OfficeAutomation_Document_OpenProve_Department = txtDepartment.Text;
        t_OfficeAutomation_Document_OpenProve.OfficeAutomation_Document_OpenProve_Code = txtCode.Text;
        t_OfficeAutomation_Document_OpenProve.OfficeAutomation_Document_OpenProve_EnterDate = txtEnterDate.Text;

        t_OfficeAutomation_Document_OpenProve.OfficeAutomation_Document_OpenProve_Name = txtName.Text;
        t_OfficeAutomation_Document_OpenProve.OfficeAutomation_Document_OpenProve_Phone = this.txtPhone.Text;//联系电话
        t_OfficeAutomation_Document_OpenProve.OfficeAutomation_Document_OpenProve_SignAddress = this.txtSignAddress.Text;//签收地址
        t_OfficeAutomation_Document_OpenProve.OfficeAutomation_Document_OpenProve_SignPhone = this.txtSignPhone.Text;//签收电话
        t_OfficeAutomation_Document_OpenProve.OfficeAutomation_Document_OpenProve_Recipient = txtRecipient.Text;//签收人
        t_OfficeAutomation_Document_OpenProve.OfficeAutomation_Document_OpenProve_ExceptionalCase = this.ExceptionalCase.Value;//备注
        t_OfficeAutomation_Document_OpenProve.OfficeAutomation_Document_OpenProve_SignMode = hiSignMode.Value;
      // t_OfficeAutomation_Document_OpenProve.OfficeAutomation_Document_OpenProve_Rank = txtRank.Text;
        t_OfficeAutomation_Document_OpenProve.OfficeAutomation_Document_OpenProve_Position = txtPosition.Text;

        return t_OfficeAutomation_Document_OpenProve;
    }
    ...
    private bool Update()
    {
        ...
        DataSet ds = new DataSet();
        ds = da_OfficeAutomation_Document_OpenProve_Inherit.SelectByMainID(MainID);
        ID = ds.Tables[0].Rows[0]["OfficeAutomation_Document_OpenProve_ID"].ToString();

        t_OfficeAutomation_Document_OpenProve = GetModelFromPage(new Guid(ID));

        string apply = "";
        string depname = this.txtDepartment.Text;
        string summary = this.txtName.Text;
        string applydate = "";
        string mainid = MainID;

        new DA_OfficeAutomation_Main_Inherit().UpdateMain(mainid, depname, apply, applydate, summary);
        da_OfficeAutomation_Document_OpenProve_Inherit.Edit(t_OfficeAutomation_Document_OpenProve);//修改申请表

        DA_OfficeAutomation_Document_OpenProve_Detail_Inherit da_OfficeAutomation_Document_OpenProve_Detail_Inherit = new DA_OfficeAutomation_Document_OpenProve_Detail_Inherit();
        da_OfficeAutomation_Document_OpenProve_Detail_Inherit.Delete(ID);
        bool bs = InsertOpenProveDetail(new Guid(ID));

        Common.AddLog(EmployeeID, EmployeeName, 74, new Guid(MainID), 2);//日志，修改申请表
        return bs;
    }
    ...
    /// 新增详情表
    /// </summary>
    /// <param name="gOpenProveID"></param>
    private bool InsertOpenProveDetail(Guid gOpenProveID)
    {
        bool bl = false;
        if (hdDetail.Value == "")
            return bl;

        T_OfficeAutomation_Document_OpenProve_Detail t_OfficeAutomation_Document_OpenProve_Detail;
        DA_OfficeAutomation_Document_OpenProve_Detail_Inherit da_OfficeAutomation_Document_OpenProve_Detail_Inherit = new DA_OfficeAutomation_Document_OpenProve_Detail_Inherit();
        ...
        //{
            for (int i = 0; i < details.Length; i++)
            {
                ...
                //if (detail[0] == "")
                //    continue;
                t_OfficeAutomation_Document_OpenProve_Detail = new T_OfficeAutomation_Document_OpenProve_Detail();
                t_OfficeAutomation_Document_OpenProve_Detail.OfficeAutomation_Document_OpenProve_Detail_ID = Guid.NewGuid();
                t_OfficeAutomation_Document_OpenProve_Detail.OfficeAutomation_Document_OpenProve_Detail_MainID = gOpenProveID;
                t_OfficeAutomation_Document_OpenProve_Detail.OfficeAutomation_Document_OpenProve_Detail_Prove = detail[0].Trim();
                t_OfficeAutomation_Document_OpenProve_Detail.OfficeAutomation_Document_OpenProve_Detail_No = detail[1].Trim();
                t_OfficeAutomation_Document_OpenProve_Detail.OfficeAutomation_Document_OpenProve_Detail_Reasondb = detail[2].Trim();
                t_OfficeAutomation_Document_OpenProve_Detail.OfficeAutomation_Document_OpenProve_Detail_Address = detail[3].Trim();
                t_OfficeAutomation_Document_OpenProve_Detail.OfficeAutomation_Document_OpenProve_Detail_Sdate = detail[4].Trim();
                t_OfficeAutomation_Document_OpenProve_Detail.OfficeAutomation_Document_OpenProve_Detail_Edate = detail[5].Trim();
                t_OfficeAutomation_Document_OpenProve_Detail.OfficeAutomation_Document_OpenProve_Detail_AnotherR = detail[6].Trim();
                t_OfficeAutomation_Document_OpenProve_Detail.OfficeAutomation_Document_OpenProve_Detail_Kind = detail[7].Trim();
                t_OfficeAutomation_Document_OpenProve_Detail.OfficeAutomation_Document_OpenProve_Detail_AnotherKind = detail[8].Trim();
                if (t_OfficeAutomation_Document_OpenProve_Detail.OfficeAutomation_Document_OpenProve_Detail_Reasondb == "1")
                    bl = true;
                da_OfficeAutomation_Document_OpenProve_Detail_Inherit.Add(t_OfficeAutomation_Document_OpenProve_Detail);
            }
        //}
        //catch (Exception ee)
        //{
    ...
    protected void btnReWrite_Click(object sender, EventArgs e)
    {
        ...
        Session["FLG_ReWrite76"] = "1";
        Response.Write("<script>window.open('Apply_OpenProve_Detail.aspx?MainID=" + MainID + "');</script>");
    }
    protected void btnSPDF_Click(object sender, EventArgs e) //M_PDF
    {
        try
        {
            ...
            Response.AddHeader("content-disposition", "attachment; filename=" + HttpUtility.UrlEncode("薪酬福利类证明开具申请表.pdf"));//强制下载 
    ...
    protected void btnCancelSign_Click(object sender, EventArgs e) //20141202：CancelSign
    {
        try
        {
            ...
            string signEmployeeName = EmployeeName;
            DA_OfficeAutomation_Document_OpenProve_Inherit da_OfficeAutomation_Document_OpenProve_Inherit = new DA_OfficeAutomation_Document_OpenProve_Inherit();
            DataSet ds = da_OfficeAutomation_Document_OpenProve_Inherit.SelectByMainID(MainID); //在不同的表中要修改
            DataRow drRow = ds.Tables[0].Rows[0];
            string apply = ds.Tables[0].Rows[0]["OfficeAutomation_Document_OpenProve_Apply"].ToString();
            string employname;
            string serialNumber = drRow["OfficeAutomation_SerialNumber"].ToString();
        ...
        try
        {
            ...
            if (Purview.Contains("OA_ITPower_001"))//是否有编辑权限
            {
                ...
                if (!b)
                {
                    ...
                    RunJS("alert('所作的修改已保存！');var win=window.showModalDialog(\"Apply_OpenProve_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=800px;dialogHeight=320px\");if(win=='success'){window.location='/Apply/Apply_Search.aspx'; }");
            }
            else
                Alert("未开通编辑修改权限。");
            #endregion
    ...
    protected void btnEditFlow_DoClick(object sender, EventArgs e)
    {
        try
        {
            ...
            //在不同的表中要修改
            RunJS("var win=window.showModalDialog(\"Apply_OpenProve_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=830px;dialogHeight=280px\");if(win=='success'){window.location='/Ajax/Editor_Flow.ashx?MainID=" + MainID + "&applyUrl=" + applyUrl + "&apply=" + apply + "&serialNumber=" + serialNumber + "&department=" + department + "&Jhref=" + Page.Request.Url + "&mid=" + EmployeeID + "'; }");
    ...
    protected void btnLoadPath_Click(object sender, EventArgs e)
    {
        ...
        { sFileName = tempName + ".doc"; sPdfFileName = tempName + ".pdf"; }
        ...
        { sFileName = tempName + ".docx"; sPdfFileName = tempName + ".pdf"; }
        else if (this.txtFilePath.PostedFile.ContentType.ToString() == "application/vnd.ms-excel")
        { sFileName = tempName + ".xls"; sPdfFileName = tempName + ".pdf"; }
        ...
        { sFileName = tempName + ".xlsx"; sPdfFileName = tempName + ".pdf"; }
        ...
        if (this.txtFilePath.PostedFile.ContentLength > 104857600)
        {
            RunJS("alert('上传的文件大小超过10M,请重新上传！');");
            return;
        }
    ...
    protected void btnCantUse_Click(object sender, EventArgs e)
    {
        ...
        string documentName = da_OfficeAutomation_Flow_Inherit.GetDocumentNameByMainID(MainID);
        DA_OfficeAutomation_Document_OpenProve_Inherit da_OfficeAutomation_Document_OpenProve_Inherit = new DA_OfficeAutomation_Document_OpenProve_Inherit();
        ds = da_OfficeAutomation_Document_OpenProve_Inherit.SelectByMainID(MainID); //在不同的表中要修改
        string apply = ds.Tables[0].Rows[0]["OfficeAutomation_Document_OpenProve_Apply"].ToString();
        string applyUrl = Page.Request.Url.ToString();
        applyUrl = applyUrl.Substring(applyUrl.IndexOf("/Apply/"));
