<%@ Page ValidateRequest="false" Title="项目部实收佣金调整申请表 - 广州中原电子审批系统" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="Apply_ProjBaComm_Detail.aspx.cs" Inherits="Apply_ProjBaComm_Apply_ProjBaComm_Detail" %>
    ...
    <script type="text/javascript">
	        ...
	        $("#tbDetail tr").each(function(n) {
	            if (n != 0 && n != $("#tbDetail tr").size()-1) {
	                if ($.trim($("#txtReportNo" + n).val()) == "") {
	                    ...
	                    alert("第" + n + "行的单位地址必须填写。");
	                    $("#txtAddress" + n).focus();
	                    flag = false;
	                }
	                else if ($.trim($("#txtBname" + n).val()) == "") {
	                    alert("第" + n + "行的客户名称必须填写。");
	                    $("#txtBname" + n).focus();
	                    flag = false;
	                }
	                    ...
	                    data += $.trim($("#txtDetail_pNo" + n).html()) 
                            + "&&" + $.trim($("#txtReportNo" + n).val()) 
                            + "&&" + $.trim($("#txtDealDate" + n).val()) 
                            + "&&" + $.trim($("#txtAddress" + n).val()) 
                            + "&&" + $.trim($("#txtBname" + n).val()) 
                            + "&&" + $.trim($("#txtBackMoney" + n).val()) 
                            + "&&" + ($("#rdoIsOnJob" + n + "1").prop('checked')?"1":"0") 
                            + "&&" + ($("#rdoIsPayComm" + n + "1").prop('checked')?"1":"0")
                            + "||";
	            }
	        });
	        //}
		...
		function upload() {
			...
			if(sReturnValue=='success')
				window.location='Apply_ProjBaComm_Detail.aspx?MainID=<%=MainID %>';
		}
		...
		function editflow(){
			var win=window.showModalDialog("Apply_ProjBaComm_Flow.aspx?MainID=<%=MainID %>","","dialogWidth=800px;dialogHeight=250px");
					if(win=='success')
						window.location="Apply_ProjBaComm_Detail.aspx?MainID=<%=MainID %>";
		}
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
	...
	<div style='text-align: center; width: 840px; margin: 0 auto;'>
		...
		<div style='text-align: center'>
            ...
            <div id="snum" style="width:800px;margin:0 auto;"><span style="float:right;" class="file_number">文档编码(自动生成)：<%=SerialNumber %></span></div>
			<table id="tbAround"  width='800px'>
				...
				<tr>
					...
					<td class="auto-style2">关于<asp:textbox id="txtBuilding" runat="server" Width="75%"></asp:textbox>的<br />实收佣金调整申请</td>
					<td class="auto-style3">回复电话</td>
					<td class="tl PL10">020-<asp:TextBox ID="txtReplyPhone" runat="server"></asp:TextBox></td>
				</tr>
				<tr>
					<td class="tl PL10" colspan="4">
						第一部分：实收佣金调整的原因详述（包括事件的始末、退佣的金额等等）<br/>
						<asp:textbox id="txtReason" runat="server" TextMode="MultiLine" Width="98%" Rows="10" style="overflow: auto;"></asp:textbox>
					</td>
				</tr>
                <tr>
                    <td class="auto-style4">是否调整为预收挂账</td>
                    <td style="padding-left: 10px; text-align: left">
                        <asp:RadioButton ID="rdbTurnBack1" runat="server" GroupName="TurnBack" Text="是" />　
                        <asp:RadioButton ID="rdbTurnBack2" runat="server" GroupName="TurnBack" Text="否" />
                    </td>
                    <td class="auto-style3">是否退回发展商</td>
                    <td style="padding-left: 10px; text-align: left">
                        <asp:RadioButton ID="rdbBDeveloper1" runat="server" GroupName="BDeveloper" Text="是" />　
                        <asp:RadioButton ID="rdbBDeveloper2" runat="server" GroupName="BDeveloper" Text="否" />
                    </td>
                </tr>
				<tr>
				...
				<tr id="trManager3" class="noborder" style="height: 85px;">
					<td class="auto-style4">事业部负责人</td>
					<td colspan="3" class="tl PL10">
                ...
                <tr id="trManager5" class="noborder" style="height: 85px;">
					<td class="auto-style4" rowspan="2">二级市场总经理</td>
					<td colspan="3" class="tl PL10">
                ...
                <tr id="trManager7" class="noborder" style="height: 85px;">
					<td class="auto-style4">物业部区域负责人</td>
					<td colspan="3" class="tl PL10">
				...
				<tr class="noborder" style="height: 85px;">
					<td rowspan="2"  class="auto-style4">法律部意见</td>
					<td colspan="3" class="tl PL10" style=" ">
				...
				<tr class="noborder" style="height: 85px;">
					<td rowspan="3"  class="auto-style4">财务部意见</td>
					<td colspan="3" class="tl PL10" style=" ">
                        ...
                        <asp:CheckBox ID="ckbAddIDx10" runat="server" Text="增加宁伟雄审批"  Visible="false"/><br />
...
<%--                <tr id="trLogistics1" class="noborder" style="height: 85px;display: none;">
    ...
    <asp:Button ID="btnWillEnd" runat="server" Text="结束" OnClick="btnWillEnd_Click" Visible="False" /></td>
				...
				<tr id="trGeneralManager" class="noborder" style="height: 85px; display: none;">
					<td class="auto-style4" >董事总经理<br />审批</td>
					<td colspan="3" class="tl PL10" style=" ">
						<input id="rdbYesIDx15" type="radio" name="agree15" /><label for="rdbYesIDx15">同意</label><input id="rdbNoIDx15" type="radio" name="agree15" /><label for="rdbNoIDx15">不同意</label>
                        <input id="rdbOtherIDx15" type="radio" name="agree15" /><label for="rdbOtherIDx15">其他意见</label><br />
						<textarea id="txtIDx15" rows="5" style="width: 98%; overflow: auto;"></textarea><input type="button" id="btnSignIDx15" value="签署意见" onclick="sign('15')" style="display: none;" />
                        <br />
                        <span style="padding-right: 30px; padding-bottom: 10px; text-align: right; float: right;">
...
<%--                <tr id="tlsc2" class="noborder" style="height: 85px; display: none;">
                ...
                <tr id="trAddAnoF1" class="noborder" style="height: 85px; display: none;">
					<td class="auto-style4" >申请人回复审批意见<br />（可选项）</td>
					<td colspan="3" class="tl PL10" style=" ">
                ...
                <tr id="trAddAnoF3" class="noborder" style="height: 85px; display: none;">
					<td class="auto-style4" >董事总经理复审</td>
					<td colspan="3" class="tl PL10" style=" ">
        ...
        <asp:button runat="server" id="btnSignCompelete" text="标注已完成" onclick="btnSignCompelete_Click" visible="false" />
