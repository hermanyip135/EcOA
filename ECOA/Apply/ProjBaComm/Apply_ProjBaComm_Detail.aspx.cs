...
public partial class Apply_ProjBaComm_Apply_ProjBaComm_Detail : BasePage
{
    #region 变量声明及定义
    bool tpdf = false;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            try
            {
                if (Session["FLG_ReWrite72"].ToString() == "1")
                {
                    ViewState["FLG_ReWrite"] = "1";
                    Session["FLG_ReWrite72"] = null;
                }
            }
            catch
            {
        ...
        else
        {
            ...
            ds = da_OfficeAutomation_Document_ProjBaComm_Inherit.SelectByMainID(MainID);
            if (ds.Tables[0].Rows.Count > 0)
            {
                DataRow dr = ds.Tables[0].Rows[0];
                ID = dr["OfficeAutomation_Document_ProjBaComm_ID"].ToString();
            }
        }
    }
    ...
    public void NewPage()
    {
        ...
        rdbBDeveloper2.Checked = true;
        rdbTurnBack1.Checked = true;
        lblApply.Text = EmployeeName;
        lblApplyDate.Text = DateTime.Now.ToString("yyyy-MM-dd");

        SbJs.Append("<script type=\"text/javascript\">$(\"#btnAddRow\").show();$(\"#btnDeleteRow\").show();$(\"#divUploadDetailed\").show();");
        DrawDetailTable(1);
        IsNewApply = true;
    ...
    public void LoadPage()
    {
        ...
        DA_OfficeAutomation_Document_ProjBaComm_Inherit da_OfficeAutomation_Document_ProjBaComm_Inherit = new DA_OfficeAutomation_Document_ProjBaComm_Inherit();
        DA_OfficeAutomation_Document_ProjBaComm_Detail_Inherit da_OfficeAutomation_Document_ProjBaComm_Detail_Inherit = new DataAccess.Operate.DA_OfficeAutomation_Document_ProjBaComm_Detail_Inherit();
        IsNewApply = false;
        string flowState;
        try
        {
        ...
        DataRow dr = ds.Tables[0].Rows[0];
        ID = dr["OfficeAutomation_Document_ProjBaComm_ID"].ToString();
        string applicant=dr["OfficeAutomation_Document_ProjBaComm_Apply"].ToString();
        ApplyN = applicant;
        txtApplyID.Text = dr["OfficeAutomation_Document_ProjBaComm_ApplyID"].ToString();
        txtDepartment.Text = dr["OfficeAutomation_Document_ProjBaComm_Department"].ToString();
        lblApply.Text = applicant;
        lblApplyDate.Text = DateTime.Parse(dr["OfficeAutomation_Document_ProjBaComm_ApplyDate"].ToString()).ToString("yyyy-MM-dd");
        txtBuilding.Text = dr["OfficeAutomation_Document_ProjBaComm_Building"].ToString();
        txtReplyPhone.Text = dr["OfficeAutomation_Document_ProjBaComm_ReplyPhone"].ToString();
        txtReason.Text = dr["OfficeAutomation_Document_ProjBaComm_Reason"].ToString();
        txtMoneyCount.Text = dr["OfficeAutomation_Document_ProjBaComm_MoneyCount"].ToString();
        txtMeasure.Text = dr["OfficeAutomation_Document_ProjBaComm_Measure"].ToString();
        if (dr["OfficeAutomation_Document_ProjBaComm_TurnBack"].ToString() == "1")
            rdbTurnBack1.Checked = true;
        else if (dr["OfficeAutomation_Document_ProjBaComm_TurnBack"].ToString() == "2")
            rdbTurnBack2.Checked = true;
        if (dr["OfficeAutomation_Document_ProjBaComm_BDeveloper"].ToString() == "1")
            rdbBDeveloper1.Checked = true;
        else if (dr["OfficeAutomation_Document_ProjBaComm_BDeveloper"].ToString() == "2")
            rdbBDeveloper2.Checked = true;
        SerialNumber = dr["OfficeAutomation_SerialNumber"].ToString();

        ds = da_OfficeAutomation_Document_ProjBaComm_Detail_Inherit.SelectByID(ID);
        ...
        else
        {
            ...
            for (int n = 0; n < detailCount; n++)
            {
                ...
                SbJs.Append("$('#txtReportNo" + i + "').val('" + dr["OfficeAutomation_Document_ProjBaComm_Detail_ReportNo"] + "');");
                SbJs.Append("$('#txtDealDate" + i + "').val('" + dr["OfficeAutomation_Document_ProjBaComm_Detail_DealDate"] + "');");
                SbJs.Append("$('#txtAddress" + i + "').val('" + dr["OfficeAutomation_Document_ProjBaComm_Detail_Address"] + "');");
                SbJs.Append("$('#txtBname" + i + "').val('" + dr["OfficeAutomation_Document_ProjBaComm_Detail_Bname"] + "');");
                SbJs.Append("$('#txtBackMoney" + i + "').val('" + dr["OfficeAutomation_Document_ProjBaComm_Detail_BackMoney"] + "');");
                SbJs.Append("$('#rdoIsOnJob" + i + (dr["OfficeAutomation_Document_ProjBaComm_Detail_IsOnJob"].ToString()=="True"?"1":"0") + "').attr('checked','checked');");
                SbJs.Append("$('#rdoIsPayComm" + i + (dr["OfficeAutomation_Document_ProjBaComm_Detail_IsPayComm"].ToString() == "True" ? "1" : "0") + "').attr('checked','checked');");
            }
        }
        ...
        if (isApplicant)
        {
            ...
            if (flowState == "1")
            {
                GetAllDepartment();
                btnSave.Visible = true;
                SbJs.Append("$(\"#btnAddRow\").show();$(\"#btnDeleteRow\").show();$(\"#divUploadDetailed\").show();");
            }
        ...
        //            }
        //        }
        //    if (decimal.Parse(txtMoneyCount.Text) > 20000)
        //    {
        //        flowsd = da_OfficeAutomation_Flow_Inherit.SelectByMainIDAndIdx(MainID, 11);
        //        if (flowsd == null)
        //        {
        ...
        if ((EmployeeID == CommonConst.EMP_FINANCE_PUBLIC_ID || EmployeeID == "3575" || EmployeeID == "24962" || EmployeeID == "27997" || EmployeeID == "20208") && flowState == "3")
            btnSignSave.Visible = true;
        if ((EmployeeID == "3575" || EmployeeID == "24962" || EmployeeID == "27997" || EmployeeID == "41960" ) && flowState == "3")
            btnSignCompelete.Visible = true;
        ...
        for (int i = 0; i < drc.Count; i++)
        {
            ...
            if (drc[i]["OfficeAutomation_Flow_Audit"].ToString() == "False" && flag2)//正待审理
            {
                ...
                if (curemp.Contains(EmployeeName)) //M_Add：宁伟雄
                {
                    switch (curidx)
                    {
                        case "10":
    ...
    public void DrawDetailTable(int n)
    {
        ...
        for (int i = 1; i <= n; i++)
        {
            ...
            SbHtml.Append("     <td><input type=\"text\" id=\"txtReportNo" + i + "\" style=\"width:90%\"/></td>");
            ...
            SbHtml.Append("     <td><input type=\"text\" id=\"txtBname" + i + "\" style=\"width:90%\"/></td>");
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        T_OfficeAutomation_Document_ProjBaComm t_OfficeAutomation_Document_ProjBaComm = new T_OfficeAutomation_Document_ProjBaComm();
        T_OfficeAutomation_Flow t_OfficeAutomation_Flow = new T_OfficeAutomation_Flow();
        ...
        try
        {
            if (IsNewApply)
            {
                ...
                t_OfficeAutomation_Main.OfficeAutomation_SerialNumber = "ProjBaComm" + DateTime.Now.ToString("yyyyMMddHHmmssfffffff");
                t_OfficeAutomation_Main.OfficeAutomation_DocumentID = 67;
                ...
                t_OfficeAutomation_Document_ProjBaComm.OfficeAutomation_Document_ProjBaComm_ID = Guid.NewGuid();
                t_OfficeAutomation_Document_ProjBaComm.OfficeAutomation_Document_ProjBaComm_MainID = t_OfficeAutomation_Main.OfficeAutomation_Main_ID;
                t_OfficeAutomation_Document_ProjBaComm.OfficeAutomation_Document_ProjBaComm_Apply = EmployeeName;
                t_OfficeAutomation_Document_ProjBaComm.OfficeAutomation_Document_ProjBaComm_ApplyDate = DateTime.Now;
                t_OfficeAutomation_Document_ProjBaComm.OfficeAutomation_Document_ProjBaComm_ApplyID = txtApplyID.Text;
                t_OfficeAutomation_Document_ProjBaComm.OfficeAutomation_Document_ProjBaComm_Department = txtDepartment.Text;
                t_OfficeAutomation_Document_ProjBaComm.OfficeAutomation_Document_ProjBaComm_Building = txtBuilding.Text;
                t_OfficeAutomation_Document_ProjBaComm.OfficeAutomation_Document_ProjBaComm_ReplyPhone = txtReplyPhone.Text;
                t_OfficeAutomation_Document_ProjBaComm.OfficeAutomation_Document_ProjBaComm_Reason = txtReason.Text;
                t_OfficeAutomation_Document_ProjBaComm.OfficeAutomation_Document_ProjBaComm_MoneyCount = Decimal.Parse(txtMoneyCount.Text);
                t_OfficeAutomation_Document_ProjBaComm.OfficeAutomation_Document_ProjBaComm_Measure = txtMeasure.Text;
                t_OfficeAutomation_Document_ProjBaComm.OfficeAutomation_Document_ProjBaComm_TurnBack = rdbTurnBack1.Checked ? "1" : rdbTurnBack2.Checked ? "2" : "0";
                t_OfficeAutomation_Document_ProjBaComm.OfficeAutomation_Document_ProjBaComm_BDeveloper = rdbBDeveloper1.Checked ? "1" : rdbBDeveloper2.Checked ? "2" : "0";
                ...
                t_OfficeAutomation_Main.OfficeAutomation_Main_Summary = txtBuilding.Text + txtMoneyCount.Text;
                ...
                da_OfficeAutomation_Document_ProjBaComm_Inherit.Insert(t_OfficeAutomation_Document_ProjBaComm);//插入申请表

                InsertProjBaCommDetail(t_OfficeAutomation_Document_ProjBaComm.OfficeAutomation_Document_ProjBaComm_ID);
                ...
                #region 是否退回发展商\预收挂账为“是”，添加黄志超审批

                if (rdbTurnBack1.Checked == true)
                {
                    t_OfficeAutomation_Flow.OfficeAutomation_Flow_ID = Guid.NewGuid();
                ...
                #region 是否退回发展商为“是”，单笔退佣>￥20,0000，则增加董事总经理环节

                if (rdbBDeveloper1.Checked == true && t_OfficeAutomation_Document_ProjBaComm.OfficeAutomation_Document_ProjBaComm_MoneyCount > 200000)
                {
                ...
                Common.AddLog(EmployeeID, EmployeeName, 71, new Guid(MainID), 1);//日志，创建申请表
                Common.AddLog(EmployeeID, EmployeeName, 3, new Guid(MainID), 1);//日志，创建流程

                RunJS("window.showModalDialog(\"/Apply/Apply_UploadFile.aspx?MainID=" + MainID + "\", '" + MainID + "', \"dialogHeight=165px\");var win=window.showModalDialog(\"Apply_ProjBaComm_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=800px;dialogHeight=320px\");if(win=='success'){window.location='/Apply/Apply_Search.aspx'; }");
                #endregion
            }
            else
            {
    ...
    protected void btnSign_Click(object sender, EventArgs e)
    {
        ...
        DA_OfficeAutomation_Document_ProjBaComm_Inherit da_OfficeAutomation_Document_ProjBaComm_Inherit = new DA_OfficeAutomation_Document_ProjBaComm_Inherit();
        DA_OfficeAutomation_Document_ProjBaComm_Detail_Inherit da_OfficeAutomation_Document_ProjBaComm_Detail_Inherit = new DA_OfficeAutomation_Document_ProjBaComm_Detail_Inherit();

        DataSet ds = da_OfficeAutomation_Document_ProjBaComm_Inherit.SelectByMainID(MainID);
        DataRow drRow = ds.Tables[0].Rows[0];
        ID = drRow["OfficeAutomation_Document_ProjBaComm_ID"].ToString(); 
        ...
        if (Purview.Contains("OA_ITPower_002"))
        {
            try
            {
                ...
                if (flowIDx == "10") //M_Add：宁伟雄 20150202
                {
                    T_OfficeAutomation_Flow t_OfficeAutomation_Flow = new T_OfficeAutomation_Flow();
                    t_OfficeAutomation_Flow.OfficeAutomation_Flow_MainID = new Guid(MainID);
                    t_OfficeAutomation_Flow.OfficeAutomation_Flow_Suggestion = "";

                    if (ckbAddIDx10.Checked)//增加审批环节是否勾选，如果是则添加第7步黄志超审批
                    {
                ...
                bool isSignSuccess = (flowIDx == "8" || flowIDx == "10" || ((IList)flowN).Contains(flowIDx)) ? da_OfficeAutomation_Flow_Inherit.UpdateContainIsAgreeForSign(MainID, signEmployeeId, signEmployeeName, hdSuggestion.Value, flowIDx, hdIsAgree.Value) : da_OfficeAutomation_Flow_Inherit.UpdateContainIsAgreeForSignAndMulti(MainID, signEmployeeId, signEmployeeName, hdSuggestion.Value, flowIDx, hdIsAgree.Value);
                ...
                if (isSignSuccess)
                {
                    ...
                    string apply = ds.Tables[0].Rows[0]["OfficeAutomation_Document_ProjBaComm_Apply"].ToString();
                    string employname;
                    string serialNumber = drRow["OfficeAutomation_SerialNumber"].ToString();
                    string documentName = da_OfficeAutomation_Flow_Inherit.GetDocumentNameByMainID(MainID);
                    string department = drRow["OfficeAutomation_Document_ProjBaComm_Department"].ToString();
                    ...
                    sbMailBody.Append("<br/><br/>申请人：" + drRow["OfficeAutomation_Document_ProjBaComm_Apply"]);
                    sbMailBody.Append("<br/>申请部门：" + drRow["OfficeAutomation_Document_ProjBaComm_Department"]);
                    sbMailBody.Append("<br/>申请日期：" + DateTime.Parse(drRow["OfficeAutomation_Document_ProjBaComm_ApplyDate"].ToString()).ToString("yyyy-MM-dd"));
                    sbMailBody.Append("<br/>文件主题：关于" + drRow["OfficeAutomation_Document_ProjBaComm_Building"].ToString() + "的退佣申请");
                    sbMailBody.Append("<br/>回复电话：020-" + drRow["OfficeAutomation_Document_ProjBaComm_ReplyPhone"]);
                    sbMailBody.Append("<br/>退佣原因：" + drRow["OfficeAutomation_Document_ProjBaComm_Reason"]);
                    sbMailBody.Append("<br/>退佣交易详细情况：");
                    ds = da_OfficeAutomation_Document_ProjBaComm_Detail_Inherit.SelectByMainID(MainID);
                    foreach (DataRow dr in ds.Tables[0].Rows)
                    {
                        sbMailBody.Append("<br/>成交报告编号：" + dr["OfficeAutomation_Document_ProjBaComm_Detail_ReportNo"]);
                        sbMailBody.Append("<br/>成交日期：" + dr["OfficeAutomation_Document_ProjBaComm_Detail_DealDate"]);
                        sbMailBody.Append("<br/>单位地址：" + dr["OfficeAutomation_Document_ProjBaComm_Detail_Address"]);
                        sbMailBody.Append("<br/>客户名称：" + dr["OfficeAutomation_Document_ProjBaComm_Detail_Bname"]);
                        sbMailBody.Append("<br/>退佣金额：" + dr["OfficeAutomation_Document_ProjBaComm_Detail_BackMoney"]);
                        sbMailBody.Append("<br/>是否坏账：" + (dr["OfficeAutomation_Document_ProjBaComm_Detail_IsOnJob"].ToString() == "True" ? "是" : "否"));
                        sbMailBody.Append("<br/>是否已出佣：" + (dr["OfficeAutomation_Document_ProjBaComm_Detail_IsPayComm"].ToString() == "True" ? "是" : "否"));
                        sbMailBody.Append("<br/>");
                    }

                    sbMailBody.Append("<br/>合计退佣金额：" + drRow["OfficeAutomation_Document_ProjBaComm_MoneyCount"]);
                    //sbMailBody.Append("<br/>内部处理方案：" + drRow["OfficeAutomation_Document_ProjBaComm_Measure"]);
                    ...
                    if (hdIsAgree.Value != "0")//同意或其他意见
                    {
                        ...
                        if (da_OfficeAutomation_Flow_Inherit.IsFlowComplete(MainID))
                        {
                            ...
                            if(employeeList.Contains("黄轩明"))
                                employname = CommonConst.EMP_GMO_NAME + ",李小清,潘焕心";
                            else
                                employname = "李小清,潘焕心";
                            employnames = employname.Split(',');
                            for (int i = 0; i < employnames.Length; i++)
    ...
    protected void btnSignSave_Click(object sender, EventArgs e)
    {
        ...
        {
            DA_OfficeAutomation_Document_ProjBaComm_Inherit da_OfficeAutomation_Document_ProjBaComm_Inherit = new DA_OfficeAutomation_Document_ProjBaComm_Inherit();
            if (EmployeeID == CommonConst.EMP_FINANCE_PUBLIC_ID || EmployeeID == "3575" || EmployeeID == "24962" || EmployeeID == "27997" || EmployeeID == "20208")
                da_OfficeAutomation_Document_ProjBaComm_Inherit.AddRemarkByID_II(ID, CommonConst.SIGN_FINANCE);
            Alert("标记成功。");
        }
        catch
        {
    ...
    private void Update()
    {
        ...
        DataSet ds = new DataSet();
        ds = da_OfficeAutomation_Document_ProjBaComm_Inherit.SelectByMainID(MainID);
        ID = ds.Tables[0].Rows[0]["OfficeAutomation_Document_ProjBaComm_ID"].ToString();

        t_OfficeAutomation_Document_ProjBaComm.OfficeAutomation_Document_ProjBaComm_ID = new Guid(ID);
        t_OfficeAutomation_Document_ProjBaComm.OfficeAutomation_Document_ProjBaComm_MainID = new Guid(MainID);
        t_OfficeAutomation_Document_ProjBaComm.OfficeAutomation_Document_ProjBaComm_Apply = EmployeeName;
        t_OfficeAutomation_Document_ProjBaComm.OfficeAutomation_Document_ProjBaComm_ApplyDate = DateTime.Now;
        t_OfficeAutomation_Document_ProjBaComm.OfficeAutomation_Document_ProjBaComm_ApplyID = txtApplyID.Text;
        t_OfficeAutomation_Document_ProjBaComm.OfficeAutomation_Document_ProjBaComm_Department = txtDepartment.Text;
        ...
        string summary = this.txtBuilding.Text + this.txtMoneyCount.Text;
        string applydate = "";
        string mainid = MainID;

        da_OfficeAutomation_Main_Inherit.UpdateMain(mainid, depname, apply, applydate, summary);
        da_OfficeAutomation_Document_ProjBaComm_Inherit.Update(t_OfficeAutomation_Document_ProjBaComm);//修改申请表

        DA_OfficeAutomation_Document_ProjBaComm_Detail_Inherit da_OfficeAutomation_Document_ProjBaComm_Detail_Inherit = new DA_OfficeAutomation_Document_ProjBaComm_Detail_Inherit();
        da_OfficeAutomation_Document_ProjBaComm_Detail_Inherit.Delete(ID);
        InsertProjBaCommDetail(new Guid(ID));
        ...
        Common.AddLog(EmployeeID, EmployeeName, 71, new Guid(MainID), 2);//日志，修改申请表
    }
    ...
    /// 新增退佣申请明细
    /// </summary>
    /// <param name="gProjBaCommID"></param>
    private void InsertProjBaCommDetail(Guid gProjBaCommID)
    {
        if (hdDetail.Value == "")
            return;

        T_OfficeAutomation_Document_ProjBaComm_Detail t_OfficeAutomation_Document_ProjBaComm_Detail;
        DA_OfficeAutomation_Document_ProjBaComm_Detail_Inherit da_OfficeAutomation_Document_ProjBaComm_Detail_Inherit = new DA_OfficeAutomation_Document_ProjBaComm_Detail_Inherit();
        ...
        for (int i = 0; i < details.Length; i++)
        {
            ...
            t_OfficeAutomation_Document_ProjBaComm_Detail = new T_OfficeAutomation_Document_ProjBaComm_Detail();
            t_OfficeAutomation_Document_ProjBaComm_Detail.OfficeAutomation_Document_ProjBaComm_Detail_ID = Guid.NewGuid();
            t_OfficeAutomation_Document_ProjBaComm_Detail.OfficeAutomation_Document_ProjBaComm_Detail_MainID = gProjBaCommID;
            t_OfficeAutomation_Document_ProjBaComm_Detail.OfficeAutomation_Document_ProjBaComm_Detail_pNo = detail[0];
            t_OfficeAutomation_Document_ProjBaComm_Detail.OfficeAutomation_Document_ProjBaComm_Detail_ReportNo = detail[1];
            t_OfficeAutomation_Document_ProjBaComm_Detail.OfficeAutomation_Document_ProjBaComm_Detail_DealDate = detail[2];
            t_OfficeAutomation_Document_ProjBaComm_Detail.OfficeAutomation_Document_ProjBaComm_Detail_Address = detail[3];
            t_OfficeAutomation_Document_ProjBaComm_Detail.OfficeAutomation_Document_ProjBaComm_Detail_Bname = detail[4];
            t_OfficeAutomation_Document_ProjBaComm_Detail.OfficeAutomation_Document_ProjBaComm_Detail_BackMoney = detail[5];
            t_OfficeAutomation_Document_ProjBaComm_Detail.OfficeAutomation_Document_ProjBaComm_Detail_IsOnJob = detail[6] == "1";
            t_OfficeAutomation_Document_ProjBaComm_Detail.OfficeAutomation_Document_ProjBaComm_Detail_IsPayComm = detail[7] == "1";

            da_OfficeAutomation_Document_ProjBaComm_Detail_Inherit.Insert(t_OfficeAutomation_Document_ProjBaComm_Detail);
        }
    }
    ...
    protected void btnReWrite_Click(object sender, EventArgs e)
    {
        ...
        Session["FLG_ReWrite72"] = "1";
        Response.Write("<script>window.open('Apply_ProjBaComm_Detail.aspx?MainID=" + MainID + "');</script>");
    }
    protected void btnSPDF_Click(object sender, EventArgs e) //M_PDF
    {
        try
        {
            ...
            Response.AddHeader("content-disposition", "attachment; filename=" + HttpUtility.UrlEncode("项目部实收佣金调整申请表.pdf"));//强制下载 
    ...
    protected void btnSaveLogisticsFlow_Click(object sender, EventArgs e)
    {
        ...
        DataSet ds = new DataSet();
        ds = da_OfficeAutomation_Document_ProjBaComm_Inherit.SelectByMainID(MainID);
        ID = ds.Tables[0].Rows[0]["OfficeAutomation_Document_ProjBaComm_ID"].ToString(); //在不同的表要注意修改
        ...
        if (path != "")
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                try
                {
                    ...
                    {
                        s1 = ds.Tables[0].Rows[i]["成交报告编号"].ToString();
                        s2 = ds.Tables[0].Rows[i]["成交日期"].ToString();
                        s3 = ds.Tables[0].Rows[i]["单位地址"].ToString();
                        s4 = ds.Tables[0].Rows[i]["客户名称"].ToString();
                        s5 = ds.Tables[0].Rows[i]["退佣金额"].ToString();
                        s6 = ds.Tables[0].Rows[i]["是否坏账"].ToString() == "是" ? true : false;
                        s7 = ds.Tables[0].Rows[i]["是否已出佣"].ToString() == "是" ? true : false;

                        if (!string.IsNullOrEmpty(s1))
                        {
                            success++;
                            SbJs.Append("$('#txtDetail_pNo" + success + "').html('" + success + "');");
                            SbJs.Append("$('#txtReportNo" + success + "').val('" + s1 + "');");
                            SbJs.Append("$('#txtDealDate" + success + "').val('" + s2.Remove(s2.IndexOf(' ')) + "');");
                            SbJs.Append("$('#txtAddress" + success + "').val('" + s3 + "');");
                            SbJs.Append("$('#txtBname" + success + "').val('" + s4 + "');");
                            SbJs.Append("$('#txtBackMoney" + success + "').val('" + s5 + "');");
                            SbJs.Append("$('#rdoIsOnJob" + success + (s6 ? "1" : "0") + "').attr('checked','checked');");
                            SbJs.Append("$('#rdoIsPayComm" + success + (s7 ? "1" : "0") + "').attr('checked','checked');");
                        }
                        if (s1 == "退佣金额合计：")
                            txtMoneyCount.Text = s5;
                    }
    ...
    protected void btnCancelSign_Click(object sender, EventArgs e) //20141202：CancelSign
    {
        try
        {
            ...
            string signEmployeeName = EmployeeName;
            DA_OfficeAutomation_Document_ProjBaComm_Inherit da_OfficeAutomation_Document_ProjBaComm_Inherit = new DA_OfficeAutomation_Document_ProjBaComm_Inherit();
            DataSet ds = da_OfficeAutomation_Document_ProjBaComm_Inherit.SelectByMainID(MainID); //在不同的表中要修改
            DataRow drRow = ds.Tables[0].Rows[0];
            string apply = ds.Tables[0].Rows[0]["OfficeAutomation_Document_ProjBaComm_Apply"].ToString();
            string employname;
            string serialNumber = drRow["OfficeAutomation_SerialNumber"].ToString();
    ...
    protected void btnEditFlow_DoClick(object sender, EventArgs e)
    {
        try
        {
            ...
            //在不同的表中要修改
            RunJS("var win=window.showModalDialog(\"Apply_ProjBaComm_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=830px;dialogHeight=320px\");if(win=='success'){window.location='/Ajax/Editor_Flow.ashx?MainID=" + MainID + "&applyUrl=" + applyUrl + "&apply=" + apply + "&serialNumber=" + serialNumber + "&department=" + department + "&Jhref=" + Page.Request.Url + "&mid=" + EmployeeID + "'; }");
    ...
    {
        DA_OfficeAutomation_Flow_Inherit da_OfficeAutomation_Flow_Inherit = new DA_OfficeAutomation_Flow_Inherit();
        DA_OfficeAutomation_Document_ProjBaComm_Inherit da_OfficeAutomation_Document_BadDebts_Inherit = new DA_OfficeAutomation_Document_ProjBaComm_Inherit();
    ...
    protected void btnSignCompelete_Click(object sender, EventArgs e)
    {
        try
        {
            DA_OfficeAutomation_Document_ProjBaComm_Inherit da_OfficeAutomation_Document_ProjBaComm_Inherit = new DA_OfficeAutomation_Document_ProjBaComm_Inherit();
            if (EmployeeID == "3575" || EmployeeID == "24962" || EmployeeID == "27997" || EmployeeID == "41960")
                da_OfficeAutomation_Document_ProjBaComm_Inherit.AddRemarkByID_II(ID, "（已完成）");
            Alert("标记成功。");
        }
        catch
        {
