...
public partial class Apply_HousingFundBaseAdjust_Apply_HousingFundBaseAdjust_Detail :  BasePage
{
    #region 变量声明及定义
    bool tpdf = false;
    ...
    public StringBuilder SbFlow = new StringBuilder();//2016923-52100
    #endregion
    ...
    public void NewPage()
    {
        ...
        this.ApplyID.Value = EmployeeID;

        this.PayMonth.Text = DateTime.Now.ToString("yyyy");
    ...
    public void LoadPage()
    {
        ...
        var HousingFundChangeBLL = new DA_OfficeAutomation_Document_HousingFundBaseAdjust_Operate();
        ...
        DataSet dsHousingFundAdjst = new DataSet();
        string where = "[OfficeAutomation_Main_ID]='" + MainID + "'";
        var Mainobj = da_OfficeAutomation_Main_Inherit.GetModel(where);
        //从数据库中读取
        dsHousingFundAdjst = HousingFundChangeBLL.SelectByMainID(MainID);
        string HousingFundChangeID = dsHousingFundAdjst.Tables[0].Rows[0]["OfficeAutomation_Document_HousingFundBaseAdjust_ID"].ToString();
        string applicant = dsHousingFundAdjst.Tables[0].Rows[0]["OfficeAutomation_Document_HousingFundBaseAdjust_Apply"].ToString();       //申请人
        this.Apply.Text = applicant;
        this.ApplyDate.Text = Mainobj.OfficeAutomation_Main_ApplyDate.ToString();
        this.litSerialNumber.Text = dsHousingFundAdjst.Tables[0].Rows[0]["OfficeAutomation_SerialNumber"].ToString();

        var obj = ECOA.Common.SerializationHelper.GetEntity<T_OfficeAutomation_Document_HousingFundAdjustment>(dsHousingFundAdjst.Tables[0]);  //DataSet 转 实体
        CommonSerialization.BindObjectToControl("t_OfficeAutomation_Document_HousingFundBaseAdjust", obj, this.Page.Master.FindControl("ContentPlaceHolder1"));   //实体给form赋值

        var detailbll = new DataAccess.Operate.DA_OfficeAutomation_Document_HousingFundBaseAdjust_Detail_Operate();
        var a = detailbll.SelectListByMainID(MainID);
        var detaillist = a.Select(m => new
        {
            Pos = m.OfficeAutomation_Document_HousingFundBaseAdjust_Detail_Pos,
            Code = m.OfficeAutomation_Document_HousingFundBaseAdjust_Detail_Code,
            Dep = m.OfficeAutomation_Document_HousingFundBaseAdjust_Detail_Dep,
            Grade = m.OfficeAutomation_Document_HousingFundBaseAdjust_Detail_Grade,
            InDate = m.OfficeAutomation_Document_HousingFundBaseAdjust_Detail_InDate,
            Name = m.OfficeAutomation_Document_HousingFundBaseAdjust_Detail_Name,
            PayMonth = m.OfficeAutomation_Document_HousingFundBaseAdjust_Detail_PayMonth,
            BaseText = m.OfficeAutomation_Document_HousingFundBaseAdjust_Detail_BaseText,
            ChoiceBase = m.OfficeAutomation_Document_HousingFundBaseAdjust_Detail_ChoiceBase,
          //  PayRate = m.OfficeAutomation_Document_HousingFundBaseAdjust_Detail_PayRate,
            Reason = m.OfficeAutomation_Document_HousingFundBaseAdjust_Detail_Reason,
        });
        this.hidHousingFundChange_Detail.Value = JsonConvert.SerializeObject(detaillist);
        #endregion
    ...
    protected void btnReWrite_Click(object sender, EventArgs e)
    {
        ...
        Response.Write("<script>window.open('Apply_HousingFundBaseAdjust_Detail.aspx?MainID=" + MainID + "');</script>");
    }
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        var t_HousingFundBaseAdjust = new T_OfficeAutomation_Document_HousingFundBaseAdjust();
        var t_OfficeAutomation_Flow = new T_OfficeAutomation_Flow();

        var da_OfficeAutomation_Main_Inherit = new DA_OfficeAutomation_Main_Inherit();
        var da_HousingFundBaseAdjustBLL = new DA_OfficeAutomation_Document_HousingFundBaseAdjust_Operate();
        ...
        try
        {
            if (IsNewApply)
            {
                ...
                t_OfficeAutomation_Main.OfficeAutomation_SerialNumber = "HFundBaseAdjust" + DateTime.Now.ToString("yyyyMMddHHmmssfffffff");
                t_OfficeAutomation_Main.OfficeAutomation_DocumentID = 95;
                t_HousingFundBaseAdjust.OfficeAutomation_Document_HousingFundBaseAdjust_MainID = new Guid(MainID);
                ...
                t_HousingFundBaseAdjust = GetModelFromPage(DetailID);

                var detail = InsertDetail(this.hidHousingFundChange_Detail.Value, DetailID);

                t_OfficeAutomation_Main.OfficeAutomation_Main_Apply = EmployeeName;
                t_OfficeAutomation_Main.OfficeAutomation_Main_Department = this.Department.Value;
                t_OfficeAutomation_Main.OfficeAutomation_Main_Summary = detail;
                ...
                da_HousingFundBaseAdjustBLL.Add(t_HousingFundBaseAdjust);//插入申请表
                ...
                Common.AddLog(EmployeeID, EmployeeName, 95, new Guid(MainID), 1);//日志，创建申请表
                ...
                string msnBody = "您好，邱超琼：您有编号为" + t_OfficeAutomation_Main.OfficeAutomation_SerialNumber + "的" + documentName + "需要您的审批。<br />申请表地址为：" + applyUrl + "<br /><br /><span style=\"font-weight: bolder\">注意：该链接需要登陆门户系统并打开电子审批系统后才能进入，否则只能跳到登陆页面。</span>";

                Common.SendMessageEX(true, documentName, "邱超琼", "请审批", msnBody, msnBody, MainID);
                #endregion
            }
            else
            {
    ...
    protected void btnSPDF_Click(object sender, EventArgs e) //M_PDF
    {
        try
        {
            ...
            Response.AddHeader("content-disposition", "attachment; filename=" + HttpUtility.UrlEncode("公积金缴存基数年度调整申请表(营业部).pdf"));//强制下载 
    ...
    protected void btnEditFlow_DoClick(object sender, EventArgs e)
    {
        try
        {
            ...
            string email;
            string msnBody;
            var HousingFundAdjustBLL = new DA_OfficeAutomation_Document_HousingFundBaseAdjust_Detail_Operate();
            DataSet ds = HousingFundAdjustBLL.SelectByMainID(MainID); //在不同的表中要修改
            DataRow drRow = ds.Tables[0].Rows[0];
            string apply = ds.Tables[0].Rows[0]["OfficeAutomation_Document_HousingFundBaseAdjust_Apply"].ToString();
            string employname;
            string serialNumber = drRow["OfficeAutomation_SerialNumber"].ToString();
            string documentName = da_OfficeAutomation_Flow_Inherit.GetDocumentNameByMainID(MainID);
            string department = drRow["OfficeAutomation_Document_HousingFundBaseAdjust_Department"].ToString();
            string applyUrl = Page.Request.Url.ToString();
            applyUrl = applyUrl.Substring(applyUrl.IndexOf("/Apply/"));
            applyUrl = "内部网地址：" + System.Configuration.ConfigurationSettings.AppSettings["InternalURL"] + applyUrl;
            ...
            //在不同的表中要修改
          //  RunJS("var win=window.showModalDialog(\"Apply_HousingFundChange_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=830px;dialogHeight=280px\");if(win=='success'){window.location='/Ajax/Editor_Flow.ashx?MainID=" + MainID + "&applyUrl=" + applyUrl + "&apply=" + apply + "&serialNumber=" + serialNumber + "&department=" + department + "&Jhref=" + Page.Request.Url + "&mid=" + EmployeeID + "'; }");
    ...
    protected void btnSign_Click(object sender, EventArgs e)
    {
        ...
        var HousingFundAdjustBLL = new DA_OfficeAutomation_Document_HousingFundBaseAdjust_Operate(); ;

        DataSet ds = HousingFundAdjustBLL.SelectByMainID(MainID);
        DataRow drRow = ds.Tables[0].Rows[0];
      //  ID = drRow["OfficeAutomation_Document_HousingFundAdjustment_ID"].ToString();
        ...
        if (Purview.Contains("OA_ITPower_002"))
        {
            try
            {
                ...
                if (isSignSuccess)
                {
                    ...
                    sbMailBody.Append("<br/><br/>申请人：" + drRow["OfficeAutomation_Document_HousingFundBaseAdjust_Apply"]);
                    sbMailBody.Append("<br/>申请表：公积金特殊申请表");
                    sbMailBody.Append("<br/>申请部门：" + drRow["OfficeAutomation_Document_HousingFundBaseAdjust_Department"]);
                    sbMailBody.Append("<br/>申请日期：" + DateTime.Parse(drRow["OfficeAutomation_Document_HousingFundBaseAdjust_ApplyDate"].ToString()).ToString("yyyy-MM-dd"));
    ...
    private T_OfficeAutomation_Document_HousingFundBaseAdjust GetModelFromPage(Guid HousingFundBaseAdjustID)
    {
        var obj = new T_OfficeAutomation_Document_HousingFundBaseAdjust();
        obj = CommonSerialization.ReqDeserilizeAnEntity<T_OfficeAutomation_Document_HousingFundBaseAdjust>("t_OfficeAutomation_Document_HousingFundBaseAdjust", this.Page.Master.FindControl("ContentPlaceHolder1"));
        obj.OfficeAutomation_Document_HousingFundBaseAdjust_MainID = new Guid(MainID);
        obj.OfficeAutomation_Document_HousingFundBaseAdjust_ID = HousingFundBaseAdjustID;
        obj.OfficeAutomation_Document_HousingFundBaseAdjust_ApplyDate = obj.OfficeAutomation_Document_HousingFundBaseAdjust_ApplyDate == DateTime.MinValue ? DateTime.Now : obj.OfficeAutomation_Document_HousingFundBaseAdjust_ApplyDate;
        obj.OfficeAutomation_Document_HousingFundBaseAdjust_Apply = string.IsNullOrEmpty(obj.OfficeAutomation_Document_HousingFundBaseAdjust_Apply) ? EmployeeName : obj.OfficeAutomation_Document_HousingFundBaseAdjust_Apply;
        return obj;
    }
    ...
    {
        var obj = new T_OfficeAutomation_Document_HousingFundBaseAdjust();
        var HousingFundBaseAdjustBLL = new DA_OfficeAutomation_Document_HousingFundBaseAdjust_Operate();

        DataSet ds = new DataSet();
        ds = HousingFundBaseAdjustBLL.SelectByMainID(MainID);
        ID = ds.Tables[0].Rows[0]["OfficeAutomation_Document_HousingFundBaseAdjust_ID"].ToString();

        obj = GetModelFromPage(new Guid(ID));
        var detail = InsertDetail(this.hidHousingFundChange_Detail.Value, new Guid(ID));

        string apply = "";
        string depname = this.Department.Value;
        string summary = detail;
        string applydate = "";
        string mainid = MainID;

        new DA_OfficeAutomation_Main_Inherit().UpdateMain(mainid, depname, apply, applydate, summary);
        HousingFundBaseAdjustBLL.Edit(obj);//修改申请表

        

        Common.AddLog(EmployeeID, EmployeeName, 95, new Guid(MainID), 2);//日志，修改申请表
    }
    ...
    /// <returns></returns>
    private string InsertDetail(string json, Guid DetailID)
    {
        var t = new { Name = "", Dep = "", Code = "", InDate = "", Pos = "", Grade = "", PayMonth = "", BaseText = "", ChoiceBase = "", Reason = "" };
        var l = Enumerable.Repeat(t, 1).ToList();
        var list = JsonConvert.DeserializeAnonymousType(json, l);
        var result = "";
        var bll = new DataAccess.Operate.DA_OfficeAutomation_Document_HousingFundBaseAdjust_Detail_Operate();
        bll.DeleteByMainID(MainID);
        foreach (var i in list)
        {
            var obj = new T_OfficeAutomation_Document_HousingFundBaseAdjust_Detail
            {

                OfficeAutomation_Document_HousingFundBaseAdjust_Detail_Addtime = DateTime.Now,
                OfficeAutomation_Document_HousingFundBaseAdjust_Detail_Code = i.Code,
                OfficeAutomation_Document_HousingFundBaseAdjust_Detail_Dep = i.Dep,
                OfficeAutomation_Document_HousingFundBaseAdjust_Detail_Grade = i.Grade,
                OfficeAutomation_Document_HousingFundBaseAdjust_Detail_ID = Guid.NewGuid(),
                OfficeAutomation_Document_HousingFundBaseAdjust_Detail_InDate = i.InDate,
                OfficeAutomation_Document_HousingFundBaseAdjust_Detail_MainID = DetailID,
                OfficeAutomation_Document_HousingFundBaseAdjust_Detail_Name = i.Name,
                OfficeAutomation_Document_HousingFundBaseAdjust_Detail_PayMonth = i.PayMonth,
                OfficeAutomation_Document_HousingFundBaseAdjust_Detail_ChoiceBase = i.ChoiceBase,
                OfficeAutomation_Document_HousingFundBaseAdjust_Detail_BaseText = i.BaseText,
                OfficeAutomation_Document_HousingFundBaseAdjust_Detail_Pos = i.Pos,
                OfficeAutomation_Document_HousingFundBaseAdjust_Detail_Reason = i.Reason
            };
            result += i.Name + ",";
            bll.Add(obj);
        }
