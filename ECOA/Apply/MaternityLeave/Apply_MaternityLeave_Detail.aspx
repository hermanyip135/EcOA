<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Apply_MaternityLeave_Detail.aspx.cs"  MasterPageFile="~/MasterPage.master" Inherits="Apply_MaternityLeave_Apply_MaternityLeave_Detail" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function check() {
            ...
            var $eDate = $("#<%=txtApplyForDate.ClientID %>");
            if ($.trim($eDate.val()) == "") {
                alert("申请日期不可为空。");
                $eDate.focus();
                return false;
            ...
            if (!$("#<%=rbtType1.ClientID %>").prop("checked") && !$("#<%=rbtType2.ClientID %>").prop("checked")
                ...
                && !$("#<%=rbtType9.ClientID %>").prop("checked")
                ) {
                alert("请选择请假类型。");
            ...
            if ($.trim($("#<%=txtBeSSDate.ClientID %>").val()) == "") {
                alert("请假时限开始时不可为空。");
                $("#<%=txtBeSSDate.ClientID %>").focus();
                return false;
            }
            if ($.trim($("#<%=txtBeMMDate.ClientID %>").val()) == "") {
                alert("请假时限开始分不可为空。");
                $("#<%=txtBeMMDate.ClientID %>").focus();
                return false;
            }
            ...
            if ($.trim($("#<%=txtEndSSDate.ClientID %>").val()) == "") {
                alert("请假时限结束时不可为空。");
                $("#<%=txtEndSSDate.ClientID %>").focus();
                return false;
            }
            if ($.trim($("#<%=txtEndMMDate.ClientID %>").val()) == "") {
                alert("请假时限结束分不可为空。");
                $("#<%=txtEndMMDate.ClientID %>").focus();
                return false;
            }
...
<%--                      <input id="txtBeYYYY" type="text" runat="server" class="threeRowText" />年
                      <input id="txtBeMM" type="text" runat="server" class="threeRowText" />月
                      <input id="txtBeDD" type="text" runat="server" class="threeRowText" />日
                      <input id="txtBeSS" type="text" runat="server" class="threeRowText" />时
                      <input id="txtBeM" type="text" runat="server" class="threeRowText" />分至
                      <input id="txtEndYYYY" type="text" runat="server" class="threeRowText" />年
                      <input id="txtEndMM" type="text" runat="server" class="threeRowText" />月
                      <input id="txtEndDD" type="text" runat="server" class="threeRowText" />日
                      <input id="txtEndSS" type="text" runat="server" class="threeRowText" />时
                      <input id="txtEndM" type="text" runat="server" class="threeRowText" />分--%>
                    </td>
                </tr>
                <tr>
                            ...
                            <tr class="noborder" style="height: 85px;" idx="20">
                                ...
                                <td class="tl PL10">
                                    ...
                                    <div>
                                        ...
                                        <asp:radiobutton id="rbtSalary1" runat="server" groupname="rbtSalary" text="有薪" value="1" />
                                        <asp:radiobutton id="rbtSalary2" runat="server" groupname="rbtSalary" text="扣薪" value="2" />
                                         <asp:radiobutton id="rbtFullDate1" runat="server" groupname="rbtFullDate" text="全勤" value="1" />
                                        <asp:radiobutton id="rbtFullDate2" runat="server" groupname="rbtFullDate" text="非全勤" value="2" />
                                    </div>
