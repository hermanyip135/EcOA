...
public partial class Apply_SolHold_Apply_SolHold_Detail : BasePage
{
    #region 变量声明及定义
    bool tpdf = false;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            try
            {
                if (Session["FLG_ReWrite78"].ToString() == "1")
                {
                    ViewState["FLG_ReWrite"] = "1";
                    Session["FLG_ReWrite78"] = null;
                }
            }
            catch
            {
    ...
    public void NewPage()
    {
        ...
        ApplyDate.Text = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
    ...
    public void LoadPage()
    {
        ...
        ds = da_OfficeAutomation_Document_SolHold_Inherit.SelectByMainID(MainID);
        DataRow dr = ds.Tables[0].Rows[0];
        ID = dr["OfficeAutomation_Document_SolHold_ID"].ToString();
        string applicant = dr["OfficeAutomation_Document_SolHold_Apply"].ToString();
        ApplyN = applicant;
        this.litSerialNumber.Text = dr["OfficeAutomation_SerialNumber"].ToString();

        var obj = ECOA.Common.SerializationHelper.GetEntity<T_OfficeAutomation_Document_SolHold>(ds.Tables[0]);  //DataSet 转 实体
        CommonSerialization.BindObjectToControl("t_OfficeAutomation_Document_SolHold", obj, this.Page.Master.FindControl("ContentPlaceHolder1"));   //实体给form赋值

        this.txtRemark.Text = obj.OfficeAutomation_Document_SolHold_Remark;
        if (obj.OfficeAutomation_Document_SolHold_RemarkDate != null)//2016-8-1
        {
            this.txtRemarkDate.InnerText = ((DateTime)obj.OfficeAutomation_Document_SolHold_RemarkDate).ToString("yyyy-MM-dd HH:mm:ss");
        }
        ...
        //签名列表
        var flowsignlist = da_OfficeAutomation_Flow_Inherit.GetFlowsSignList(ds, EmployeeID, EmployeeName);
        if (flowsignlist != null)
        {
            this.hidSignFlowsJson.Value = Newtonsoft.Json.JsonConvert.SerializeObject(flowsignlist);
            
            //已经通过申请 || 未通过申请，但已经到Idx=5（到法律部） 
            var o = flowsignlist.Find(m => (m.Audit && m.Idx == 5) || (m.Idx == 5 && m.SignbtnShow));
            
            //登录人是Idx=5流程的审核人之一(流程未审核时)
            if (o != null && !o.Audit && (o.EmployeeID + ",").Contains(EmployeeID + ","))
            {
                //显示最后的完成结果填写
                //this.btnFin.Visible = true;
            }
            else if (o != null && o.Audit && o.Auditors.Find(m => m.Code == EmployeeID) != null)
            {
                //流程已通过审核 && 审核人登录后
                //显示最后的完成结果填写
                this.btnFin.Visible = true;
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        T_OfficeAutomation_Document_SolHold t_OfficeAutomation_Document_SolHold = new T_OfficeAutomation_Document_SolHold();
        ...
        try
        {
            ...
            if (IsNewApply)
            {
                ...
                t_OfficeAutomation_Main.OfficeAutomation_SerialNumber = "SolHold" + DateTime.Now.ToString("yyyyMMddHHmmssfffffff");
                t_OfficeAutomation_Main.OfficeAutomation_DocumentID = 73; //在《申请表字典表》t_Dic_OfficeAutomation_Document，找到该表的主键，这步一错则会打开别的表
                ...
                t_OfficeAutomation_Document_SolHold = GetModelFromPage(Guid.NewGuid());
                ...
                da_OfficeAutomation_Document_SolHold_Inherit.Add(t_OfficeAutomation_Document_SolHold);//插入申请表

                //根据默认流程表中的固定环节添加流程
                da_OfficeAutomation_Flow_Inherit.InsertDefaultFlow(t_OfficeAutomation_Main.OfficeAutomation_Main_ID.ToString());

                Common.AddLog(EmployeeID, EmployeeName, 77, new Guid(MainID), 1);//日志，创建申请表
                Common.AddLog(EmployeeID, EmployeeName, 3, new Guid(MainID), 1);//日志，创建流程

                RunJS("window.showModalDialog(\"/Apply/Apply_UploadFile.aspx?MainID=" + MainID + "\", '" + MainID + "', \"dialogHeight=165px\");var win=window.showModalDialog(\"Apply_SolHold_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=800px;dialogHeight=320px\");if(win=='success'){window.location='/Apply/Apply_Search.aspx'; }");
                #endregion
            }
            else
            {
    ...
    protected void btnSign_Click(object sender, EventArgs e)
    {
        ...
        DataSet ds = da_OfficeAutomation_Document_SolHold_Inherit.SelectByMainID(MainID);
        DataRow drRow = ds.Tables[0].Rows[0];
        ID = drRow["OfficeAutomation_Document_SolHold_ID"].ToString(); 
        ...
        if (Purview.Contains("OA_ITPower_002"))
        {
            try
            {
                ...
                if (isSignSuccess)
                {
                    ...
                    string apply = ds.Tables[0].Rows[0]["OfficeAutomation_Document_SolHold_Apply"].ToString();
                    string employname;
                    string serialNumber = drRow["OfficeAutomation_SerialNumber"].ToString();
                    string documentName = da_OfficeAutomation_Flow_Inherit.GetDocumentNameByMainID(MainID);
                    string department = drRow["OfficeAutomation_Document_SolHold_Department"].ToString();
                    ...
                    sbMailBody.Append("<br/><br/>申请人：" + drRow["OfficeAutomation_Document_SolHold_Apply"]);
                    sbMailBody.Append("<br/>申请分行/部门：" + drRow["OfficeAutomation_Document_SolHold_Department"]);
                    sbMailBody.Append("<br/>填写日期：" + DateTime.Parse(drRow["OfficeAutomation_Document_SolHold_ApplyDate"].ToString()).ToString("yyyy-MM-dd"));
                    sbMailBody.Append("<br/>经办人：" + drRow["OfficeAutomation_Document_SolHold_ApplyID"]);
                    sbMailBody.Append("<br/>联络方式：" + drRow["OfficeAutomation_Document_SolHold_Phone"]);
                    sbMailBody.Append("<br/>解HOLD物业地址：" + drRow["OfficeAutomation_Document_SolHold_Address"]);

                    sbMailBody.Append("<br/>解HOLD佣金金额：" + drRow["OfficeAutomation_Document_SolHold_Money"]);
                    sbMailBody.Append("<br/>HOLD原因（或欠交资料）：" + drRow["OfficeAutomation_Document_SolHold_Reason"]);
                    sbMailBody.Append("<br/>解HOLD理由：" + drRow["OfficeAutomation_Document_SolHold_Summary"]);
    ...
    protected void btnSignSave_Click(object sender, EventArgs e)
    {
        ...
        //        da_OfficeAutomation_Document_SolHold_Inherit.AddRemarkByID(ID, CommonConst.SIGN_FINANCE);
        //    Alert("标记成功。");
        //}
        //catch
        //{
    ...
    private T_OfficeAutomation_Document_SolHold GetModelFromPage(Guid ID)
    {
        var obj = new T_OfficeAutomation_Document_SolHold();
        obj = CommonSerialization.ReqDeserilizeAnEntity<T_OfficeAutomation_Document_SolHold>("t_OfficeAutomation_Document_SolHold", this.Page.Master.FindControl("ContentPlaceHolder1"));
        obj.OfficeAutomation_Document_SolHold_MainID = new Guid(MainID);
        obj.OfficeAutomation_Document_SolHold_ID = ID;
        obj.OfficeAutomation_Document_SolHold_ApplyDate = obj.OfficeAutomation_Document_SolHold_ApplyDate == DateTime.MinValue ? DateTime.Now : obj.OfficeAutomation_Document_SolHold_ApplyDate;
        obj.OfficeAutomation_Document_SolHold_Apply = string.IsNullOrEmpty(obj.OfficeAutomation_Document_SolHold_Apply) ? EmployeeName : obj.OfficeAutomation_Document_SolHold_Apply;
        return obj;
    }
    ...
    private void Update()
    {
        ...
        DataSet ds = new DataSet();
        ds = da_OfficeAutomation_Document_SolHold_Inherit.SelectByMainID(MainID);
        ID = ds.Tables[0].Rows[0]["OfficeAutomation_Document_SolHold_ID"].ToString();

        t_OfficeAutomation_Document_SolHold = GetModelFromPage(new Guid(ID));
        ...
        da_OfficeAutomation_Document_SolHold_Inherit.Edit(t_OfficeAutomation_Document_SolHold);//修改申请表

        Common.AddLog(EmployeeID, EmployeeName, 77, new Guid(MainID), 2);//日志，修改申请表
    }
    ...
    /// 新增明细表
    /// </summary>
    /// <param name="gSolHoldID"></param>
    //private void InsertSolHoldDetail(Guid gSolHoldID)
    //{
    //    if (hdDetail.Value == "")
    //        return;

    //    T_OfficeAutomation_Document_SolHold_Detail t_OfficeAutomation_Document_SolHold_Detail;
    //    DA_OfficeAutomation_Document_SolHold_Detail_Inherit da_OfficeAutomation_Document_SolHold_Detail_Inherit = new DA_OfficeAutomation_Document_SolHold_Detail_Inherit();
    ...
    //            if (detail[0] == "")
    //                continue;
    //            t_OfficeAutomation_Document_SolHold_Detail = new T_OfficeAutomation_Document_SolHold_Detail();
    //            t_OfficeAutomation_Document_SolHold_Detail.OfficeAutomation_Document_SolHold_Detail_ID = Guid.NewGuid();
    //            t_OfficeAutomation_Document_SolHold_Detail.OfficeAutomation_Document_SolHold_Detail_MainID = gSolHoldID;
    //            t_OfficeAutomation_Document_SolHold_Detail.OfficeAutomation_Document_SolHold_Detail_Property = detail[0];
    //            t_OfficeAutomation_Document_SolHold_Detail.OfficeAutomation_Document_SolHold_Detail_Controler = detail[1];
    //            t_OfficeAutomation_Document_SolHold_Detail.OfficeAutomation_Document_SolHold_Detail_PropertyID = detail[2];
    //            t_OfficeAutomation_Document_SolHold_Detail.OfficeAutomation_Document_SolHold_Detail_PropertyDate = detail[3];
    //            t_OfficeAutomation_Document_SolHold_Detail.OfficeAutomation_Document_SolHold_Detail_OldDeal = detail[4];
    //            t_OfficeAutomation_Document_SolHold_Detail.OfficeAutomation_Document_SolHold_Detail_NewDeal = detail[5];
    //            t_OfficeAutomation_Document_SolHold_Detail.OfficeAutomation_Document_SolHold_Detail_AjustDeal = detail[6];
    //            t_OfficeAutomation_Document_SolHold_Detail.OfficeAutomation_Document_SolHold_Detail_ShouldComm = detail[7];
    //            t_OfficeAutomation_Document_SolHold_Detail.OfficeAutomation_Document_SolHold_Detail_ActualComm = detail[8];
    //            t_OfficeAutomation_Document_SolHold_Detail.OfficeAutomation_Document_SolHold_Detail_AjustComm = detail[9];
    //            t_OfficeAutomation_Document_SolHold_Detail.OfficeAutomation_Document_SolHold_Detail_LeadReason = int.Parse(detail[10]);
    //            t_OfficeAutomation_Document_SolHold_Detail.OfficeAutomation_Document_SolHold_Detail_Commitment = detail[11];
    //            t_OfficeAutomation_Document_SolHold_Detail.OfficeAutomation_Document_SolHold_Detail_Reason = detail[12];

    //            da_OfficeAutomation_Document_SolHold_Detail_Inherit.Insert(t_OfficeAutomation_Document_SolHold_Detail);
    //        }
    //    }
    //    catch (Exception ee)
    //    {
    ...
    protected void btnSPDF_Click(object sender, EventArgs e) //M_PDF
    {
        try
        {
            ...
            Response.AddHeader("content-disposition", "attachment; filename=" + HttpUtility.UrlEncode("物业部批量解hold申请.pdf"));//强制下载 
    ...
    protected void btnCancelSign_Click(object sender, EventArgs e) //20141202：CancelSign
    {
        try
        {
            ...
            string signEmployeeName = EmployeeName;
            DA_OfficeAutomation_Document_SolHold_Inherit da_OfficeAutomation_Document_SolHold_Inherit = new DA_OfficeAutomation_Document_SolHold_Inherit();
            DataSet ds = da_OfficeAutomation_Document_SolHold_Inherit.SelectByMainID(MainID); //在不同的表中要修改
            DataRow drRow = ds.Tables[0].Rows[0];
            string apply = ds.Tables[0].Rows[0]["OfficeAutomation_Document_SolHold_Apply"].ToString();
            string employname;
            string serialNumber = drRow["OfficeAutomation_SerialNumber"].ToString();
    ...
    protected void btnEditFlow_DoClick(object sender, EventArgs e)
    {
        try
        {
            ...
            //在不同的表中要修改
            RunJS("var win=window.showModalDialog(\"Apply_SolHold_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=800px;dialogHeight=320px\");if(win=='success'){window.location='/Ajax/Editor_Flow.ashx?MainID=" + MainID + "&applyUrl=" + applyUrl + "&apply=" + apply + "&serialNumber=" + serialNumber + "&department=" + department + "&Jhref=" + Page.Request.Url + "&mid=" + EmployeeID + "'; }");
    ...
    protected void btnFin_Click(object sender, EventArgs e)
    {
        ...
        {
            RunJS("alert('请先填写处理意见');");
            return;
        }
        var bll = new DataAccess.Operate.DA_OfficeAutomation_Document_SolHold_Inherit();
        var obj = bll.GetModel("OfficeAutomation_Document_SolHold_MainID='" + MainID + "'");
        if (obj != null)
        {
            obj.OfficeAutomation_Document_SolHold_Remark = this.txtRemark.Text;
            obj.OfficeAutomation_Document_SolHold_RemarkDate = DateTime.Now;//2016-8-1
            obj.OfficeAutomation_Document_SolHold_FinText = "已处理";
            bll.Edit(obj);

            string summary = this.txtRemark.Text;
            string mainid = MainID;

            new DA_OfficeAutomation_Main_Inherit().UpdateMain(mainid, "", "", "", summary);
        ...
        LoadPage();
        //给申请人发邮件通知
        var ApplyName = this.Apply.Text;    //申请人姓名
        //var MessageBody = string.Format("您好！您的[物业部批量解HOLD申请]已由法律部{0}处理完毕，处理意见为：{1}，请查核。", EmployeeName, obj.OfficeAutomation_Document_SolHold_Remark);  //邮件内容

        //=============2016-8-1====================
        DataSet information = new DataSet();
        information = bll.SelectByMainID(MainID);
        DataRow dr = information.Tables[0].Rows[0];
        string MessageBody = "您好！您的[物业部批量解HOLD申请]已由法律部" + EmployeeName + "处理完毕，处理意见为：【" + this.txtRemark.Text + "】，请查核。<br />" +
            "申请人：" + dr["OfficeAutomation_Document_SolHold_Apply"].ToString() + "<br />" +
            "申请分行/部门：" + dr["OfficeAutomation_Document_SolHold_Department"].ToString() + "<br />" +
            "填写日期：" + dr["OfficeAutomation_Document_SolHold_ApplyDate"] + "<br />" +
            "经办人：" + dr["OfficeAutomation_Document_SolHold_ApplyID"].ToString() + "<br />" +
            "联络方式：" + dr["OfficeAutomation_Document_SolHold_Phone"].ToString() + "<br />" +
            "项目名称：" + dr["OfficeAutomation_Document_SolHold_Address"].ToString() + "<br />" +
            "解HOLD佣金金额：" + dr["OfficeAutomation_Document_SolHold_Money"].ToString() + "<br />" +
            "HOLD佣原因：" + dr["OfficeAutomation_Document_SolHold_Reason"].ToString() + "<br />" +
            "解HOLD理由：" + dr["OfficeAutomation_Document_SolHold_Summary"].ToString() + "<br />" +
            "文档编码：" + dr["OfficeAutomation_SerialNumber"].ToString() + "\n";
        //==========================================

        Common.SendMessageEX(false, ApplyName, "你的物业部批量解HOLD申请已处理", MessageBody, MessageBody);
    }
}