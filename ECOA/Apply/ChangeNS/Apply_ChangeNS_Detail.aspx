<%@ Page ValidateRequest="false" Title="网签变更、特殊申请表 - 广州中原电子审批系统" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="Apply_ChangeNS_Detail.aspx.cs" Inherits="Apply_ChangeNS_Apply_ChangeNS_Detail" %>
    ...
    <script type="text/javascript">
        ...
        $(function() {      
            ...
            if($("#<%=cbCNS1.ClientID%>").prop('checked')){
                ...
                //if(!jsontype.hasOwnProperty("4") && !SearchId("4"))
                //{
                //    jsontype["4"]="关系证明/法律部签批的变更申请";
                //    $("#spd4").show();
                //}
                    

            }
            if($("#<%=cbCNS2.ClientID%>").prop('checked')){
            ...
            if($("#<%=cbCNS3.ClientID%>").prop('checked')){
                ...
                if(!jsontype.hasOwnProperty("5") && !SearchId("5"))
                {
                    jsontype["5"]="委托公证书";
                    $("#spd5").show();
                }
            ...
            if($("#<%=cbCNS5.ClientID%>").prop('checked')){
            ...
            //arrtype[0]="存量房买卖合同变更协议";
            //arrtype[1]="网签委托确认书";
            //arrtype[2]="新买家身份证";
            //arrtype[3]="购房资格调查确认表";
            //arrtype[4]="关系证明/法律部签批的变更申请";
            //arrtype[5]="委托公证书";
            //arrtype[6]="代理人身份证资料";
            ...
            $("#<%=cbCNS1.ClientID%>").click(function(){
                ...
                if($("#<%=cbCNS1.ClientID%>").prop('checked')){
                    ...
                    //if(arrtype.indexOf("存量房买卖合同变更协议")==-1)
                    //    arrtype.splice(0,0,"存量房买卖合同变更协议")
                    //if(arrtype.indexOf("网签委托确认书")==-1)
                    //    arrtype.splice(1,0,"网签委托确认书")
                    //if(arrtype.indexOf("新买家身份证")==-1)
                    //    arrtype.splice(2,0,"新买家身份证")
                    //if(arrtype.indexOf("购房资格调查确认表")==-1)
                    //    arrtype.splice(3,0,"购房资格调查确认表")
                    //if(arrtype.indexOf("关系证明/法律部签批的变更申请")==-1)
                    //    arrtype.splice(4,0,"关系证明/法律部签批的变更申请")
                    ...
                    $.each(jsontype, function(val, text) {  
                        ...
                        if(!$("#<%=cbCNS5.ClientID%>").prop('checked') && !$("#<%=cbCNS4.ClientID%>").prop('checked') && !$("#<%=cbCNS3.ClientID%>").prop('checked') && !$("#<%=cbCNS2.ClientID%>").prop('checked') && !$("#<%=cbCNS1.ClientID%>").prop('checked'))
                    {
                        $("#spd0").hide();
                        delete jsontype["0"]
                        //arrtype.removeByValue("存量房买卖合同变更协议");
                    }
                    

                    if(!$("#<%=cbCNS4.ClientID%>").prop('checked') && !$("#<%=cbCNS2.ClientID%>").prop('checked') && !$("#<%=cbCNS1.ClientID%>").prop('checked'))
                    {
                        $("#spd1").hide();
                        delete jsontype["1"]
                        //arrtype.removeByValue("网签委托确认书");
                    }
                   

                    if(!$("#<%=cbCNS2.ClientID%>").prop('checked') && !$("#<%=cbCNS1.ClientID%>").prop('checked'))
                    {
                        $("#spd2").hide();
                        $("#spd3").hide();
                        delete jsontype["2"]
                        delete jsontype["3"]
                        // arrtype.removeByValue("新买家身份证");
                        //arrtype.removeByValue("购房资格调查确认表");
                    }

                    $("#spd4").hide();
                    delete jsontype["4"]
                    //arrtype.removeByValue("关系证明/法律部签批的变更申请");
            ...
            $("#<%=cbCNS3.ClientID%>").click(function(){
                ...
                if($("#<%=cbCNS3.ClientID%>").prop('checked')){
                    ...
                    //if(arrtype.indexOf("存量房买卖合同变更协议")==-1)
                    //    arrtype.splice(0,0,"存量房买卖合同变更协议")
                    //if(arrtype.indexOf("委托公证书")==-1)
                    //    arrtype.splice(5,0,"委托公证书")
                    //if(arrtype.indexOf("代理人身份证资料")==-1)
                    //    arrtype.splice(6,0,"代理人身份证资料")
                    ...
                    delete jsontype["5"]
                    delete jsontype["6"]
                    //arrtype.removeByValue("委托公证书");
                    //arrtype.removeByValue("代理人身份证资料");
            ...
            $("#<%=cbCNS5.ClientID%>").click(function(){
                $("#<%=ddlDataType.ClientID%> option").remove();
                $("#<%=ddlDataType.ClientID%>").append($('<option value="-1">---请选择---</option>'));
        ...
        function ClearCtb(){
            ...
            $("#<%=cbCNS5.ClientID%>").removeAttr("checked");
            $("#spd0").hide();
            $("#spd1").hide();
            $("#spd2").hide();
            $("#spd3").hide();
        ...
        function DeleteRow(t)
        {
            ...
            if(id=="0" && ($("#<%=cbCNS5.ClientID%>").prop('checked') || $("#<%=cbCNS4.ClientID%>").prop('checked') || $("#<%=cbCNS3.ClientID%>").prop('checked') || $("#<%=cbCNS2.ClientID%>").prop('checked') || $("#<%=cbCNS1.ClientID%>").prop('checked')))
            {
                jsontype[""+id+""]="存量房买卖合同变更协议";
                $("#spd0").show();
            }
            else if(id=="1" && ($("#<%=cbCNS4.ClientID%>").prop('checked') || $("#<%=cbCNS2.ClientID%>").prop('checked') || $("#<%=cbCNS1.ClientID%>").prop('checked')))
            {
                jsontype[""+id+""]="网签委托确认书";
                $("#spd1").show();
            }
            else if(id=="2" && ($("#<%=cbCNS2.ClientID%>").prop('checked') || $("#<%=cbCNS1.ClientID%>").prop('checked')))
            {
                jsontype[""+id+""]="新买家身份证";
                $("#spd2").show();
            }
            else if(id=="3" && ($("#<%=cbCNS2.ClientID%>").prop('checked') || $("#<%=cbCNS1.ClientID%>").prop('checked')))
            {
                jsontype[""+id+""]="购房资格调查确认表";
                $("#spd3").show();
            }
            <%--else if(id=="4" && ($("#<%=cbCNS1.ClientID%>").prop('checked')))
            {
                jsontype[""+id+""]="关系证明/法律部签批的变更申请";
                $("#spd4").show();
            }
            --%>
            else if(id=="5" && ($("#<%=cbCNS3.ClientID%>").prop('checked')))
            {
                jsontype[""+id+""]="委托公证书";
                $("#spd5").show();
            }
            else if(id=="6" && $("#<%=cbCNS3.ClientID%>").prop('checked'))
            {
                jsontype[""+id+""]="代理人身份证资料";
                $("#spd6").show();
            }
        ...
        function showMajordomo(e)
        {
            if(e.value =='高于成交价' || e.value =='低于成交价')
            {
                $("#isshowMajordomo").css("display", "block");
            }
            else
            {
                $("#isshowMajordomo").css("display", "none");
            }
        }
        function check() {
            //var filename="";
           ...
           <%-- if($("#<%=cbCNS1.ClientID %>").prop("checked"))
            ...
            if($("#<%=ddlDataType.ClientID%>").children().length>1 && ($("#<%=cbCNS1.ClientID %>").prop("checked") || $("#<%=cbCNS2.ClientID %>").prop("checked") || $("#<%=cbCNS3.ClientID %>").prop("checked") || $("#<%=cbCNS4.ClientID %>").prop("checked") || $("#<%=cbCNS5.ClientID %>").prop("checked")))
            {
                alert("请上传必须资料！");
                return false;
            }
        ...
        function upload() {
            ...
            if(sReturnValue=='success')
                window.location='Apply_ChangeNS_Detail.aspx?MainID=<%=MainID %>';
        }

        function editflow(){ //&dpm="+$("#<%//=txtDepartment.ClientID %>").val()+"
            var win=window.showModalDialog("Apply_ChangeNS_Flow.aspx?MainID=<%=MainID %>&flowsadd="+flowsl+"","","dialogWidth=800px;dialogHeight=320px");
            if(win=='success')
                window.location="Apply_ChangeNS_Detail.aspx?MainID=<%=MainID %>";
        }
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <div style='text-align: center; width: 840px; margin: 0 auto;'>
        ...
        <div style='text-align: center'>
            ...
            <table id="tbAround" width='640px'>
                <tr>
                    <td class="tl PL10" colspan="4">致：汇瀚网签部
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    <td colspan="4" style="padding: 10px; text-align: left; line-height: 25px;">申请事项：（必填，不填不接受申请，特殊申请和网签变更只能选择其一）<br />
                        ...
                        <asp:DropDownList ID="ddlPayWay" runat="server" Width="100px" AutoPostBack="True" OnSelectedIndexChanged="ddlPayWay_SelectedIndexChanged">
                            <asp:ListItem>请选择</asp:ListItem>
                            <asp:ListItem>一次性</asp:ListItem>
                            <asp:ListItem>按揭</asp:ListItem>
                        </asp:DropDownList><br />
                        <span style="font-weight: bold">★必须保证一定会在三天内归还有买卖双方签名的网签版《中介服务合同》和《存量房买卖合同》、递件受理回执各壹份到汇瀚网签部，否则愿意接受HOLD佣处理。如取走合同两个月内未归还，一律视为遗失合同，愿意接受公司违规通告的处罚。
                        </span>
                        <br />
                        ...
                        <br />
                       <%--买卖双方确认网签成交价--%>
                        <asp:CheckBox ID="rdbSpecialApply4" runat="server" GroupName="SpecialApply" Text="（4）买卖双方确认,该物业综合交易价格扣除溢价、交易成本、装修家电(如有)等费用之后,楼价为" OnCheckedChanged="rdbSpecialApply1_CheckedChanged" />
                        <asp:TextBox ID="txtSurePrice" runat="server"></asp:TextBox>元　
                     <%--   与成交价对比：
                        <asp:DropDownList ID="ddlCompareP" runat="server" Width="150px">
                            <asp:ListItem>请选择</asp:ListItem>
                            <asp:ListItem>高于成交价</asp:ListItem>
                               <asp:ListItem>低于成交价</asp:ListItem>
                             </asp:DropDownList>--%>
                          <%--  SYSREQ201901071546557337915 --%>
                            <%--<asp:ListItem>低于成交价5成</asp:ListItem>
                            <asp:ListItem>低于成交价7成</asp:ListItem>--%>
                            
                        <%--    <asp:ListItem>与成交价一致</asp:ListItem>--%>
                          
                            <%--SYSREQ201812271637430742709--%> 
                            <%--<asp:ListItem>其它情况</asp:ListItem>--%>
                       
                        <br />
                        <span style=" font-family:SimSun; font-weight:bold; color:red">注意：此申请需提交至区域负责人审批，否则汇瀚网签部将不受理此申请。</span><br/>
                        ...
                        <asp:DropDownList ID="ddlRelationship" runat="server" Width="100px">
                            ...
                            <br />
                        <asp:CheckBox ID="cbCNS2" runat="server" Text="（2）加人名、减人名" /><br />
                        <asp:CheckBox ID="cbCNS3" runat="server" Text="（3）代理人变更" /><br />
                        <asp:CheckBox ID="cbCNS4" runat="server" Text="（4）报价变更" />：
                        ...
                        变更后报价与成交价对比：
                        <asp:DropDownList ID="ddlCompareOfChange" runat="server" Width="200px" onchange="showMajordomo(this)">
                           ...
                           <%-- <asp:ListItem>低于成交价5成（含5成）</asp:ListItem>
                            ...
                            <%--<asp:ListItem>其他情况</asp:ListItem>--%>
                             ...
                             <span id="isshowMajordomo" style=" font-family:SimSun; font-weight:bold;color:red"  >注意：此申请需提交至区域负责人审批，否则汇瀚网签部将不受理此申请。</span>
                        <asp:CheckBox ID="cbCNS5" runat="server" Text="（5）其他变更" /><br />
                            </div>
                        ...
                        <%--<span id="spd4" style="display: none">关系证明/法律部签批的变更申请，
                        </span>  --%>               
                         <span id="spd5" style="display: none">委托公证书，
                        </span>
                         <span id="spd6" style="display: none">代理人身份证资料，
                        </span>


                       <%-- <span id="Span0" style="display: none">存量房买卖合同变更协议
                        </span>
                        <span id="spd4" style="display: none">，网签委托确认书
                        </span>
                        <span id="spd2" style="display: none">，新买家身份证，
                            购房资格调查确认表
                        </span>
                        <span id="spd1" style="display: none">，关系证明/法律部签批的变更申请
                        </span>
                        <span id="spd3" style="display: none">，委托公证书，
                            代理人身份证资料
                        </span>--%>
                ...
                <tr id="traddflow" style="display:none">
                    <td style="text-align: left; padding-left: 10px;" colspan="4">
                        <div class="flow">分行主管：<input type="text" id="txtidx1" style="width:190px;border: 1px solid #98b8b5; margin-right:20px" />区域经理：<input type="text" id="txtidx3" style="width:190px;border: 1px solid #98b8b5;" /></div>
                        <div class="flow">区域总监：<input type="text" id="txtidx4" style="width:190px;border: 1px solid #98b8b5; margin-right:20px" />区域负责人：<input type="text" id="txtidx5" style="width:190px;border: 1px solid #98b8b5;" /></div>
                        <div class="flow"> 法律部：<input type="text" id="txtidx6" style="width:190px;border: 1px solid #98b8b5; margin-right:20px" />汇瀚营运总经理：<input type="text" id="txtidx10" style="width:190px;border: 1px solid #98b8b5;" /></div>
                ...
                <tr id="trM9" class="noborder" style="height: 85px; display: none;">
                    <td class="auto-style4">汇瀚服务中心主管</td>
                    <td colspan="3" class="tl PL10" style="">
                ...
                <tr id="trM10" class="noborder" style="height: 85px; display: none;">
                    <td class="auto-style4">汇瀚营运总经理</td>
                    <td colspan="3" class="tl PL10" style="">
                  ...
                  <tr id="trCOO" class="noborder" style="height: 85px; display: none;">
                    ...
                    <td colspan="3" class="tl PL10" style="">
                        ...
                        <label for="rdbNoIDx20">不同意</label><br />
                        <textarea id="txtIDx20" rows="3" style="width: 98%; overflow: auto;" cols="20" name="S30"></textarea>
    ...
    <script type="text/javascript">
        $.each($("textarea:not([id*=txtDescribe])"), function (idx, item) { autoTextarea(this); }); //使对话框自适应
        var selectCompareOfChange =$("#<%=ddlCompareOfChange.ClientID %>").val();
        if(selectCompareOfChange == '高于成交价' || selectCompareOfChange == '低于成交价')
        {
            $("#isshowMajordomo").css("display", "block");
        }
        else
