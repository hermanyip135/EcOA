...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    <script type="text/javascript">
        ...
        $(function() {      
            if(!isNewApply){
                $("#<%=hdDetail.ClientID%>").val(2);
            }
            else
                $("#<%=hdDetail2.ClientID%>").val('<%=EmployeeName %>');
            if($("#<%=hdDetail2.ClientID%>").val() == '')
                $("#<%=hdDetail2.ClientID%>").val('<%=ApplyN %>');
            ...
            //if(new Date($("#<%=txtApplyDate.ClientID%>").val().replace(/\-/g, "\/")) > new Date('<%=CommonConst.ASSET_OLD_TIME%>'.replace(/\-/g, "\/")))
                //$("#<%=txtExportPlace.ClientID %>").css("background-color","#e3e3e3").attr("readonly","readonly");
        ...
        function check() {
            if('<%=EmployeeID%>' != '18331' && $("#<%=hdDetail.ClientID%>").val()==""){
                alert("请选择相关的资产。");
                $("#btnSelect").focus();
                return false;
            }
        ...
        function AlterAsset(){  //M_NewTable：20151202
            var sReturnValue = window.open('Apply_DetailAsset.aspx?Data='
                +$('#<%=hdDetail.ClientID%>').val()
                +'&EmpName='+escape($('#<%=hdDetail2.ClientID%>').val())
                +'&Asset_Dpm='+escape($('#<%=txtExportDepartment.ClientID%>').val())
        ...
        function SelectAsset() { //20150519：M_AssetSelt
            ...
            var url = "Apply_zAsset.aspx?mainID=<%=MainID %>&Asset_Dpm=" 
                + escape($.trim($("#<%=txtExportDepartment.ClientID %>").val())) 
                +'&url_DpmRec='+escape($('#<%=txtImportDepartment.ClientID%>').val())
                +'&url_PlaceRec='+escape($('#<%=txtImportPlace.ClientID%>').val())
                +"&Data="+$("#<%=hdDetail.ClientID%>").val();
            var sReturnValue = window.showModalDialog(url, "", "dialogHeight="+screen.height+"px;dialogWidth="+screen.width+"px;");
            if(sReturnValue == 'AddSuccess'){
                $("#DetailAsset").show();
                $("#DetailAsset").css("margin-top","30px");
                $("#<%=hdDetail.ClientID%>").val(1);
            ...
            <table id="tbAround" width='640px'>
                ...
                <tr>
                    <td colspan="2">
                        ...
                        <div id="DetailAsset" style="text-align: left; margin-left: 10px; display: none; margin-top: 10px;">
                            详见：<a onclick="return AlterAsset();" class="sp" target="_blank" style="font-size: 20px">调动资产明细表</a>
<%--<a href="Apply_DetailAsset.aspx?Data='+$('#<%=hdDetail.ClientID%>').val()+'&EmpName='+escape('<%=ApplyN %>')" class="sp" target="_blank" style="font-size: 20px">调动资产明细表</a>--%>
                        </div>
        ...
        <asp:UpdatePanel ID="UpdatePanel1" runat="server">
            ...
            <input type="hidden" id="hdDetail" runat="server" value="" />
