<%@ Page ValidateRequest="false" Title="项目报数申请表(物业部加签) - 广州中原电子审批系统" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="Apply_ProjReDaAdd_Detail.aspx.cs" Inherits="Apply_ProjReDaAdd_Apply_ProjReDaAdd_Detail" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
        ...
        $(function() {      
            ...
            $("#<%=txtAreaMaster.ClientID%>").val($(this).find("option:selected").val());
            $("#<%=hdAreaMaster.ClientID%>").val($(this).find("option:selected").val());
            $("#ddlAreaProj").change(function(){
                $("#<%=txtAreaMaster.ClientID%>").val($(this).find("option:selected").val());
                $("#<%=txtTurnsAgentXX1.ClientID%>").val($(this).find("option:selected").text());
                $("#<%=hdAreaMaster.ClientID%>").val($(this).find("option:selected").val());
                $("#<%=hdTurnsAgentXX1.ClientID%>").val($(this).find("option:selected").text());
            });
        ...
        function check() {
            ...
            $("#ctl00_ContentPlaceHolder1_cblDealOfficeType input[type=checkbox]").each(function(){
                if(this.checked)
                {
                    flag = true;
                    typeValues += $(this).next().text() + "|";
                }
            });
            if (!flag) {
                alert("请选择物业性质！");
            ...
            if (!$("#<%=rdbAgentModel1.ClientID %>").prop("checked") && !$("#<%=rdbAgentModel2.ClientID %>").prop("checked")) { alert("请选择代理模式"); return false; }
            if (!$("#<%=rdbNewProj.ClientID %>").prop("checked") && !$("#<%=rdbOldProj.ClientID %>").prop("checked")) { alert("请选择项目类型");$("#<%=rdbNewProj.ClientID %>").focus(); return false; }
            ...
            if ($("#<%=rdbJOrT2.ClientID%>").prop("checked")) {
                if ($.trim($("#<%=txtTurnsAgentXX1.ClientID%>").val()) == "") { alert("物业部代理信息名称1不可为空！"); $("#<%=txtTurnsAgentXX1.ClientID%>").focus(); return false; }
                if ($.trim($("#<%=txtTurnsAgentXX2.ClientID%>").val()) == "") { alert("物业部代理信息名称2不可为空！"); $("#<%=txtTurnsAgentXX2.ClientID%>").focus(); return false; }

                if ($.trim($("#<%=txtAgencyFee3.ClientID%>").val()) == "") { alert("物业部代理信息代理费1不可为空！"); $("#<%=txtAgencyFee3.ClientID%>").focus(); return false; }
                if ($.trim($("#<%=txtAgencyFee4.ClientID%>").val()) == "") { alert("物业部代理信息代理费2不可为空！"); $("#<%=txtAgencyFee4.ClientID%>").focus(); return false; }
                if ($("#<%=rdbIsCashPrize31.ClientID%>").prop("checked")) {
                    if ($.trim($("#<%=txtCashPrize3.ClientID%>").val()) == "") { alert("物业部代理信息现金奖1不可为空！"); $("#<%=txtCashPrize3.ClientID%>").focus(); return false; }
	            }
                if ($("#<%=rdbIsCashPrize41.ClientID%>").prop("checked")) {
                    if ($.trim($("#<%=txtCashPrize4.ClientID%>").val()) == "") { alert("物业部代理信息现金奖2不可为空！"); $("#<%=txtCashPrize4.ClientID%>").focus(); return false; }
                }
                if (!$("#<%=rdbIsCashPrize31.ClientID%>").prop("checked") && !$("#<%=rdbIsCashPrize32.ClientID%>").prop("checked")) {
                    alert("请选择是否有物业部代理信息现金奖1");
                    $("#<%=rdbIsCashPrize31.ClientID%>").focus();
	                return false;
                }
                if (!$("#<%=rdbIsCashPrize41.ClientID%>").prop("checked") && !$("#<%=rdbIsCashPrize42.ClientID%>").prop("checked")) {
                    alert("请选择是否有物业部代理信息现金奖2");
                    $("#<%=rdbIsCashPrize41.ClientID%>").focus();
	                return false;
                }

                if ($("#<%=rdbIsPFear31.ClientID%>").prop("checked")) {
                    if ($.trim($("#<%=txtPFear3.ClientID%>").val()) == "") { alert("物业部代理信息项目费用3不可为空！"); $("#<%=txtPFear3.ClientID%>").focus(); return false; }
                }
                if ($("#<%=rdbIsPFear41.ClientID%>").prop("checked")) {
                    if ($.trim($("#<%=txtPFear4.ClientID%>").val()) == "") { alert("物业部代理信息项目费用4不可为空！"); $("#<%=txtPFear4.ClientID%>").focus(); return false; }
                }
                if (!$("#<%=rdbIsPFear31.ClientID%>").prop("checked") && !$("#<%=rdbIsPFear32.ClientID%>").prop("checked")) {
                    alert("请选择是否有物业部代理信息项目费用3");
                    $("#<%=rdbIsPFear31.ClientID%>").focus();
	                return false;
                }
                if (!$("#<%=rdbIsPFear41.ClientID%>").prop("checked") && !$("#<%=rdbIsPFear42.ClientID%>").prop("checked")) {
                    alert("请选择是否有物业部代理信息项目费用4");
                    $("#<%=rdbIsPFear41.ClientID%>").focus();
	                return false;
                }
                if ($.trim($("#<%=txtAgencyBeginDate1.ClientID%>").val()) == "") { alert("物业部代理信息代理开始期1不可为空！"); $("#<%=txtAgencyBeginDate1.ClientID%>").focus(); return false; }
                if ($.trim($("#<%=txtAgencyBeginDate2.ClientID%>").val()) == "") { alert("物业部代理信息代理开始期2不可为空！"); $("#<%=txtAgencyBeginDate2.ClientID%>").focus(); return false; }
                if ($.trim($("#<%=txtAgencyEndDate1.ClientID%>").val()) == "") { alert("物业部代理信息代理结束期1不可为空！"); $("#<%=txtAgencyFee3.ClientID%>").focus(); return false; }
                if ($.trim($("#<%=txtAgencyEndDate2.ClientID%>").val()) == "") { alert("物业部代理信息代理结束期2不可为空！"); $("#<%=txtAgencyFee4.ClientID%>").focus(); return false; }
            }
            //20141027+
            if($.trim($("#<%=txtAreaMaster.ClientID%>").val()) == ""){
                alert("请选择区域"); 
                $("#<%=txtAreaMaster.ClientID%>").focus();
                return false;
            }
        ...
        function upload() {
            ...
            if(sReturnValue=='success')
                window.location='Apply_ProjReDaAdd_Detail.aspx?MainID=<%=MainID %>';
        }

        function editflow(){
            var win=window.showModalDialog("Apply_ProjReDaAdd_Flow.aspx?MainID=<%=MainID %>","","dialogWidth=830px;dialogHeight=280px");
                    if(win=='success')
                        window.location="Apply_ProjReDaAdd_Detail.aspx?MainID=<%=MainID %>";
        }
        ...
        function sign(idx) {
            ...
            //if(idx=='1'||idx=='2'||idx=='4'||idx=='11'){//789
        ...
        function PageInit()
        {
            var val = $("#<%=hdDealOfficeType.ClientID%>").val();
            if(val == undefined || val == "")
                return;
            var temp = val.split('|');
            
            $("#ctl00_ContentPlaceHolder1_cblDealOfficeType label").each(function(){
            ...
            $("#ddlAreaProj").val($("#<%=this.txtAreaMaster.ClientID%>").val());
        }
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <div style='text-align: center; width: 840px; margin: 0 auto;'>
        <div class="noprint">
        <%=SbFlow.ToString() %>
        <uc1:FlowShow ID="FlowShow1" runat="server" />
        ...
        <div style='text-align: center'>
            ...
            <table id="tbAround" width='640px'>
                ...
                <tr>
                    ...
                    <td class="tl PL10"><asp:RadioButton ID="rdbHavePreSaleID" runat="server" Text="是" GroupName="PreSaleID" />(编号<asp:TextBox ID="txtPreSaleID" runat="server" Width="100"></asp:TextBox>)<asp:RadioButton ID="rdbNoPreSaleID" runat="server" Text="否" GroupName="PreSaleID" /></td>
                </tr>
                <tr>
                    <td>项目地址</td>
                ...
                <tr>
                    <td colspan="4">物业部自接信息</td>
                </tr>
                <tr>
                    <td colspan="2">区域</td>
                    <td colspan="2">区域负责人姓名</td>
                </tr>
                <tr>
                    <td colspan="2">
                        <select id="ddlAreaProj" style="width: 50%">
                            <%=SbOptions.ToString() %>
                        </select>
                    </td>
                    <td colspan="2"><asp:textbox id="txtAreaMaster" runat="server" Width="150px" BackColor="#E3E3E3"></asp:textbox><asp:hiddenfield id="hdAreaMaster" runat="server" /></td>
                </tr>
                <tr>
                    <td>物业部代理信息</td>
                    <td colspan="3"class="tl PL10">
                        　<div>　第1点为中原物业部代理信息（必填项）；第2点为同场/轮流代理的行家信息。若因无渠道了解行家相关信息无法填写，敬请注明“无渠道了解相关信息”。</div><br />
                    ...
                    <td colspan="4">
                        <table class='sample tc' width='98%' style="margin:0 auto;">
                            ...
                            <tr>			
                                ...
                                <td><asp:TextBox ID="txtLinkPerformance" runat="server"></asp:TextBox></td>
                            </tr>
                            <tr style="display:none;">
                                ...
                                <td colspan="2"><asp:TextBox ID="txtTotalProfit" runat="server"></asp:TextBox></td>
                            </tr>
                        </table>
                    </td>
                </tr>
                ...
                <tr id="trManager2" class="noborder" style="height: 85px;">
                    ...
                    <td colspan="3" class="tl PL10" style=" ">
                        ...
                        <textarea id="txtIDx2" rows="5" style="width: 98%; overflow: auto;"></textarea><input type="button" id="btnSignIDx2" value="签署意见" onclick="sign('2')" style="display: none;" /><div class="signdate">日期：<span id="spanDateIDx2">_________</span></div>
                    </td>
                </tr>
                <tr id="trShowFlow3" class="noborder" style="height: 85px;">
                    ...
                    <td colspan="3" class="tl PL10" style=" ">
                        ...
                        <textarea id="txtIDx3" rows="5" style="width: 98%; overflow: auto;"></textarea><input type="button" id="btnSignIDx3" value="签署意见" onclick="sign('3')" style="display: none;" /><div class="signdate">日期：<span id="spanDateIDx3">_________</span></div>
                    </td>
                </tr>
                ...
                <tr id="trShowFlow4_5" class="noborder" style="height: 85px;">
                    <td rowspan="1" >物业部区域负责人</td>   
                    <td colspan="3" class="tl PL10" style=" ">
                        <input id="rdbYesIDx5" type="radio" name="agree5" /><label for="rdbYesIDx5">同意</label><input id="rdbOtherIDx5" type="radio" name="agree5" /><label for="rdbOtherIDx5">其他意见</label><br />
                ...
                <tr id="trShowFlow9">
                    <td colspan="3" class="tl PL10" style=" ">
                        ...
                        <textarea id="txtIDx10" rows="5" style="width: 98%; overflow: auto;" cols="20" name="S4"></textarea><input type="button" id="btnSignIDx10" value="签署意见" onclick="sign('10')" style="display: none;" /><div class="signdate">日期：<span id="spanDateIDx10">_________</span></div>
                    </td>
                </tr>
                ...
                <tr id="trLogistics2" class="noborder" style="height: 85px;">
                    ...
                    <td colspan="3" class="tl PL10" style=" ">
                      ...
                      　<a id="dfd" href="javascript:void(0)" style="display: none;" onclick="deleteFlow()">删除添加的审批环节</a><br />
                        <textarea id="txtIDx12" rows="5" style="width: 98%; overflow: auto;"></textarea><input type="button" id="btnSignIDx12" value="签署意见" onclick="sign('12')" style="display: none;" /><div class="signdate">日期：<span id="spanDateIDx12">_________</span></div>
                    </td>
                </tr>
                ...
                <tr id="trGeneralManager" class="noborder" style="height: 85px; ">
                    ...
                    <td colspan="3" class="tl PL10" style=" ">
                        ...
                        <textarea id="txtIDx13" rows="5" style="width: 98%; overflow: auto;"></textarea><input type="button" id="btnSignIDx13" value="签署意见" onclick="sign('13')" style="display: none;" /><div class="signdate">日期：<span id="spanDateIDx13">_________</span></div>
                    </td>
                </tr>
        ...
        <input type="hidden" id="hdDetail" runat="server" />
            ...
            <asp:hiddenfield id="hdTurnsAgentXX1" runat="server" />
