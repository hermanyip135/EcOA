...
public partial class Apply_ProjAgentClause_Apply_ProjAgentClause_Detail : BasePage
{
    #region 变量声明及定义
    bool tpdf = false;
    ...
    public string temMainID = "";//2016/8/24 52100
    #endregion
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            try
            {
                if (Session["FLG_ReWrite14"].ToString() == "1")
                {
                    ViewState["FLG_ReWrite"] = "1";
                    Session["FLG_ReWrite14"] = null;
                }
            }
            catch
            {
    ...
    public void LoadPage()
    {
        ...
        {
            //从暂存xml中读取
            var obj = new Common().GetTempSaveEntity<DataEntity.PageModel.Apply_ProjAgentClause_Detail>("ProjAgentClause", "", Mainobj.OfficeAutomation_SerialNumber);
            ds = Common.GetPageDetailDS<DataEntity.PageModel.T_OfficeAutomation_Document_ProjAgentClause_M>(obj.ProjAgentClause, Mainobj);
        ...
        else
        {
            ...
            //从数据库中读取
            ds = da_OfficeAutomation_Document_ProjAgentClause_Inherit.SelectByMainID(MainID);
        }

        #region FormInit
        DataRow dr = ds.Tables[0].Rows[0];
        ID = dr["OfficeAutomation_Document_ProjAgentClause_ID"].ToString();

        string applicant = dr["OfficeAutomation_Document_ProjAgentClause_Apply"].ToString();
        ApplyN = applicant;
        txtDepartment.Text = dr["OfficeAutomation_Document_ProjAgentClause_Department"].ToString();
        lblApplyDate.Text = DateTime.Parse(dr["OfficeAutomation_Document_ProjAgentClause_ApplyDate"].ToString()).ToString("yyyy-MM-dd");
        lblApply.Text = applicant;
        txtReplyPhone.Text = dr["OfficeAutomation_Document_ProjAgentClause_ReplyPhone"].ToString();
        txtApplyID.Text = dr["OfficeAutomation_Document_ProjAgentClause_ApplyID"].ToString();
        txtProjName.Text = dr["OfficeAutomation_Document_ProjAgentClause_ProjName"].ToString();
        txtDeveloperName.Text = dr["OfficeAutomation_Document_ProjAgentClause_DeveloperName"].ToString();
        txtProjAddress.Text = dr["OfficeAutomation_Document_ProjAgentClause_ProjAddress"].ToString();
        txtAgentStart.Text = dr["OfficeAutomation_Document_ProjAgentClause_AgentStart"].ToString();
        txtAgentEnd.Text = dr["OfficeAutomation_Document_ProjAgentClause_AgentEnd"].ToString();
        txtSaleDate.Text = dr["OfficeAutomation_Document_ProjAgentClause_SaleDate"].ToString();

        InitListControler(dr["OfficeAutomation_Document_ProjAgentClause_DealOfficeTypeIDs"].ToString());

        txtDealOfficeOther.Text = dr["OfficeAutomation_Document_ProjAgentClause_DealOfficeOther"].ToString();
        txtGoodsQuantity.Text = dr["OfficeAutomation_Document_ProjAgentClause_GoodsQuantity"].ToString();
        txtGoodsValue.Text = dr["OfficeAutomation_Document_ProjAgentClause_GoodsValue"].ToString();
        txtPreComm.Text = dr["OfficeAutomation_Document_ProjAgentClause_PreComm"].ToString();

        switch (dr["OfficeAutomation_Document_ProjAgentClause_AgentModel"].ToString())
        {
            case "1":
        ...
        switch (dr["OfficeAutomation_Document_ProjAgentClause_PorjectExam"].ToString())
        {
            case "True":
                projectEY.Checked = true;
                break;
            case "False":
                projectEN.Checked = true;
                break;
            default:
                break;
        }

        txtCommPoint.Text = dr["OfficeAutomation_Document_ProjAgentClause_CommPoint"].ToString();

        switch (dr["OfficeAutomation_Document_ProjAgentClause_ContractType"].ToString())
        {
            case "1":
        ...
        txtContractTypeOther.Text = dr["OfficeAutomation_Document_ProjAgentClause_ContractTypeOther"].ToString();
        txtRemark.Text = dr["OfficeAutomation_Document_ProjAgentClause_Remark"].ToString();
        //txtSumPerformance.Text = dr["OfficeAutomation_Document_ProjAgentClause_SumPerformance"].ToString();
        //txtSumProfit.Text = dr["OfficeAutomation_Document_ProjAgentClause_SumProfit"].ToString();

        //string[] performanceProfitInfo = Regex.Split(dr["OfficeAutomation_Document_ProjAgentClause_PerformanceProfitInfo"].ToString(), "\\|\\|");

        //DrawDetailTable(performanceProfitInfo.Length);

        //for (int n = 0; n < performanceProfitInfo.Length; n++)
        //{
        //    if (performanceProfitInfo[n] != "")
        //    {
        //        string[] info = Regex.Split(performanceProfitInfo[n], "\\&\\&");

        //        int i = n + 1;

        //        SbJs.Append("$('#txtStartYear" + i + "').val('" + info[0] + "');");
        //        SbJs.Append("$('#txtEndYear" + i + "').val('" + info[1] + "');");
        //        SbJs.Append("$('#txtPerformance" + i + "').val('" + info[2] + "');");
        //        SbJs.Append("$('#txtProfit" + i + "').val('" + info[3] + "');");
        //    }
        //}
        txtClauseSettleComm.Text = dr["OfficeAutomation_Document_ProjAgentClause_ClauseSettleComm"].ToString();
        txtClauseFine.Text = dr["OfficeAutomation_Document_ProjAgentClause_ClauseFine"].ToString();
        txtClauseAreaLimit.Text = dr["OfficeAutomation_Document_ProjAgentClause_ClauseAreaLimit"].ToString();
        txtClausePerforCheck.Text = dr["OfficeAutomation_Document_ProjAgentClause_ClausePerforCheck"].ToString();
        txtClauseHonest.Text = dr["OfficeAutomation_Document_ProjAgentClause_ClauseHonest"].ToString();
        txtClauseOther.Text = dr["OfficeAutomation_Document_ProjAgentClause_ClauseOther"].ToString();

        if (flowState == "3")
        {
            txtClauseSettleComm.Visible = !string.IsNullOrEmpty(txtClauseSettleComm.Text);
            txtClauseFine.Visible = !string.IsNullOrEmpty(txtClauseFine.Text);
            txtClauseAreaLimit.Visible = !string.IsNullOrEmpty(txtClauseAreaLimit.Text);
            txtClausePerforCheck.Visible = !string.IsNullOrEmpty(txtClausePerforCheck.Text);
            txtClauseHonest.Visible = !string.IsNullOrEmpty(txtClauseHonest.Text);
            txtClauseOther.Visible = !string.IsNullOrEmpty(txtClauseOther.Text);
        }

        //20141027+
        if (dr["OfficeAutomation_Document_ProjAgentClause_JOrT"].ToString() == "1")
        {
            rdbJOrT1.Checked = true;
            txtSamePlaceXX1.Text = dr["OfficeAutomation_Document_ProjAgentClause_SamePlaceXX1"].ToString();
            txtSamePlaceXX2.Text = dr["OfficeAutomation_Document_ProjAgentClause_SamePlaceXX2"].ToString();
            txtAgencyFee1.Text = dr["OfficeAutomation_Document_ProjAgentClause_AgencyFee1"].ToString();
            txtAgencyFee2.Text = dr["OfficeAutomation_Document_ProjAgentClause_AgencyFee2"].ToString();
            if (dr["OfficeAutomation_Document_ProjAgentClause_IsCashPrize1"].ToString() == "True")
            {
                rdbIsCashPrize11.Checked = true;
                txtCashPrize1.Text = dr["OfficeAutomation_Document_ProjAgentClause_CashPrize1"].ToString();
            }
            else if (dr["OfficeAutomation_Document_ProjAgentClause_IsCashPrize1"].ToString() == "False")
                rdbIsCashPrize12.Checked = true;
            if (dr["OfficeAutomation_Document_ProjAgentClause_IsCashPrize2"].ToString() == "True")
            {
                rdbIsCashPrize21.Checked = true;
                txtCashPrize2.Text = dr["OfficeAutomation_Document_ProjAgentClause_CashPrize2"].ToString();
            }
            else if (dr["OfficeAutomation_Document_ProjAgentClause_IsCashPrize2"].ToString() == "False")
                rdbIsCashPrize22.Checked = true;

            if (dr["OfficeAutomation_Document_ProjAgentClause_IsPFear1"].ToString() == "True")
            {
                rdbIsPFear11.Checked = true;
                txtPFear1.Text = dr["OfficeAutomation_Document_ProjAgentClause_PFear1"].ToString();
            }
            else if (dr["OfficeAutomation_Document_ProjAgentClause_IsPFear1"].ToString() == "False")
                rdbIsPFear12.Checked = true;
            if (dr["OfficeAutomation_Document_ProjAgentClause_IsPFear2"].ToString() == "True")
            {
                rdbIsPFear21.Checked = true;
                txtPFear2.Text = dr["OfficeAutomation_Document_ProjAgentClause_PFear2"].ToString();
            }
            else if (dr["OfficeAutomation_Document_ProjAgentClause_IsPFear2"].ToString() == "False")
                rdbIsPFear22.Checked = true;
        }
        else if (dr["OfficeAutomation_Document_ProjAgentClause_JOrT"].ToString() == "2")
        {
            rdbJOrT2.Checked = true;
            txtTurnsAgentXX1.Text = dr["OfficeAutomation_Document_ProjAgentClause_TurnsAgentXX1"].ToString();
            txtTurnsAgentXX2.Text = dr["OfficeAutomation_Document_ProjAgentClause_TurnsAgentXX2"].ToString();
            txtAgencyFee3.Text = dr["OfficeAutomation_Document_ProjAgentClause_AgencyFee3"].ToString();
            txtAgencyFee4.Text = dr["OfficeAutomation_Document_ProjAgentClause_AgencyFee4"].ToString();
            if (dr["OfficeAutomation_Document_ProjAgentClause_IsCashPrize3"].ToString() == "True")
            {
                rdbIsCashPrize31.Checked = true;
                txtCashPrize3.Text = dr["OfficeAutomation_Document_ProjAgentClause_CashPrize3"].ToString();
            }
            else if (dr["OfficeAutomation_Document_ProjAgentClause_IsCashPrize3"].ToString() == "False")
                rdbIsCashPrize32.Checked = true;
            if (dr["OfficeAutomation_Document_ProjAgentClause_IsCashPrize4"].ToString() == "True")
            {
                rdbIsCashPrize41.Checked = true;
                txtCashPrize4.Text = dr["OfficeAutomation_Document_ProjAgentClause_CashPrize4"].ToString();
            }
            else if (dr["OfficeAutomation_Document_ProjAgentClause_IsCashPrize4"].ToString() == "False")
                rdbIsCashPrize42.Checked = true;

            if (dr["OfficeAutomation_Document_ProjAgentClause_IsPFear3"].ToString() == "True")
            {
                rdbIsPFear31.Checked = true;
                txtPFear3.Text = dr["OfficeAutomation_Document_ProjAgentClause_PFear3"].ToString();
            }
            else if (dr["OfficeAutomation_Document_ProjAgentClause_IsPFear3"].ToString() == "False")
                rdbIsPFear32.Checked = true;
            if (dr["OfficeAutomation_Document_ProjAgentClause_IsPFear4"].ToString() == "True")
            {
                rdbIsPFear41.Checked = true;
                txtPFear4.Text = dr["OfficeAutomation_Document_ProjAgentClause_PFear4"].ToString();
            }
            else if (dr["OfficeAutomation_Document_ProjAgentClause_IsPFear4"].ToString() == "False")
                rdbIsPFear42.Checked = true;
            txtAgencyBeginDate1.Text = dr["OfficeAutomation_Document_ProjAgentClause_AgencyBeginDate1"].ToString();
            txtAgencyBeginDate2.Text = dr["OfficeAutomation_Document_ProjAgentClause_AgencyBeginDate2"].ToString();
            txtAgencyEndDate1.Text = dr["OfficeAutomation_Document_ProjAgentClause_AgencyEndDate1"].ToString();
            txtAgencyEndDate2.Text = dr["OfficeAutomation_Document_ProjAgentClause_AgencyEndDate2"].ToString();
        }
        else if (dr["OfficeAutomation_Document_ProjAgentClause_JOrT"].ToString() == "3")
        ...
        if (flowState == "2" && (applicant == EmployeeName || EmployeeName == CommonConst.EMP_PROJECT_CLAUSEANDPROJCOST_NAME) && !tpdf && showf) //20141215：M_AlterC
    ...
    public void DrawDetailTable(int n)
    {
        for (int i = 1; i <= n; i++)
        {
            SbHtml.Append("<tr id='trPerformanceProfit" + i + "'>");
            SbHtml.Append("     <td><input type=\"text\" id=\"txtStartYear" + i + "\" style=\"width:90px\"/>至<input type=\"text\" id=\"txtEndYear" + i + "\" style=\"width:90px\"/></td>");
            SbHtml.Append("     <td><input type=\"text\" id=\"txtPerformance" + i + "\"/></td>");
            SbHtml.Append("     <td><input type=\"text\" id=\"txtProfit" + i + "\"/></td>");
            SbHtml.Append("</tr>");
        }
        SbJs.Append("i=" + n + ";");
    }
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        T_OfficeAutomation_Document_ProjAgentClause t_OfficeAutomation_Document_ProjAgentClause = new T_OfficeAutomation_Document_ProjAgentClause();
        ...
        try
        {
            ...
            if (MainID == "")
            {
                ...
                t_OfficeAutomation_Main.OfficeAutomation_SerialNumber = "ProjAgentClause" + DateTime.Now.ToString("yyyyMMddHHmmssfffffff");
                t_OfficeAutomation_Main.OfficeAutomation_DocumentID = 21;
                ...
                t_OfficeAutomation_Document_ProjAgentClause = GetModelFromPage(Guid.NewGuid());
                ...
                da_OfficeAutomation_Document_ProjAgentClause_Inherit.Add(t_OfficeAutomation_Document_ProjAgentClause);//插入申请表
                ...
                RunJS("window.showModalDialog(\"/Apply/Apply_UploadFile.aspx?MainID=" + MainID + "\", '" + MainID + "', \"dialogHeight=165px\");var win=window.showModalDialog(\"Apply_ProjAgentClause_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=800px;dialogHeight=250px\");if(win=='success'){window.location='/Apply/Apply_Search.aspx'; }");
                #endregion
            }
            else
            {
    ...
    {
        var SerialNumber = "ProjAgentClause" + DateTime.Now.ToString("yyyyMMddHHmmss");
        var DocumentID = 21;
        ...
        //判断是否多次点击保存按钮
        DataSet ds = new DataSet();
        var da_OfficeAutomation_Document_ProjAgentClause_Inherit = new DA_OfficeAutomation_Document_ProjAgentClause_Inherit();
        var ProjAgentClause = new DataEntity.PageModel.T_OfficeAutomation_Document_ProjAgentClause_M();
        ds = da_OfficeAutomation_Document_ProjAgentClause_Inherit.SelectByMainID(MainID);
        if (ds.Tables[0].Rows.Count == 0)
        {
            ProjAgentClause = GetModelMFromPage(Guid.NewGuid());
        }
        else
        {
            var ProjAgentClause_ID = ds.Tables[0].Rows[0]["OfficeAutomation_Document_ProjAgentClause_ID"].ToString();
            ProjAgentClause = GetModelMFromPage(new Guid(ProjAgentClause_ID));
        }
        //var ProjAgentClause = GetModelMFromPage(Guid.NewGuid());
        da_OfficeAutomation_Document_ProjAgentClause_Inherit.HandleBase(ProjAgentClause);//插入关键内容
        var obj = new DataEntity.PageModel.Apply_ProjAgentClause_Detail();
        obj.MainEntity = t_OfficeAutomation_Main;
        obj.ProjAgentClause = ProjAgentClause;

        //写入xml
        var result = new Common().SaveTempSaveFile<DataEntity.PageModel.Apply_ProjAgentClause_Detail>(obj, "ProjAgentClause", "", t_OfficeAutomation_Main.OfficeAutomation_SerialNumber);
        if (result)
        {
    ...
    protected void btnSign_Click(object sender, EventArgs e)
    {
        ...
        DataSet ds = da_OfficeAutomation_Document_ProjAgentClause_Inherit.SelectByMainID(MainID);
        DataRow drRow = ds.Tables[0].Rows[0];
        ID = drRow["OfficeAutomation_Document_ProjAgentClause_ID"].ToString();
        ...
        if (Purview.Contains("OA_ITPower_002"))
        {
            try
            {
                ...
                if (isSignSuccess)
                {
                    ...
                    string apply = ds.Tables[0].Rows[0]["OfficeAutomation_Document_ProjAgentClause_Apply"].ToString();
                    string employname;
                    string serialNumber = drRow["OfficeAutomation_SerialNumber"].ToString();
                    string documentName = da_OfficeAutomation_Flow_Inherit.GetDocumentNameByMainID(MainID);
                    string department = drRow["OfficeAutomation_Document_ProjAgentClause_Department"].ToString();
                    ...
                    sbMailBody.Append("<br/><br/>申请部门：" + drRow["OfficeAutomation_Document_ProjAgentClause_Department"]);
                    sbMailBody.Append("<br/>申请日期：" + DateTime.Parse(drRow["OfficeAutomation_Document_ProjAgentClause_ApplyDate"].ToString()).ToString("yyyy-MM-dd"));
                    sbMailBody.Append("<br/>跟进人：" + drRow["OfficeAutomation_Document_ProjAgentClause_Apply"]);
                    sbMailBody.Append("<br/>跟进人联系电话：" + drRow["OfficeAutomation_Document_ProjAgentClause_ReplyPhone"]);
                    sbMailBody.Append("<br/>项目名称：" + drRow["OfficeAutomation_Document_ProjAgentClause_ProjName"]);
                    sbMailBody.Append("<br/>开发商名称：" + drRow["OfficeAutomation_Document_ProjAgentClause_DeveloperName"]);
                    sbMailBody.Append("<br/>项目地址：" + drRow["OfficeAutomation_Document_ProjAgentClause_ProjAddress"]);
                    sbMailBody.Append("<br/>代理期：" + DateTime.Parse(drRow["OfficeAutomation_Document_ProjAgentClause_AgentStart"].ToString()).ToString("yyyy-MM-dd") + "至" + DateTime.Parse(drRow["OfficeAutomation_Document_ProjAgentClause_AgentEnd"].ToString()).ToString("yyyy-MM-dd"));
                    DateTime saleDate = new DateTime();
                    string sSaleDate;
                    if (DateTime.TryParse(drRow["OfficeAutomation_Document_ProjAgentClause_SaleDate"].ToString(), out saleDate))
                        sSaleDate = saleDate.ToString("yyyy-MM-dd");
                    else
                        sSaleDate = drRow["OfficeAutomation_Document_ProjAgentClause_SaleDate"].ToString();
                    sbMailBody.Append("<br/>开售日：" + sSaleDate);
                    sbMailBody.Append("<br/>货量：" + drRow["OfficeAutomation_Document_ProjAgentClause_GoodsQuantity"]);
                    sbMailBody.Append("<br/>货值：" + drRow["OfficeAutomation_Document_ProjAgentClause_GoodsValue"]);
                    sbMailBody.Append("<br/>预计创佣：" + drRow["OfficeAutomation_Document_ProjAgentClause_PreComm"]);
                    sbMailBody.Append("<br/>佣金点数：" + drRow["OfficeAutomation_Document_ProjAgentClause_CommPoint"]);
                    sbMailBody.Append("<br/>备注：" + drRow["OfficeAutomation_Document_ProjAgentClause_Remark"]);
                    sbMailBody.Append("<br/><br/>结佣条件条款：" + drRow["OfficeAutomation_Document_ProjAgentClause_ClauseSettleComm"]);
                    sbMailBody.Append("<br/>扣罚条款：" + drRow["OfficeAutomation_Document_ProjAgentClause_ClauseFine"]);
                    sbMailBody.Append("<br/>区域限制条款：" + drRow["OfficeAutomation_Document_ProjAgentClause_ClauseAreaLimit"]);
                    sbMailBody.Append("<br/>绩效考核条款：" + drRow["OfficeAutomation_Document_ProjAgentClause_ClausePerforCheck"]);
                    sbMailBody.Append("<br/>廉洁条款：" + drRow["OfficeAutomation_Document_ProjAgentClause_ClauseHonest"]);
                    sbMailBody.Append("<br/>其他条款：" + drRow["OfficeAutomation_Document_ProjAgentClause_ClauseOther"]);
                        ...
                        if (hdIsAgree.Value == "2" && drc[4]["OfficeAutomation_Flow_AuditorID"].ToString() != "") //其它意见，通知法律部++++
                        {
                            ...
                            for (int i = 0; i < employnames.Length; i++)
                            {
                                if (!employeeList.Contains(employnames[i]))
                                {
                                    ...
                                    email = employnames[i];
                                    Common.SendMessageEX(false, email, "关于项目销售代理合同条款申请表的其它意见", msnBody + mailBody, msnBody);
                        ...
                        if (drc[4]["OfficeAutomation_Flow_AuditorID"].ToString() != "") //不同意，通知法律部++++
                        {
                            ...
                            for (int i = 0; i < employnames.Length; i++)
                            {
                                if (!employeeList.Contains(employnames[i]))
                                {
                                    ...
                                    email = employnames[i];
                                    Common.SendMessageEX(false, email, "请注意，有一份项目销售代理合同条款申请表未通过审批", msnBody + mailBody, msnBody);
    ...
    protected void btnSignSave_Click(object sender, EventArgs e)
    {
        ...
        //        da_OfficeAutomation_Document_ProjAgentClause_Inherit.AddRemarkByID(ID, CommonConst.SIGN_FINANCE);
        //    Alert("标记成功。");
        //}
        //catch
        //{
    ...
    private void Update()
    {
        ...
        DataSet ds = new DataSet();
        ds = da_OfficeAutomation_Document_ProjAgentClause_Inherit.SelectByMainID(MainID);
        ID = ds.Tables[0].Rows[0]["OfficeAutomation_Document_ProjAgentClause_ID"].ToString();

        t_OfficeAutomation_Document_ProjAgentClause = GetModelFromPage(new Guid(ID));
        string apply = "";
        ...
        da_OfficeAutomation_Document_ProjAgentClause_Inherit.Update(t_OfficeAutomation_Document_ProjAgentClause);//修改申请表
    ...
    private T_OfficeAutomation_Document_ProjAgentClause GetModelFromPage(Guid ProjAgentClauseID)
    {
        DataEntity.T_OfficeAutomation_Document_ProjAgentClause model = new DataEntity.T_OfficeAutomation_Document_ProjAgentClause();
        model.OfficeAutomation_Document_ProjAgentClause_ID = ProjAgentClauseID;
        model.OfficeAutomation_Document_ProjAgentClause_MainID = new Guid(MainID);
        model.OfficeAutomation_Document_ProjAgentClause_Department = txtDepartment.Text;
        model.OfficeAutomation_Document_ProjAgentClause_ApplyDate = DateTime.Now;
        model.OfficeAutomation_Document_ProjAgentClause_Apply = EmployeeName;
        model.OfficeAutomation_Document_ProjAgentClause_ReplyPhone = txtReplyPhone.Text;
        model.OfficeAutomation_Document_ProjAgentClause_ApplyID = txtApplyID.Text;
        model.OfficeAutomation_Document_ProjAgentClause_ProjName = txtProjName.Text;
        model.OfficeAutomation_Document_ProjAgentClause_DeveloperName = txtDeveloperName.Text;
        model.OfficeAutomation_Document_ProjAgentClause_ProjAddress = txtProjAddress.Text;
        model.OfficeAutomation_Document_ProjAgentClause_AgentStart = txtAgentStart.Text;
        model.OfficeAutomation_Document_ProjAgentClause_AgentEnd = txtAgentEnd.Text;
        model.OfficeAutomation_Document_ProjAgentClause_SaleDate = txtSaleDate.Text;
        model.OfficeAutomation_Document_ProjAgentClause_DealOfficeTypeIDs = sOfficeType; // hdDealOfficeType.Value;
        model.OfficeAutomation_Document_ProjAgentClause_DealOfficeOther = txtDealOfficeOther.Text;
        model.OfficeAutomation_Document_ProjAgentClause_GoodsQuantity = txtGoodsQuantity.Text;
        model.OfficeAutomation_Document_ProjAgentClause_GoodsValue = txtGoodsValue.Text;
        model.OfficeAutomation_Document_ProjAgentClause_PreComm = txtPreComm.Text;
        model.OfficeAutomation_Document_ProjAgentClause_AgentModel = rdbAgentModel1.Checked ? 1 : 2;

        model.OfficeAutomation_Document_ProjAgentClause_PorjectExam = projectEY.Checked ? true : false;

        model.OfficeAutomation_Document_ProjAgentClause_CommPoint = txtCommPoint.Text;
        model.OfficeAutomation_Document_ProjAgentClause_ContractType = rdbContractType1.Checked ? 1 : (rdbContractType2.Checked ? 2 : 3);
        model.OfficeAutomation_Document_ProjAgentClause_ContractTypeOther = txtContractTypeOther.Text;
        model.OfficeAutomation_Document_ProjAgentClause_Remark = txtRemark.Text;
        //model.OfficeAutomation_Document_ProjAgentClause_PerformanceProfitInfo = hdPerformanceProfit.Value;
        //model.OfficeAutomation_Document_ProjAgentClause_SumPerformance = txtSumPerformance.Text;
        //model.OfficeAutomation_Document_ProjAgentClause_SumProfit = txtSumProfit.Text;
        model.OfficeAutomation_Document_ProjAgentClause_ClauseSettleComm = txtClauseSettleComm.Text;
        model.OfficeAutomation_Document_ProjAgentClause_ClauseFine = txtClauseFine.Text;
        model.OfficeAutomation_Document_ProjAgentClause_ClauseAreaLimit = txtClauseAreaLimit.Text;
        model.OfficeAutomation_Document_ProjAgentClause_ClausePerforCheck = txtClausePerforCheck.Text;
        model.OfficeAutomation_Document_ProjAgentClause_ClauseHonest = txtClauseHonest.Text;
        model.OfficeAutomation_Document_ProjAgentClause_ClauseOther = txtClauseOther.Text;

        //20141027+
        model.OfficeAutomation_Document_ProjAgentClause_JOrT = rdbJOrT1.Checked ? 1 : rdbJOrT2.Checked ? 2 : rdbJOrT3.Checked ? 3 : 0;
        model.OfficeAutomation_Document_ProjAgentClause_SamePlaceXX1 = txtSamePlaceXX1.Text;
        model.OfficeAutomation_Document_ProjAgentClause_SamePlaceXX2 = txtSamePlaceXX2.Text;
        model.OfficeAutomation_Document_ProjAgentClause_TurnsAgentXX1 = txtTurnsAgentXX1.Text;
        model.OfficeAutomation_Document_ProjAgentClause_TurnsAgentXX2 = txtTurnsAgentXX2.Text;
        model.OfficeAutomation_Document_ProjAgentClause_AgencyFee1 = txtAgencyFee1.Text;
        model.OfficeAutomation_Document_ProjAgentClause_AgencyFee2 = txtAgencyFee2.Text;
        model.OfficeAutomation_Document_ProjAgentClause_AgencyFee3 = txtAgencyFee3.Text;
        model.OfficeAutomation_Document_ProjAgentClause_AgencyFee4 = txtAgencyFee4.Text;
        model.OfficeAutomation_Document_ProjAgentClause_IsCashPrize1 = rdbIsCashPrize11.Checked;
        model.OfficeAutomation_Document_ProjAgentClause_IsCashPrize2 = rdbIsCashPrize21.Checked;
        model.OfficeAutomation_Document_ProjAgentClause_IsCashPrize3 = rdbIsCashPrize31.Checked;
        model.OfficeAutomation_Document_ProjAgentClause_IsCashPrize4 = rdbIsCashPrize41.Checked;
        model.OfficeAutomation_Document_ProjAgentClause_CashPrize1 = txtCashPrize1.Text;
        model.OfficeAutomation_Document_ProjAgentClause_CashPrize2 = txtCashPrize2.Text;
        model.OfficeAutomation_Document_ProjAgentClause_CashPrize3 = txtCashPrize3.Text;
        model.OfficeAutomation_Document_ProjAgentClause_CashPrize4 = txtCashPrize4.Text;
        model.OfficeAutomation_Document_ProjAgentClause_AgencyBeginDate1 = txtAgencyBeginDate1.Text;
        model.OfficeAutomation_Document_ProjAgentClause_AgencyBeginDate2 = txtAgencyBeginDate2.Text;
        model.OfficeAutomation_Document_ProjAgentClause_AgencyEndDate1 = txtAgencyEndDate1.Text;
        model.OfficeAutomation_Document_ProjAgentClause_AgencyEndDate2 = txtAgencyEndDate2.Text;
        model.OfficeAutomation_Document_ProjAgentClause_IsPFear1 = rdbIsPFear11.Checked;
        model.OfficeAutomation_Document_ProjAgentClause_IsPFear2 = rdbIsPFear21.Checked;
        model.OfficeAutomation_Document_ProjAgentClause_IsPFear3 = rdbIsPFear31.Checked;
        model.OfficeAutomation_Document_ProjAgentClause_IsPFear4 = rdbIsPFear41.Checked;
        model.OfficeAutomation_Document_ProjAgentClause_PFear1 = txtPFear1.Text;
        model.OfficeAutomation_Document_ProjAgentClause_PFear2 = txtPFear2.Text;
        model.OfficeAutomation_Document_ProjAgentClause_PFear3 = txtPFear3.Text;
        model.OfficeAutomation_Document_ProjAgentClause_PFear4 = txtPFear4.Text;
        //20141027+
    ...
    //暂时保存
    private DataEntity.PageModel.T_OfficeAutomation_Document_ProjAgentClause_M GetModelMFromPage(Guid ProjAgentClauseID)
    {
        DataEntity.PageModel.T_OfficeAutomation_Document_ProjAgentClause_M model = new DataEntity.PageModel.T_OfficeAutomation_Document_ProjAgentClause_M();
        model.OfficeAutomation_Document_ProjAgentClause_ID = ProjAgentClauseID.ToString();
        model.OfficeAutomation_Document_ProjAgentClause_MainID = MainID;
        model.OfficeAutomation_Document_ProjAgentClause_Department = txtDepartment.Text != null ? txtDepartment.Text : "";
        model.OfficeAutomation_Document_ProjAgentClause_ApplyDate = DateTime.Now.ToString();
        model.OfficeAutomation_Document_ProjAgentClause_Apply = EmployeeName;
        model.OfficeAutomation_Document_ProjAgentClause_ReplyPhone = txtReplyPhone.Text != null ? txtReplyPhone.Text : "";
        model.OfficeAutomation_Document_ProjAgentClause_ApplyID = txtApplyID.Text != null ? txtApplyID.Text : "";
        model.OfficeAutomation_Document_ProjAgentClause_ProjName = txtProjName.Text != null ? txtProjName.Text : "";
        model.OfficeAutomation_Document_ProjAgentClause_DeveloperName = txtDeveloperName.Text != null ? txtDeveloperName.Text : "";
        model.OfficeAutomation_Document_ProjAgentClause_ProjAddress = txtProjAddress.Text != null ? txtProjAddress.Text : "";
        model.OfficeAutomation_Document_ProjAgentClause_AgentStart = txtAgentStart.Text != null ? txtAgentStart.Text : "";
        model.OfficeAutomation_Document_ProjAgentClause_AgentEnd = txtAgentEnd.Text != null ? txtAgentEnd.Text : "";
        model.OfficeAutomation_Document_ProjAgentClause_SaleDate = txtSaleDate.Text != null ? txtSaleDate.Text : "";
        model.OfficeAutomation_Document_ProjAgentClause_DealOfficeTypeIDs = sOfficeType; // hdDealOfficeType.Value;
        model.OfficeAutomation_Document_ProjAgentClause_DealOfficeOther = txtDealOfficeOther.Text != null ? txtDealOfficeOther.Text : "";
        model.OfficeAutomation_Document_ProjAgentClause_GoodsQuantity = txtGoodsQuantity.Text != null ? txtGoodsQuantity.Text : "";
        model.OfficeAutomation_Document_ProjAgentClause_GoodsValue = txtGoodsValue.Text != null ? txtGoodsValue.Text : "";
        model.OfficeAutomation_Document_ProjAgentClause_PreComm = txtPreComm.Text != null ? txtPreComm.Text : "";
        model.OfficeAutomation_Document_ProjAgentClause_AgentModel = rdbAgentModel1.Checked ? "1" :(rdbAgentModel2.Checked ? "2":"3");

        if (projectEY.Checked || projectEN.Checked) 
        {
            model.OfficeAutomation_Document_ProjAgentClause_PorjectExam = projectEY.Checked.ToString();
        }

        model.OfficeAutomation_Document_ProjAgentClause_CommPoint = txtCommPoint.Text != null ? txtCommPoint.Text : "";
        model.OfficeAutomation_Document_ProjAgentClause_ContractType = rdbContractType1.Checked ? "1" : (rdbContractType2.Checked ? "2" : (rdbContractType3.Checked ? "3":"4"));
        model.OfficeAutomation_Document_ProjAgentClause_ContractTypeOther = txtContractTypeOther.Text != null ? txtContractTypeOther.Text : "";
        model.OfficeAutomation_Document_ProjAgentClause_Remark = txtRemark.Text;
        model.OfficeAutomation_Document_ProjAgentClause_ClauseSettleComm = txtClauseSettleComm.Text != null ? txtClauseSettleComm.Text : "";
        model.OfficeAutomation_Document_ProjAgentClause_ClauseFine = txtClauseFine.Text != null ? txtClauseFine.Text : "";
        model.OfficeAutomation_Document_ProjAgentClause_ClauseAreaLimit = txtClauseAreaLimit.Text != null ? txtClauseAreaLimit.Text : "";
        model.OfficeAutomation_Document_ProjAgentClause_ClausePerforCheck = txtClausePerforCheck.Text != null ? txtClausePerforCheck.Text : "";
        model.OfficeAutomation_Document_ProjAgentClause_ClauseHonest = txtClauseHonest.Text != null ? txtClauseHonest.Text : "";
        model.OfficeAutomation_Document_ProjAgentClause_ClauseOther = txtClauseOther.Text;
        ...
        {
            model.OfficeAutomation_Document_ProjAgentClause_IsCashPrize1 = rdbIsCashPrize11.Checked.ToString();
        }
        if (rdbIsCashPrize21.Checked || rdbIsCashPrize22.Checked)
        {
            model.OfficeAutomation_Document_ProjAgentClause_IsCashPrize2 = rdbIsCashPrize21.Checked.ToString();
        }
        if (rdbIsCashPrize31.Checked || rdbIsCashPrize32.Checked)
        {
            model.OfficeAutomation_Document_ProjAgentClause_IsCashPrize3 = rdbIsCashPrize31.Checked.ToString();
        }
        if (rdbIsCashPrize41.Checked || rdbIsCashPrize42.Checked)
        {
            model.OfficeAutomation_Document_ProjAgentClause_IsCashPrize4 = rdbIsCashPrize41.Checked.ToString();
        }
        ...
        {
            model.OfficeAutomation_Document_ProjAgentClause_IsPFear1 = rdbIsPFear11.Checked.ToString();
        }
        if (rdbIsPFear21.Checked || rdbIsPFear22.Checked)
        {
            model.OfficeAutomation_Document_ProjAgentClause_IsPFear2 = rdbIsPFear21.Checked.ToString();
        }
        if (rdbIsPFear31.Checked || rdbIsPFear32.Checked)
        {
            model.OfficeAutomation_Document_ProjAgentClause_IsPFear3 = rdbIsPFear31.Checked.ToString();
        }
        if (rdbIsPFear41.Checked || rdbIsPFear42.Checked)
        {
            model.OfficeAutomation_Document_ProjAgentClause_IsPFear4 = rdbIsPFear41.Checked.ToString();
        }
        model.OfficeAutomation_Document_ProjAgentClause_PFear1 = txtPFear1.Text;
        model.OfficeAutomation_Document_ProjAgentClause_PFear2 = txtPFear2.Text;
        model.OfficeAutomation_Document_ProjAgentClause_PFear3 = txtPFear3.Text;
    ...
    protected void btnReWrite_Click(object sender, EventArgs e)
    {
        ...
        Session["FLG_ReWrite14"] = "1";
        Response.Write("<script>window.open('Apply_ProjAgentClause_Detail.aspx?MainID=" + MainID + "');</script>");
    }
    protected void btnSPDF_Click(object sender, EventArgs e) //M_PDF
    {
        try
        {
            ...
            Response.AddHeader("content-disposition", "attachment; filename=" + HttpUtility.UrlEncode("项目销售代理合同条款申请表.pdf"));//强制下载 
    ...
    protected void btnSaveLogisticsFlow_Click(object sender, EventArgs e)
    {
        ...
        DataSet ds = new DataSet();
        ds = da_OfficeAutomation_Document_ProjAgentClause_Inherit.SelectByMainID(MainID);
        ID = ds.Tables[0].Rows[0]["OfficeAutomation_Document_ProjAgentClause_ID"].ToString(); //在不同的表要注意修改
    ...
    protected void btnCancelSign_Click(object sender, EventArgs e) //20141202：CancelSign
    {
        try
        {
            ...
            string signEmployeeName = EmployeeName;
            DA_OfficeAutomation_Document_ProjAgentClause_Inherit da_OfficeAutomation_Document_ProjAgentClause_Inherit = new DA_OfficeAutomation_Document_ProjAgentClause_Inherit();
            DataSet ds = da_OfficeAutomation_Document_ProjAgentClause_Inherit.SelectByMainID(MainID); //在不同的表中要修改
            DataRow drRow = ds.Tables[0].Rows[0];
            string apply = ds.Tables[0].Rows[0]["OfficeAutomation_Document_ProjAgentClause_Apply"].ToString();
            string employname;
            string serialNumber = drRow["OfficeAutomation_SerialNumber"].ToString();
    ...
    protected void btnEditFlow_DoClick(object sender, EventArgs e)
    {
        try
        {
            ...
            //在不同的表中要修改
            RunJS("var win=window.showModalDialog(\"Apply_ProjAgentClause_Flow.aspx?MainID=" + MainID + "\",\"\",\"dialogWidth=830px;dialogHeight=280px\");if(win=='success'){window.location='/Ajax/Editor_Flow.ashx?MainID=" + MainID + "&applyUrl=" + applyUrl + "&apply=" + apply + "&serialNumber=" + serialNumber + "&department=" + department + "&Jhref=" + Page.Request.Url + "&mid=" + EmployeeID + "'; }");
    ...
    {
        DA_OfficeAutomation_Flow_Inherit da_OfficeAutomation_Flow_Inherit = new DA_OfficeAutomation_Flow_Inherit();
        DA_OfficeAutomation_Document_ProjAgentClause_Inherit da_OfficeAutomation_Document_BadDebts_Inherit = new DA_OfficeAutomation_Document_ProjAgentClause_Inherit();
