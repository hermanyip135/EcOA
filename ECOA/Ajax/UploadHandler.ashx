    ...
    public void ProcessRequest (HttpContext context) {
        ...
        if (file != null)
        {
            ...
            else if (applytype == "FinAffairs")
            {
                ...
                string datumtypename = datumtype == "1" ? "分行行政助理交接清单" : datumtype == "2" ? "代收款收据及发票待处理的明细" : datumtype == "3" ? "秘书交接表(分行匙柜部分)" : datumtype == "4" ? "营业员应收佣金表" : datumtype == "5" ? "分行固定资产明细表" : "";
                sb.Append("{");
                sb.Append("'name':");
                //sb.Append("'" + Uri.EscapeDataString(attchname) + "'");
                sb.Append("'" +attchname + "'");
