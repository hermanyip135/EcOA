...
public class HR_Handler : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        switch (action)
        {
            ...
            case "getEmpInfoByEmpName":
                ...
                break;
            case "getEmployeeByCodeDate":
                getEmployeeByCodeDate(context);
                break;
            case "GetEmployeeAndLabour":
                GetEmployeeAndLabour(context);
                break;
            default:
                break;
        }
    ...
    public void GetEmployeeAndLabour(HttpContext context)
    {
        ...
        if (code != "")
        {
            ...
            ds = da_Employee_Inherit.GetEmployeeAndLabour(code);
            if (ds != null && ds.Tables != null && ds.Tables[0].Rows.Count > 0)
            {
                ...
                try
                {
                    ...
                    if("1900-01-01".Equals(DateTime.Parse(dr["InDueDate"].ToString()).ToString("yyyy-MM-dd")))
                    {
                     s = "";
                    }
                }
    ...
    public void getEmployeeByCodeDate(HttpContext context)
    {
         DataAccess.KDHR.Service wsKDHR = new DataAccess.KDHR.Service();
   
         string code = BasePage.GetFormString("code");
         string ExDate = BasePage.GetFormString("ExDate");
         if (code != "")
         {
             System.Data.DataSet ds = new System.Data.DataSet();
            ds = wsKDHR.getEmployeeDetail(Convert.ToDateTime(ExDate), code);
            if (ds.Tables[0].Rows.Count <= 0)
            {
              
              ds =  wsKDHR.getEmployeeDetailInLabour(Convert.ToDateTime(ExDate), code);
            }
            if (ds != null && ds.Tables != null && ds.Tables[0].Rows.Count > 0)
            {
                System.Data.DataRow dr = ds.Tables[0].Rows[0];
                context.Response.Write(dr["EmployeeID"].ToString() + "|"
                   + dr["EmployeeName"].ToString() + "|"
                   + dr["UnitName"].ToString() 
    ...
    public void GetHRTreeByDepartmentID(HttpContext context)
    {
        ...
        else
        {
            ...
            //项目部
            if ("ee7ec188-3cfa-4138-8d4c-97516c07a250".Equals(departmentid))
            { departmentid = "69DF1A40-E9FB-4E87-B64F-92A3C09FE3EB"; }
