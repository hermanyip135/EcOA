...
public class Apply_Handler : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        switch (action)
        {
            case "saveitpowerdeal":
                ...
                break;
            case "saveuireqfollower":
                SaveUIReqFollower(context);
                break;
            case "saveuireqplantime":
                SaveUIReqPlanTime(context);
                break;
        ...
        //{
            ...
            try
            {
                ...
                for (int i2 = 0; i2 < as1.Length; i2++)
                {
                    ...
                    if (as1[i2] == "0002")
                    {
                        if ((s5.IndexOf("0002") > -1 && s5.IndexOf("1909") <= -1) || (s5.IndexOf("0002") <= -1 && s5.IndexOf("1909") > -1))
                        {
                            s7 += as1[i2] + ",";
                        }
                    }
                   else if (as1[i2] != "18530")
                    {
    ...
    #region UI设计需求申请
    public void SaveUIReqFollower(HttpContext context)
    {
        ...
        if (id != "")
        {
            DataAccess.Operate.DA_OfficeAutomation_Document_UIReq_Inherit da_OfficeAutomation_Document_UIReq_Inherit = new DataAccess.Operate.DA_OfficeAutomation_Document_UIReq_Inherit();
            DataAccess.Operate.DA_Employee_Inherit da_Employee_Inherit = new DataAccess.Operate.DA_Employee_Inherit();
            System.Data.DataRow dr = da_OfficeAutomation_Document_UIReq_Inherit.SelectByID(id).Tables[0].Rows[0];

            if (da_OfficeAutomation_Document_UIReq_Inherit.UpdateFollowerByID(id, follower))
            {
                //通知跟进人处理申请表
                ...
                for (int i = 0; i < followers.Length; i++)
                {
                    if (followers[i] != "")
                    {
                        messageBody = "您好，" + followers[i] + "：您有编号为" + dr["OfficeAutomation_SerialNumber"].ToString() + "的UI设计需求申请需要您来处理，请在查看后填写资讯科技部预计完成时间。申请表地址为"
                           + System.Configuration.ConfigurationManager.AppSettings["URL"].ToString() + "Apply/UIReq/Apply_UIReq_Detail.aspx?MainID=" + dr["OfficeAutomation_Document_UIReq_MainID"].ToString();
                        email = followers[i];
                        Common.SendMessageEX(false, email, "有份UI设计需求申请表需要处理", messageBody, messageBody);
                    }
                }

                messageBody = "您好，" + apply + "：您的编号为" + dr["OfficeAutomation_SerialNumber"].ToString() + "的UI设计需求申请，已安排资讯科技部跟进人为：" + follower + "。登陆地址为："
                        + System.Configuration.ConfigurationManager.AppSettings["LoginURL"].ToString();
                email = apply;
                Common.SendMessageEX(false, email, "UI设计需求申请已安排跟进人", messageBody, messageBody);
    ...
    public void SaveUIReqPlanTime(HttpContext context)
    {
        string id = BasePage.GetFormString("sysreqid");
        ...
        if (id != "")
        {
            ...
            if (da_OfficeAutomation_Document_UIReq_Inherit.UpdateITPlanTimeByID(id, planTime))
            {
                //通知申请人：跟进人已填写预计完成时间
                string messageBody = "您好，" + apply + "：您的编号为" + dr["OfficeAutomation_SerialNumber"].ToString() + "的UI设计需求申请，资讯科技部跟进人："
                    + follower + "，已填写预计完成时间：" + planTime + "。请查看。申请表地址为" + System.Configuration.ConfigurationManager.AppSettings["URL"].ToString()
                    + "Apply/SysReq/Apply_UIReq_Detail.aspx?MainID=" + dr["OfficeAutomation_Document_UIReq_MainID"].ToString();
                string email = apply;
                Common.SendMessageEX(false, email, "有份UI设计需求申请表已填写预计完成时间", messageBody, messageBody);

                //通知软件组主管：跟进人已填写预计完成时间
                email = "赵萍";
                messageBody = "您好，" + email + "：编号为" + dr["OfficeAutomation_SerialNumber"].ToString() + "的UI设计需求申请，资讯科技部跟进人："
                    + follower + "，已填写预计完成时间：" + planTime + "。请查看。申请表地址为" + System.Configuration.ConfigurationManager.AppSettings["URL"].ToString()
                    + "Apply/SysReq/Apply_UIReq_Detail.aspx?MainID=" + dr["OfficeAutomation_Document_UIReq_MainID"].ToString();
                Common.SendMessageEX(false, email, "有份UI设计需求申请表已填写预计完成时间", messageBody, messageBody);
